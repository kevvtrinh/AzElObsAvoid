...
planningTimer = tic;
[result, summaryTemplate] = azElInternal.emptyAzElPlannerResult( obstacles, initialState, goalState, limits, options);
...
[seeds, gridDiagnostics] = azElInternal.search.generateTopologySeeds( ...
...
result = azElInternal.motion.runCorridorPlanner( ...
...
for name = logicalNames
    options.(name) = azElInternal.normalizeLogicalScalar( options.(name), name, "planAzElMotion:InvalidLogicalOption");
end
