...
directSeedIndex = find([seeds.Source] == "directVisibilityEdge", 1);
seedIsBlocked = false(numel(seeds), 1);
if numel(seeds) > 1 && ~isempty(directSeedIndex)
    % A detected collision can reject the straight seed without hiding it.
    seedPrioritySampleCount = 101;
    sampleFraction = linspace(0, 1, seedPrioritySampleCount).';
    directSeed = seeds(directSeedIndex);
    samplePosition_deg = directSeed.position_deg(1, :) + ...
        sampleFraction .* diff(directSeed.position_deg, 1, 1);
    sampleTime_s = initialState.time_s + ...
        sampleFraction * directSeed.EstimatedDuration_s;
    seedIsBlocked(directSeedIndex) = any(queryAzElTimeObstacle( ...
...
for seedIndex = 1:numel(seeds)
    if seedIsBlocked(seedIndex)
        ...
        seedSummaries(seedIndex).TerminationReason = ...
            "seedCollisionBlocked";
        seedSummaries(seedIndex).Message = ...
            "The geometric seed intersects protected obstacle geometry.";
        if options.Verbose
            fprintf("[HS3 seed %d/%d] %s, skipped, reason=%s.\n", ...
