...
%       UseRpAgent and RpAgentFile control the learned curve proposal only.
%       Deterministic search, kinematic certification, and nonlinear
%       collision certification always select the returned curve.
%       RpMaximumRepairAttempts limits proposals for each localized
%       colliding corner. Each later colliding corner receives the same
%       independent budget after the complete motion is re-certified.
...
% A visibility tangent to the protected boundary can be geometrically
% valid but incompatible with a nonzero turn radius. If every normal route
% fails, the outer planner generates a second route family around a
% temporary planning-only clearance. The protected obstacle field and its
% recorded safety margin remain unchanged for final certification.
needsMotionClearanceSearch = ~evaluation.Success && ~endpointBlocked && ...
    sourceObstacleCount > 0;
hasPlanningTime = toc(planningTimer) < options.MaximumPlanningTime_s;
if needsMotionClearanceSearch && hasPlanningTime
    motionClearanceSearchOptions = searchOptions;
    motionClearanceSearchOptions.CandidateClearance_deg = max( ...
        routingClearance_deg, options.TurnRadius_deg + ...
        options.RpCollisionCurveTolerance_deg);
    motionClearanceSearchOptions.BuildFullVisibilityGraphs = true;
    motionClearanceSearchOptions.BuildMergedVisibilityGraph = false;
    stageTimer = tic;
    motionClearanceSearch = buildAzElVisibilityRoutes( ...
        obstacleField, initialState, goalState, ...
        motionClearanceSearchOptions);
    motionClearanceEvaluation = ...
        azElInternal.evaluateAzElMotionCandidates( ...
            motionClearanceSearch, obstacleField, initialState, ...
            goalState, limits, options, endpointBlocked);
    stageRecords(end + 1, 1) = searchStageRecord( ...
        "motionClearance", motionClearanceSearch, ...
        motionClearanceEvaluation, toc(stageTimer));
    search = motionClearanceSearch;
    evaluation = motionClearanceEvaluation;
end
...
options = struct(...
    ...
    "RpCollisionCurveTolerance_deg", 0.001, ...
    "RpRadiusFractionTolerance", 0.025, ...
    "RpMaximumCollisionSamples", 20000, ...
    "RpMaximumRepairAttempts", 24, ...
...
logicalNames = ["AllowAzimuthWrapping" "DetectSnapshotEvents" ...
    "ContinueAfterFirstFeasible" "UseRpAgent" "Verbose"];
...
positiveNames = ["SampleTime_s" "TurnRadius_deg" ...
    "VisibilitySampleStep_deg" "RpCollisionCurveTolerance_deg" ...
    "RpRadiusFractionTolerance"];
for optionIndex = 1:numel(positiveNames)
    validateattributes(options.(positiveNames(optionIndex)), {'numeric'}, ...
        {'real','finite','scalar','positive'});
end
if options.RpRadiusFractionTolerance > 0.1
    error("planAzElMotion:InvalidRpRadiusFractionTolerance", ...
        "RpRadiusFractionTolerance must not exceed 0.1.");
end
...
validateattributes(options.RpMaximumCollisionSamples, {'numeric'}, ...
    {'real', 'finite', 'scalar', 'integer', '>=', 2});
validateattributes(options.RpMaximumRepairAttempts, {'numeric'}, ...
