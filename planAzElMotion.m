...
obstacleField = buildAzElTimeObstacleField( ...
    protectedAzElData, struct("Verbose", options.Verbose));
...
%% Section 4: Assemble Stable Output
elapsedPlanningTime_s = toc(planningTimer);
searchDiagnostics = struct( "VisibilityGraphCount", numel(search.VisibilityGraphs), ...
...
result = struct( "Success", success, "Message", message, ...
    "TerminationReason", terminationReason, ...
    "Options", options, "azElData", protectedAzElData, ...
...
options = struct( "GoalTimeMode", "earliestArrival", ...
    ...
    "MaximumRetimedVisibilityRoutes", 12, "Verbose", false);
end
