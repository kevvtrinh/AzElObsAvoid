...
% --- Run the cheapest sufficient search stage first --------------------
% Direct motion, a grouped dense-scene graph, and full snapshot graphs all
% use the same retiming and continuous validation. A failed cheap stage
% always advances automatically; successful stages advance only when the
% caller explicitly requests continued improvement.
stageRecords = repmat(searchStageRecord(), 0, 1);
directSearchOptions = searchOptions;
directSearchOptions.BuildFullVisibilityGraphs = false;
directSearchOptions.BuildMergedVisibilityGraph = false;
stageTimer = tic;
search = emptyVisibilitySearch(directSearchOptions);
evaluation = azElInternal.evaluateAzElMotionCandidates( ...
    search, obstacleField, initialState, goalState, limits, options, ...
    endpointBlocked);
stageRecords(end + 1, 1) = searchStageRecord( ...
    "direct", search, evaluation, toc(stageTimer));

sourceObstacleCount = numel(obstacleField.Obstacles);
isDenseScene = sourceObstacleCount > 4;
mergedSearch = search;
mergedStageWasRun = false;
needsMoreSearch = ~evaluation.Success || options.ContinueAfterFirstFeasible;
hasPlanningTime = toc(planningTimer) < options.MaximumPlanningTime_s;

if needsMoreSearch && hasPlanningTime && isDenseScene
    mergedSearchOptions = searchOptions;
    mergedSearchOptions.BuildFullVisibilityGraphs = false;
    mergedSearchOptions.BuildMergedVisibilityGraph = true;
    stageTimer = tic;
    mergedSearch = buildAzElVisibilityRoutes(obstacleField, initialState, ...
        goalState, mergedSearchOptions);
    mergedEvaluation = azElInternal.evaluateAzElMotionCandidates( ...
        mergedSearch, obstacleField, initialState, goalState, limits, ...
        options, endpointBlocked);
    stageRecords(end + 1, 1) = searchStageRecord( ...
        "grouped", mergedSearch, mergedEvaluation, toc(stageTimer));
    search = mergedSearch;
    evaluation = mergedEvaluation;
    mergedStageWasRun = true;
end

needsFullSearch = ~evaluation.Success || options.ContinueAfterFirstFeasible;
hasPlanningTime = toc(planningTimer) < options.MaximumPlanningTime_s;
if needsFullSearch && hasPlanningTime
    fullSearchOptions = searchOptions;
    fullSearchOptions.BuildFullVisibilityGraphs = true;
    fullSearchOptions.BuildMergedVisibilityGraph = false;
    stageTimer = tic;
    fullSearch = buildAzElVisibilityRoutes(obstacleField, initialState, ...
        goalState, fullSearchOptions);
    if mergedStageWasRun
        fullSearch.MergedVisibilityGraphs = ...
            mergedSearch.MergedVisibilityGraphs;
        fullSearch.ObstacleMergeDiagnostics = ...
            mergedSearch.ObstacleMergeDiagnostics;
    end
    fullEvaluation = azElInternal.evaluateAzElMotionCandidates( ...
        fullSearch, obstacleField, initialState, goalState, limits, ...
        options, endpointBlocked);
    stageRecords(end + 1, 1) = searchStageRecord( ...
        "full", fullSearch, fullEvaluation, toc(stageTimer));
    search = fullSearch;
...
searchDiagnostics = struct(...
    ...
    "MergedVisibilityGraphCount", ...
    numel(search.MergedVisibilityGraphs), ...
    "SuccessfulMergedVisibilityGraphCount", ...
    nnz([search.MergedVisibilityGraphs.Success]), ...
    ...
    "SearchStages", stageRecords, ...
    "AdaptiveDenseSceneSearch", isDenseScene, ...
    "FullGraphFallbackPerformed", any( ...
    string({stageRecords.Name}) == "full"), ...
...
function record = searchStageRecord(name, search, evaluation, elapsedTime_s)
%% Section 0: Header & Readme
% SYNTAX
%   record = searchStageRecord()
%   record = searchStageRecord(name, search, evaluation, elapsedTime_s)
%**************************************************************************
% PURPOSE
%   - Return stable timing and outcome diagnostics for one adaptive search
%     stage without changing any planner decision.
%**************************************************************************
% INPUTS
%   - name (scalar text)
%   - search, evaluation (scalar structs)
%   - elapsedTime_s (nonnegative numeric scalar)
%**************************************************************************
% OUTPUTS
%   - record (scalar struct)
%**************************************************************************
% UNITS
%   - Elapsed time is seconds; counts are dimensionless.
%**************************************************************************
record = struct( ...
    "Name", "", ...
    "Success", false, ...
    "TerminationReason", "notRun", ...
    "VisibilityGraphCount", 0, ...
    "MergedVisibilityGraphCount", 0, ...
    "CandidateCount", 0, ...
    "ElapsedTime_s", 0);
if nargin == 0
    return;
...
record.TerminationReason = evaluation.TerminationReason;
record.VisibilityGraphCount = numel(search.VisibilityGraphs);
record.MergedVisibilityGraphCount = ...
    numel(search.MergedVisibilityGraphs);
record.CandidateCount = evaluation.CandidateCount;
record.ElapsedTime_s = elapsedTime_s;
end

function search = emptyVisibilitySearch(options)
%% Section 0: Header & Readme
% SYNTAX
%   search = emptyVisibilitySearch(options)
%**************************************************************************
% PURPOSE
%   - Supply the candidate evaluator with a stable empty search so direct
%     motion is tested before any obstacle snapshots are expanded.
%**************************************************************************
% INPUTS
%   - options (scalar resolved search-options struct)
%**************************************************************************
% OUTPUTS
%   - search (scalar search-diagnostics struct)
%**************************************************************************
% UNITS
%   - Candidate points use degrees and seconds; counts are dimensionless.
%**************************************************************************
...
search = struct( ...
    ...
    "ObstacleMergeDiagnostics", struct( ...
        ...
        "AttemptedGraphCount", 0, ...
        "SuccessfulGraphCount", 0, ...
