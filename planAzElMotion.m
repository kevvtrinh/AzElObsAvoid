...
seeds = orderSeedsByOccupancy( ...
    seeds, obstacles, initialState.time_s);
result.Seeds = seeds;
...
for seedIndex = 1:numel(seeds)
    ...
    % Keep time for the bounded collision-driven corridor reassociation.
    initialSolveFraction = 1;
    if options.GoalTimeMode == "earliestArrival" && ...
            obstacleHistoryChanges(obstacles)
        initialSolveFraction = 0.7;
    end
    solverOptions.MaximumSolverTime_s = ...
        initialSolveFraction * seedBudget_s;
    ...
    while finalCandidate.OptimizerFeasible && ...
            ~validation.Passed && ~validation.CollisionFree && ...
        ...
        repairOptions = solverOptions;
        repairOptions.MaximumSolverTime_s = max(0.1, remainingSeedTime_s);
        repairGoalState = goalState;
        if options.GoalTimeMode == "earliestArrival" && ...
                isfinite(finalCandidate.FinalTime_s)
            % Corridor repair preserves the arrival time already found.
            repairOptions.GoalTimeMode = "fixedArrival";
            repairGoalState.time_s = finalCandidate.FinalTime_s;
        end
        finalCandidate = azElInternal.solveAzElHs3( ...
            obstacles, initialState, repairGoalState, limits, ...
            repairOptions, trialSeed);
        validation = validateCandidate( ...
            finalCandidate, obstacles, initialState, goalState, ...
            limits, options);
        if candidateIsPreferred( ...
                finalCandidate, validation, ...
                bestCandidate, bestValidation)
            bestCandidate = finalCandidate;
            bestValidation = validation;
        end
    end
...
if isempty(validatedIndices)
    ...
    bestPartialIndex = bestPartialSeed(seedSummaries);
    result.SearchDiagnostics.BestPartialSeedIndex = bestPartialIndex;
    if bestPartialIndex > 0 && ~isempty(candidates{bestPartialIndex})
        result.SearchDiagnostics.BestPartialTrajectory = ...
            boundedPartialTrajectory(candidates{bestPartialIndex});
    end
    if options.Verbose
...
function preferred = candidateIsPreferred( ...
        candidate, validation, referenceCandidate, referenceValidation)
% PURPOSE
%   - Retain the strongest validation outcome across corridor repairs.
candidateQuality = [ ...
    validation.Passed, ...
    validation.CollisionFree, ...
    validation.CollisionResolved, ...
    candidate.OptimizerFeasible, ...
    -min(candidate.MaximumConstraintViolation, realmax)];
referenceQuality = [ ...
    referenceValidation.Passed, ...
    referenceValidation.CollisionFree, ...
    referenceValidation.CollisionResolved, ...
    referenceCandidate.OptimizerFeasible, ...
    -min(referenceCandidate.MaximumConstraintViolation, realmax)];
differentIndex = find(candidateQuality ~= referenceQuality, 1, "first");
preferred = ~isempty(differentIndex) && ...
    candidateQuality(differentIndex) > referenceQuality(differentIndex);
end

function seeds = orderSeedsByOccupancy(seeds, obstacles, startTime_s)
% PURPOSE
%   - Solve coarse collision-free initializations before blocked ones.
seedCount = numel(seeds);
seedIsOccupied = false(seedCount, 1);
sampleTau = linspace(0, 1, 65).';
for seedIndex = 1:seedCount
    samplePosition_deg = interp1( ...
        seeds(seedIndex).tau, seeds(seedIndex).position_deg, ...
        sampleTau, "linear");
    sampleTime_s = startTime_s + sampleTau * ...
        seeds(seedIndex).EstimatedDuration_s;
    seedIsOccupied(seedIndex) = any(queryAzElTimeObstacle( ...
        obstacles, samplePosition_deg(:, 1), ...
        samplePosition_deg(:, 2), sampleTime_s));
end
[~, seedOrder] = sortrows([seedIsOccupied, (1:seedCount).']);
seeds = seeds(seedOrder);
for seedIndex = 1:seedCount
    seeds(seedIndex).Index = seedIndex;
end
end

function changes = obstacleHistoryChanges(obstacles)
% PURPOSE
%   - Detect input geometry motion that needs a corridor-repair reserve.
changes = false;
...
function partial = boundedPartialTrajectory(candidate)
% PURPOSE
%   - Retain one bounded failed trajectory for direct diagnostic plotting.
sampleCount = numel(candidate.time_s);
retainedIndex = (1:sampleCount).';
maximumRetainedSampleCount = 2000;
if sampleCount > maximumRetainedSampleCount
    retainedIndex = unique(round(linspace( ...
        1, sampleCount, maximumRetainedSampleCount))).';
end
partial = struct( ...
    "time_s", candidate.time_s(retainedIndex), ...
    "position_deg", candidate.position_deg(retainedIndex, :), ...
