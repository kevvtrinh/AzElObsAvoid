...
%       search slices. CollisionValidationMode selects hybrid prefiltering
...
%       RefineVisibilityAfterSuccess optionally adds progressively denser
%       search passes; failed coarse searches refine automatically until
%       the complete available snapshot history has been tried.
...
result = applyVisibilityRefinement(result, obstacles, initialState, ...
    goalState, limits, options, planningTimer);
end

%% Section 6: Local Functions

function result = applyVisibilityRefinement(result, obstacles, ...
...
%**************************************************************************
% PURPOSE
%   - Progressively densify moving-obstacle visibility snapshots after a
%     failed coarse plan or when successful-plan refinement is requested.
%   - Keep the fastest continuously validated result found across passes.
%**************************************************************************
% INPUTS
%   - result (scalar struct)
...
%**************************************************************************
snapshotDiagnostics = result.SearchDiagnostics.SnapshotDiagnostics;
if isempty(snapshotDiagnostics)
    maximumInputSnapshotCount = 0;
    maximumRetainedSnapshotCount = 0;
else
    maximumInputSnapshotCount = max( ...
        [snapshotDiagnostics.InputSnapshotCount]);
    maximumRetainedSnapshotCount = max( ...
        [snapshotDiagnostics.RetainedSnapshotCount]);
end

currentRecord = visibilityRefinementRecord(result, ...
    options.MaximumVisibilitySnapshotsPerObstacle, ...
    maximumInputSnapshotCount, maximumRetainedSnapshotCount);
history = currentRecord;
connectivityChangeIntervalCount = numel( ...
    result.SearchDiagnostics.ConnectivityChangeIntervals);
connectivitySupportsSuccessRefinement = ...
    options.VisibilitySuccessRefinementMode == "uniform" || ...
    connectivityChangeIntervalCount > 0;

hasOmittedSnapshots = maximumRetainedSnapshotCount < ...
    maximumInputSnapshotCount;
automaticFailureRefinement = ~result.Success && ...
    result.TerminationReason == "noFeasibleCandidate" && ...
    hasOmittedSnapshots;
optionalSuccessRefinement = result.Success && ...
    options.RefineVisibilityAfterSuccess && ...
    options.MaximumVisibilityRefinementPasses > 0 && ...
    connectivitySupportsSuccessRefinement && ...
    hasOmittedSnapshots;

if ~(automaticFailureRefinement || optionalSuccessRefinement)
    result.SearchDiagnostics.VisibilityRefinementHistory = history;
    result.SearchDiagnostics.VisibilityRefinementPerformed = false;
    result.SearchDiagnostics.VisibilityRefinementExhausted = ...
        ~hasOmittedSnapshots;
    result.SearchDiagnostics.SelectedVisibilitySnapshotLimit = ...
        currentRecord.SnapshotLimit;
    result.SearchDiagnostics.VisibilityRefinementStoppedBecause = ...
        visibilityRefinementStopReason(result, options, ...
        hasOmittedSnapshots, connectivityChangeIntervalCount);
    return;
end

refinementTrigger = "automaticAfterFailure";
if optionalSuccessRefinement
    refinementTrigger = "optionalAfterSuccessUniform";
    if options.VisibilitySuccessRefinementMode == "connectivity"
        refinementTrigger = "optionalAfterConnectivityChange";
    end
end

nextSnapshotLimit = min(maximumInputSnapshotCount, max( ...
    maximumRetainedSnapshotCount + 1, ...
    ceil(maximumRetainedSnapshotCount * ...
    options.VisibilityRefinementFactor)));
refinedOptions = options;
refinedOptions.MaximumVisibilitySnapshotsPerObstacle = nextSnapshotLimit;
if optionalSuccessRefinement
    refinedOptions.MaximumVisibilityRefinementPasses = ...
        options.MaximumVisibilityRefinementPasses - 1;
end

if options.Verbose
    parallelMessage = "serial";
    if options.UseParallel ~= "off"
        parallelMessage = "parallel when available";
    end
    fprintf("[AzEl][refinement] %s: increasing visibility snapshots " + ...
        "from %d to %d of %d; %d graph-change intervals (%s).\n", ...
        refinementTrigger, ...
        maximumRetainedSnapshotCount, nextSnapshotLimit, ...
        maximumInputSnapshotCount, connectivityChangeIntervalCount, ...
        parallelMessage);
end
...
refinedHistory = ...
    refinedResult.SearchDiagnostics.VisibilityRefinementHistory;
refinedHistory(1).Trigger = refinementTrigger;
...
selectedRecordIndex = find([history.Selected], 1, "first");
if isempty(selectedRecordIndex)
    selectedRecordIndex = numel(history);
    history(selectedRecordIndex).Selected = true;
end
evaluatedAllSnapshots = history(end).MaximumRetainedSnapshotCount >= ...
    history(end).MaximumInputSnapshotCount;
totalElapsedPlanningTime_s = toc(planningTimer);

result.Options = options;
result.ElapsedPlanningTime_s = totalElapsedPlanningTime_s;
result.SearchDiagnostics.ElapsedPlanningTime_s = ...
    totalElapsedPlanningTime_s;
result.SearchDiagnostics.VisibilityRefinementHistory = history;
result.SearchDiagnostics.VisibilityRefinementPerformed = true;
result.SearchDiagnostics.VisibilityRefinementExhausted = ...
    evaluatedAllSnapshots;
result.SearchDiagnostics.SelectedVisibilitySnapshotLimit = ...
    history(selectedRecordIndex).SnapshotLimit;
result.SearchDiagnostics.AutomaticFailureRefinementPerformed = any( ...
    [history.Trigger] == "automaticAfterFailure");
result.SearchDiagnostics.OptionalSuccessRefinementPerformed = any( ...
    startsWith([history.Trigger], "optionalAfter"));
result.SearchDiagnostics.VisibilityRefinementStoppedBecause = ...
    "completedRequestedRefinement";

if options.Verbose
    fprintf("[AzEl][refinement] completed %d passes; selected pass %d " + ...
        "with snapshot limit %g and success=%d.\n", numel(history), ...
        selectedRecordIndex, history(selectedRecordIndex).SnapshotLimit, ...
        result.Success);
end
end

function record = visibilityRefinementRecord(result, snapshotLimit, ...
        maximumInputSnapshotCount, maximumRetainedSnapshotCount)
%% Section 0: Header & Readme
% SYNTAX
%   record = visibilityRefinementRecord(result, snapshotLimit, ...
%       maximumInputSnapshotCount, maximumRetainedSnapshotCount)
%**************************************************************************
% PURPOSE
%   - Summarize one complete planner pass without retaining duplicate
%     visibility-graph payloads from every refinement density.
%**************************************************************************
% INPUTS
%   - result (scalar planner result)
%   - snapshotLimit, maximumInputSnapshotCount, ...
%       maximumRetainedSnapshotCount (nonnegative scalar counts)
%**************************************************************************
% OUTPUTS
...
record = struct( ...
    ...
    "MaximumInputSnapshotCount", maximumInputSnapshotCount, ...
    "MaximumRetainedSnapshotCount", maximumRetainedSnapshotCount, ...
...
function reason = visibilityRefinementStopReason(result, options, ...
        hasOmittedSnapshots, connectivityChangeIntervalCount)
%% Section 0: Header & Readme
% SYNTAX
%   reason = visibilityRefinementStopReason(result, options, ...
%       hasOmittedSnapshots, connectivityChangeIntervalCount)
%**************************************************************************
% PURPOSE
%   - Explain why another visibility refinement pass was not launched.
%**************************************************************************
% INPUTS
%   - result, options (scalar structs)
%       Current planner outcome and resolved refinement controls.
%   - hasOmittedSnapshots (logical scalar)
%   - connectivityChangeIntervalCount (nonnegative integer scalar)
%**************************************************************************
% OUTPUTS
%   - reason (scalar string)
%       Machine-readable refinement stop reason.
%**************************************************************************
% UNITS
%   - The returned reason is dimensionless text.
%**************************************************************************
if ~hasOmittedSnapshots
    reason = "allSnapshotsEvaluated";
elseif ~result.Success && ...
        result.TerminationReason ~= "noFeasibleCandidate"
    reason = "failureNotRefinable";
elseif result.Success && ~options.RefineVisibilityAfterSuccess
    reason = "successRefinementDisabled";
elseif result.Success && options.MaximumVisibilityRefinementPasses == 0
    reason = "successRefinementPassLimit";
elseif result.Success && ...
        options.VisibilitySuccessRefinementMode == "connectivity" && ...
        connectivityChangeIntervalCount == 0
    reason = "noConnectivityChange";
else
...
options = struct(...
    ...
    "MaximumVisibilityRefinementPasses", 2, ...
    "VisibilityRefinementFactor", 2, ...
    "UseParallel", false, ...
    "Verbose", false);
end
...
options.VisibilitySuccessRefinementMode = lower(string( ...
    options.VisibilitySuccessRefinementMode));
if ~isscalar(options.VisibilitySuccessRefinementMode) || ...
        ~any(options.VisibilitySuccessRefinementMode == [ ...
        "connectivity" "uniform"])
    error("planAzElMotion:InvalidVisibilitySuccessRefinementMode", ...
        "VisibilitySuccessRefinementMode must be connectivity or uniform.");
end

options.UseParallel = normalizeParallelOption(options.UseParallel);

logicalNames = ["AllowAzimuthWrapping" ...
    "RefineVisibilityAfterSuccess" "Verbose"];
...
validateattributes(options.MaximumVisibilityRefinementPasses, ...
    {'numeric'}, {'real', 'finite', 'scalar', 'integer', 'nonnegative'});
validateattributes(options.VisibilityRefinementFactor, ...
    {'numeric'}, {'real', 'finite', 'scalar', '>', 1});
