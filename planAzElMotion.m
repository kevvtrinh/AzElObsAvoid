...
%     adaptive visibility search and a certified spatial jerk retimer.
%**************************************************************************
% INPUTS
...
candidateClearance_deg = max(0.05, ...
    3 * min(options.TurnRadius_deg, 0.45));
viewOptions = struct( ...
...
for candidateIndex = 1:candidateCount
    ...
    pathLength_deg(candidateIndex) = ...
        sum(vecnorm(diff(route_deg, 1, 1), 2, 2));
    try
        ...
        timedPath = retimeSpatialPath(smoothPath, initialState, ...
        ...
        timedPath = emptyTimedPath( ...
            limits, options, string(candidateError.message));
        blocked = false(0, 1);
    end
    ...
    if timedPath.Success
        attemptArrivalTime_s(candidateIndex) = ...
...
if planningSucceeded
    ranking = [arrivalTime_s(feasibleIndex), ...
        pathLength_deg(feasibleIndex), feasibleIndex];
...
validation = validatePlan(planningSucceeded, endpointBlocked, ...
    timedSlopePath, bestAttemptProtectedBlocked, goalState, ...
    limits, options);
...
elseif ~success
    message = "Independent post-validation failed: " + ...
        validation.Message;
...
if graphIndex(selectedCandidateIndex) > 0
    selectedPath = ...
        spaceView.VisibilityGraphs(graphIndex(selectedCandidateIndex));
end
...
result = struct( ...
    ...
    "TerminationReason", terminationReason, ...
    "MotionType", "velocityCarrying", "Options", options, ...
    ...
    "spaceView", spaceView, "animation", animation, ...
    "kinematicPlot", kinematicPlot, ...
    "SearchDiagnostics", searchDiagnostics, ...
    "ElapsedPlanningTime_s", elapsedPlanningTime_s, ...
    "RandomSeed", [], "ProgressLog", progressLog, ...
...
if ~isempty(legacyMarginNames)
    error("planAzElMotion:SafetyMarginMoved", ...
        ...
        strjoin(string(legacyMarginNames), ", "));
end
options = defaults;
...
options.PolygonCandidateMode = ...
    lower(string(options.PolygonCandidateMode));
...
if (islogical(options.UseParallel) || isnumeric(options.UseParallel)) && ...
    ...
    if isnumeric(options.UseParallel) && ...
            (~isfinite(options.UseParallel) || ...
...
if ~isscalar(options.UseParallel) || ...
        ~any(options.UseParallel == ["auto" "on" "off"])
    error("planAzElMotion:InvalidUseParallel", ...
        "UseParallel must be auto, on, off, or scalar logical.");
end
positiveNames = ["SampleTime_s" "TurnRadius_deg" ...
    "VisibilitySampleStep_deg" "ExtremeDirectionCount" ...
    "MaximumTangenciesPerReference" ...
    "BoundaryRouteReductionTolerance_deg" ...
...
for index = 1:numel(names)
    ...
    if any(isnan(value)) || ~(isscalar(value) || numel(value) == 2)
        error("planAzElMotion:InvalidLimits", ...
            "%s must be scalar or two-element and cannot contain NaN.", ...
            name);
    end
...
function selectedIndex = selectBestAttempt( ...
        retimed, arrivalTime_s, pathLength_deg, graphIndex)
%% Section 0: Header & Readme
...
if size(route_deg, 1) < 2
    error("planAzElMotion:ZeroLengthRoute", ...
        "A candidate route needs two distinct points.");
end
cornerCount = size(route_deg, 1) - 2;
cornerTemplate = struct( ...
    "PathPointIndex", 0, "Position_deg", zeros(1, 2), ...
    "DeflectionAngle_rad", 0, "AppliedRadius_deg", 0, ...
...
for cornerIndex = 1:cornerCount
    ...
    tangentScale = (384 / 125) * sin(angle_rad / 2) / ...
        cos(angle_rad / 2)^2;
    maximumRadius_deg = 0.45 * min(incomingLength_deg, ...
        outgoingLength_deg) / tangentScale;
    ...
    if requestedRadius_deg >= minimumRadius_deg && ...
    ...
    for radius_deg = trialRadii_deg
        ...
        controlPoints_deg = quinticControls( ...
            corner, trim_deg, incoming, outgoing);
...
primitiveTemplate = struct( ...
    "Type", "", "StartPosition_deg", zeros(1, 2), ...
    "EndPosition_deg", zeros(1, 2), "Direction", zeros(1, 2), ...
    "Length_deg", 0, "StartArcLength_deg", 0, ...
    "EndArcLength_deg", 0, "ControlPoints_deg", zeros(6, 2), ...
...
for cornerIndex = 1:cornerCount
    ...
    if corner.Smoothed
        ...
        primitive.EndArcLength_deg = currentArcLength_deg + ...
            primitive.Length_deg;
...
if isempty(primitives)
    error("planAzElMotion:EmptySmoothPath", ...
        "Smoothing produced no nonzero primitive.");
end
primitiveBoundaryS_deg = [primitives.EndArcLength_deg].';
sampleS_deg = unique([0; (0:0.05:currentArcLength_deg).'; ...
    primitiveBoundaryS_deg; mandatoryStopArcLength_deg; ...
    currentArcLength_deg]);
definition = struct( ...
...
function controlPoints_deg = quinticControls( ...
        corner_deg, trim_deg, incoming, outgoing)
%% Section 0: Header & Readme
% Construct the symmetric quintic with zero q'' and q''' at both joins.
controlPoints_deg = [ ...
    corner_deg - trim_deg * incoming; ...
    corner_deg - 0.5 * trim_deg * incoming; ...
    corner_deg; corner_deg; ...
    corner_deg + 0.5 * trim_deg * outgoing; ...
    corner_deg + trim_deg * outgoing];
end
...
parameterGrid = linspace(0, 1, ...
    max(100, ceil(controlLength_deg / 0.004)) + 1).';
...
if primitive.Type == "line"
    ...
    position_deg = evaluateQuintic( ...
        primitive.ControlPoints_deg, min(max(parameter, 0), 1));
end
end
...
for index = 1:numel(primitives)
    ...
    if index < numel(primitives)
        belongs = belongs & ...
            queryS_deg < primitive.EndArcLength_deg - tolerance_deg;
    else
        belongs = belongs & ...
            queryS_deg <= primitive.EndArcLength_deg + tolerance_deg;
    end
    ...
    if primitive.Type == "line"
        ...
        [position, first, parameterSecond, parameterThird] = ...
            evaluateQuintic(primitive.ControlPoints_deg, ...
            min(max(parameter, 0), 1));
        speed = vecnorm(first, 2, 2);
        firstSecond = sum(first .* parameterSecond, 2);
        speedSquared = speed.^2;
        secondValue = parameterSecond ./ speedSquared - ...
            first .* firstSecond ./ speed.^4;
...
samples = struct( ...
    "arcLength_deg", queryS_deg, "position_deg", position_deg, ...
    "tangent", tangent, "secondDerivative_deg_inv", second, ...
    "thirdDerivative_deg_inv2", third, ...
    "curvature_deg_inv", vecnorm(second, 2, 2), ...
...
%% Section 0: Header & Readme
% Retime one fixed G3 path with certified per-primitive spatial limits.
tolerance = 1e-9;
totalLength_deg = smoothPath.TotalLength_deg;
...
goalSpeed_deg_s = boundarySpeed( ...
    goalState.velocity_deg_s, endpoint.tangent(2, :), tolerance);
...
if norm(initialState.acceleration_deg_s2 - ...
        ...
        norm(goalState.acceleration_deg_s2 - ...
        requiredGoalAcceleration_deg_s2) > tolerance
    timedPath = emptyTimedPath(limits, options, ...
        "Endpoint acceleration does not match the path curvature.");
    return;
end
primitiveCount = numel(smoothPath.Primitives);
...
bounds = repmat(boundTemplate, primitiveCount, 1);
maximumSpeed_deg_s = zeros(primitiveCount, 1);
maximumAcceleration_deg_s2 = zeros(primitiveCount, 1);
maximumJerk_deg_s3 = zeros(primitiveCount, 1);
...
for index = 1:primitiveCount
    bounds(index) = derivativeBounds( ...
        smoothPath.Primitives(index), boundaryS_deg(index), ...
        boundaryS_deg(index + 1), index);
    [maximumSpeed_deg_s(index), ...
        maximumAcceleration_deg_s2(index), ...
        maximumJerk_deg_s3(index)] = scalarLimits( ...
        bounds(index), effectiveLimits, tolerance);
end
mandatoryStopNode = false(primitiveCount + 1, 1);
...
[nodeSpeed_deg_s, feasible, failureMessage] = reachableNodeSpeeds( ...
...
profileTemplate = minimumTimeProfile( ...
    1, 0, 0, 1, 1, 1, tolerance);
...
for index = 1:primitiveCount
    ...
    primitive = smoothPath.Primitives(index);
    ...
    [profile.PeakVelocityByAxis_deg_s, ...
        ...
        cartesianBounds(bounds(index), profile);
...
if options.GoalTimeMode == "fixedarrival"
    ...
    if waitDuration_s > timeTolerance_s && ...
            (norm(initialState.velocity_deg_s) > tolerance || ...
...
startTime_s = motionStartTime_s + ...
    [0, cumsum([profiles(1:end - 1).Duration_s])];
for index = 1:primitiveCount
    profiles(index).StartTime_s = startTime_s(index);
    profiles(index).EndTime_s = ...
        startTime_s(index) + profiles(index).Duration_s;
end
...
constraintsSatisfied = velocitySatisfied && ...
    accelerationSatisfied && jerkSatisfied;
joinSpeed_deg_s = nodeSpeed_deg_s(2:end - 1);
ordinaryJoin = ~mandatoryStopNode(2:end - 1);
...
diagnostics = struct( ...
    ...
    "VelocityMargin_deg_s", limits.maxVelocity_deg_s - ...
        peakVelocity_deg_s, ...
    "AccelerationMargin_deg_s2", limits.maxAcceleration_deg_s2 - ...
...
timedPath = struct( ...
    ...
    "Message", "Certified analytic spatial retiming succeeded.", ...
    ...
    "SegmentProfiles", profiles, "Limits", limits, ...
    ...
    "CurveTangentialAcceleration_deg_s2", ...
        zeros(size(nodeSpeed_deg_s)), ...
    "CurveTangentialJerk_deg_s3", zeros(size(nodeSpeed_deg_s)), ...
    "SampleArcLength_deg", sampleS_deg, ...
...
if ~constraintsSatisfied
    timedPath.Message = ...
        "Internal continuous constraint certification failed.";
end
end
...
if speed_deg_s < -tolerance || ...
        norm(velocity_deg_s - speed_deg_s * tangent) > tolerance
...
bounds = struct( ...
    "RunIndex", 0, "StartArcLength_deg", 0, ...
    ...
    "EnvelopeInflationFactor", 1, "SampleCount", 0, ...
    "CertificateSubdivisionCount", 0, ...
    "CertificateFallbackCount", 0, ...
    "CertificatePrimitiveCount", 1, ...
    "SampledBoundsWithinCertificate", true, ...
    "Method", "continuousAnalyticEnvelope");
end

function bounds = derivativeBounds(primitive, startS_deg, endS_deg, index)
%% Section 0: Header & Readme
% Certify the first three arc derivatives over one complete primitive.
if primitive.Type == "line"
...
sample = samplePath(struct("Primitives", localPrimitive, ...
    "TotalLength_deg", primitive.Length_deg), ...
    linspace(0, primitive.Length_deg, 17).');
...
if ~all(isfinite([maximumSpeed_deg_s, maximumAcceleration_deg_s2, ...
        maximumJerk_deg_s3])) || ...
        any([maximumSpeed_deg_s, maximumAcceleration_deg_s2, ...
...
for index = 2:count
    cap_deg_s(index) = min( ...
        maximumSpeed_deg_s(index - 1), maximumSpeed_deg_s(index));
end
...
for passIndex = 1:max(8, 2 * (count + 1))
    ...
    for index = 1:count
        reachable = reachableSpeed(nodeSpeed_deg_s(index), ...
            length_deg(index), maximumSpeed_deg_s(index), ...
            maximumAcceleration_deg_s2(index), ...
            maximumJerk_deg_s3(index), tolerance);
        if index == count && goalSpeed_deg_s > ...
                reachable + speedTolerance_deg_s
            feasible = false;
            message = "The goal speed is not forward reachable.";
            return;
        elseif index < count
            nodeSpeed_deg_s(index + 1) = min( ...
                nodeSpeed_deg_s(index + 1), reachable);
        end
    end
    for index = count:-1:1
...
function speed_deg_s = reachableSpeed( ...
...
if min([length_deg, speedLimit_deg_s, accelerationLimit_deg_s2, ...
        jerkLimit_deg_s3]) <= 0 || ...
...
minimumDistance_deg = transitionDistance( ...
...
transitionDistance_deg = transitionDistance(startSpeed_deg_s, ...
    ...
    transitionDistance(peakSpeed_deg_s, endSpeed_deg_s, ...
    accelerationLimit_deg_s2, jerkLimit_deg_s3);
cruiseDuration_s = max(0, length_deg - transitionDistance_deg) / ...
    max(peakSpeed_deg_s, eps);
...
profile.PeakAcceleration_deg_s2 = max(abs([ ...
    phaseStartAcceleration_deg_s2; ...
    phaseStartAcceleration_deg_s2 + ...
    phaseJerk_deg_s3 .* phaseDuration_s]));
...
profile = struct( ...
    "PrimitiveType", "", "StartPosition_deg", zeros(1, 2), ...
    ...
    "PhaseDuration_s", zeros(7, 1), ...
    "PhaseJerk_deg_s3", zeros(7, 1), ...
    "PhaseStartTime_s", zeros(7, 1), ...
    "PhaseStartPosition_deg", zeros(7, 1), ...
    "PhaseStartSpeed_deg_s", zeros(7, 1), ...
    "PhaseStartAcceleration_deg_s2", zeros(7, 1), ...
...
function distance_deg = transitionDistance( ...
...
distance_deg = 0.5 * (firstSpeed_deg_s + secondSpeed_deg_s) * ...
    sum(duration_s);
end

function [duration_s, jerk_deg_s3] = transitionPhases( ...
        firstSpeed_deg_s, secondSpeed_deg_s, ...
        accelerationLimit_deg_s2, jerkLimit_deg_s3)
%% Section 0: Header & Readme
...
constantAccelerationDuration_s = max(0, ...
    changeMagnitude_deg_s / (jerkLimit_deg_s3 * jerkRampDuration_s) - ...
    jerkRampDuration_s);
duration_s = [jerkRampDuration_s; ...
    constantAccelerationDuration_s; jerkRampDuration_s];
...
function [time_s, arcLength_deg, speed_deg_s, acceleration_deg_s2, ...
        jerk_deg_s3] = sampleProfiles( ...
        profiles, initialTime_s, waitDuration_s, sampleTime_s)
%% Section 0: Header & Readme
...
for index = 1:numel(profiles)
    ...
    events_s = unique([phaseStart_s; ...
        phaseStart_s + 0.5 * profile.PhaseDuration_s; phaseEnd_s]);
    localTime_s = regularTimes( ...
        profile.Duration_s, sampleTime_s, events_s);
...
time_s = unique([0; (0:sampleTime_s:duration_s).'; ...
    events_s(:); duration_s]);
...
smoothPath = struct( ...
    "Success", false, "Message", "No smooth path was produced.", ...
    "OriginalPathPosition_deg", route_deg, ...
    "Primitives", struct([]), "TotalLength_deg", 0, ...
    "SampleArcLength_deg", zeros(0, 1), ...
    "position_deg", zeros(0, 2), "tangent", zeros(0, 2), ...
...
diagnostics = struct( ...
    ...
    "VelocitySatisfied", false, "AccelerationSatisfied", false, ...
    "JerkSatisfied", false, ...
    "JerkConstrained", any(isfinite(limits.maxJerk_deg_s3)), ...
...
timedPath = struct( ...
    ...
    "MotionStartTime_s", NaN, "WaitDuration_s", NaN, ...
    "MinimumMotionDuration_s", NaN, ...
    "GoalLineInterceptTime_s", NaN, ...
    "SegmentProfiles", repmat(profileTemplate(), 0, 1), ...
    "Limits", limits, ...
    "Options", struct("GoalTimeMode", options.GoalTimeMode, ...
        "SampleTime_s", options.SampleTime_s), ...
...
finiteState = hasMotion && all(isfinite(timedPath.time_s)) && ...
    ...
    all(isfinite(timedPath.acceleration_deg_s2(:))) && ...
    all(isfinite(timedPath.jerk_deg_s3(:)));
...
endpointMatched = hasMotion && ...
    ...
    norm(timedPath.velocity_deg_s(end, :) - ...
        goalState.velocity_deg_s) <= 1e-7 && ...
    norm(timedPath.acceleration_deg_s2(end, :) - ...
        goalState.acceleration_deg_s2) <= 1e-7;
goalTimeSatisfied = hasMotion && ...
    timedPath.time_s(end) <= goalState.time_s + 1e-7;
if options.GoalTimeMode == "fixedarrival"
    goalTimeSatisfied = hasMotion && ...
        abs(timedPath.time_s(end) - goalState.time_s) <= 1e-7;
end
sampleVelocitySatisfied = hasMotion && all(max( ...
...
sampleAccelerationSatisfied = hasMotion && all(max( ...
    ...
    limits.maxAcceleration_deg_s2 + 1e-7);
sampleJerkSatisfied = hasMotion && all(max( ...
    abs(timedPath.jerk_deg_s3), [], 1) <= ...
    limits.maxJerk_deg_s3 + 1e-7);
certificateSatisfied = hasMotion && ...
...
checks = [planningSucceeded, ~endpointBlocked, finiteState, strictTime, ...
    ...
    certificateSatisfied, collisionFree];
...
validation = struct( ...
    ...
    "ProtectedGeometryClear", collisionFree, ...
    "AzimuthPolicySatisfied", hasMotion && ...
        routeWithinAzimuthPolicy(timedPath.position_deg, options), ...
    "GoalReached", endpointMatched, ...
    "ArrivalWithinGoalTime", goalTimeSatisfied, ...
    "FiniteState", finiteState, ...
    "TimeStrictlyIncreasing", strictTime, ...
    "EndpointMatched", endpointMatched, ...
...
for index = 1:4
    ...
    lineHandles(:, index) = plot(axesHandles(index), ...
        timedPath.time_s, data{index}, ...
        "LineWidth", 1.2);
...
plotRecord = struct( ...
    "Figure", figureHandle, "Layout", layout, "Axes", axesHandles, ...
    "PositionLines", lineHandles(:, 1), ...
    "VelocityLines", lineHandles(:, 2), ...
    "AccelerationLines", lineHandles(:, 3), ...
    "JerkLines", lineHandles(:, 4), ...
    "LimitLines", gobjects(0, 1), ...
    "Diagnostics", timedPath.ConstraintDiagnostics, ...
    "Options", struct("FigureVisible", options.FigureVisible, ...
        "ShowLimits", false, "Title", options.Title), ...
    "Limits", limits);
end
...
graph = struct( ...
    "Success", false, "Message", "No visibility graph selected.", ...
...
log = struct( ...
    "Sequence", 1, "Stage", "complete", "Event", event, ...
