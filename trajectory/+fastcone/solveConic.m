function [x, fval, exitflag, output] = solveConic(f, cones, A, b, E, d, lb, ub, options)
%% Section 0: Header & Readme
% SYNTAX: [x,fval,exitflag,output] = fastcone.solveConic(f,cones,A,b,E,d,lb,ub,options)
% PURPOSE: MATLAB primal-dual solver for HS's scalar and Lorentz cones.
% INPUTS: The nine positional arguments used by MATLAB coneprog.
% OUTPUTS: Candidate, objective, status (1 converged, -2 certified infeasible,
%   0 unresolved), diagnostics.
% UNITS: Identical to the input program. No motion model is changed.
% Positive results and infeasibility witnesses are checked on original inputs.
% Unresolved programs must never be interpreted as infeasible.
...
if any(cellfun('size',coneA,1)~=2)
    x=[]; fval=NaN; exitflag=0;
    output=struct('iterations',0,'message','Unsupported cone dimension.', ...
        'unsupported',true,'totalTime_s',toc(timer));
    return
end
if coneCount==0
...
constantTails=[h(first(constantBoth))';h(second(constantBoth))'];
...
setupTime=toc(timer);
[xReduced,z,stats]=fastcone.coneKernel(G,full(h),full(cost), ...
...
if exitflag==1 && (~isfinite(originalResidual) || originalResidual>tol || ...
    ...
    exitflag=0; message='Independent MATLAB validation rejected the kernel result.';
elseif exitflag==1
    message='Original primal residual and independent MATLAB dual objective bound passed.';
else
    message='MATLAB iteration limit or numerical failure; unresolved.';
end
if stats(14)>0
    message='A positive dual bound excludes the inherited variable box; reference recovery requested.';
end
originalInfeasibility=struct('Certified',false,'Margin',-Inf);
if stats(14)>0
    % Reverse row scaling and exact cone simplifications only to propose a
    % witness. Its acceptance is independent, on the original input arrays.
    nz=numel(rowScale); lifted=zeros(nz+nnz(keepRows),1);
    linearDual=zeros(numel(constantRows),1); linearDual(~constantRows)=z(1:linearCount);
    lifted(1:nz)=linearDual(1:nz);
    lifted(heads(simple))=linearDual(nz+offsets(simple)+1);
    if any(constantBoth)
        weights=linearDual(nz+offsets(constantBoth)+1);
        tailDual=-constantTails.*weights'./vecnorm(constantTails);
        lifted(first(constantBoth))=tailDual(1,:)';
        lifted(second(constantBoth))=tailDual(2,:)';
    end
    plusDual=linearDual(nz+plus); minusDual=linearDual(nz+minus);
    lifted(heads(one))=plusDual+minusDual;
    lifted(selectedTail)=plusDual-minusDual;
    lifted(keptRows)=z(linearCount+1:end);
    coneDual=zeros(size(originalConeG,1),1);
    coneDual(keepRows)=lifted(nz+1:end)./scales(keepRows);
    rawDual=[lifted(1:nz)./rowScale;coneDual];
    rawIdentity=speye(n);
    rawG=[sparse(originalA);-rawIdentity(free(lower),:);rawIdentity(free(upper),:);originalConeG];
    rawH=[original{4}(:);-lb(free(lower));ub(free(upper));originalConeH];
    dependent=setdiff(free,sourceIndex);
    originalInfeasibility=fastcone.infeasibilityCertificate(rawG,rawH,rawDual,nz, ...
        sparse(originalE),original{6}(:),lb,ub,dependent,activeEqualities,tol);
    originalInfeasibility.LowerBoundIndices=free(lower);
    originalInfeasibility.UpperBoundIndices=free(upper);
    if originalInfeasibility.Certified
        exitflag=-2; x=[]; fval=NaN;
        message='An original-input dual witness excludes the tolerance-expanded feasible set.';
    end
end
output=struct('iterations',stats(1),'primalfeasibility',stats(3), ...
    'dualfeasibility',stats(4),'dualitygap',stats(5),'message',message, ...
    'algorithm','MATLAB analytical Jordan-block predictor-corrector', ...
    ...
    'kernelTime_s',stats(11),'kernelAnalyses',stats(9), ...
    'regularizedFactors',stats(10),'kernelStats',stats,'unsupported',false, ...
    'infeasibilityMargin',stats(14),'originalInfeasibility',originalInfeasibility);
end
