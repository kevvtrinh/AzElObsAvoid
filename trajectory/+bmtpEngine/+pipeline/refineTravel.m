...
% SYNTAX
%   [result, diagnostics] = bmtpEngine.pipeline.refineTravel(solverRequest, ...
...
if isempty(retainedPreparedMotion) || isempty(retainedProof)
    retainedPreparedMotion = bmtpEngine.pipeline.prepareFinalMotion( ...
        solverRequest, retainedControl_units, retainedSegmentTime_s);
    [retainedPreparedMotion, retainedProof] = bmtpEngine.pipeline.refineMotionSeparation( ...
        solverRequest, retainedPreparedMotion, roundoffReserve_units, separationTarget_units);
end
...
for refinementIndex = 1:refinementAttemptLimit
    ...
    if ~any(unresolvedPairs, 'all')
        ...
        [refinedPreparedMotion, refinedProof] = bmtpEngine.pipeline.refineMotionSeparation( ...
