...
%**************************************************************************
% PURPOSE
%   - Optimize one static topology seed with a composite Bezier trajectory,
...
%   - Accept motion only after direct Bernstein checks, an all-span/all-region
%     plane certificate, and independent public validation.
%**************************************************************************
% INPUTS
%   - seed (scalar struct)
...
%       Requires Passed; the caller owns the geometry-coverage proof.
...
%% Section 1: Validate And Create The Static Representation
...
restart = struct("ControlPoint_deg", zeros(0, degree + 1, 2), ...
    "SegmentTime_s", NaN);
...
diagnostics.WarmRouteResampled = ...
    originalSeedSegmentCount > maximumWarmSegmentCount;
...
for iterationIndex = 1:35
    ...
    [trialControl_deg, trialTime_s, exitFlag, output] = solveTrajectorySocp( ...
        ...
        limits, planes, roundoffReserve_deg, motionHorizon_s, ...
        trajectoryOptions);
    ...
    for activeIndex = 1:numel(activePairIndices)
        ...
        if planeExitFlag <= 0 || ~plane.Verified
            ...
            solverMessage = "A maximum-margin plane failed direct verification.";
            updateFailed = true;
            break;
        end
...
[candidate.Message, candidate.TerminationReason] = ...
    deal("A directly certified static BMTP trajectory was found.", "goalReached");
...
requestIsSupported = max(abs(endpointDerivative)) <= options.ConstraintTolerance && ...
    any(string(options.GoalTimeMode) == ["earliestArrival", "fixedArrival"]) && ...
...
maximumSegmentTime_s = totalHorizon_s / segmentCount;
lb(powerIndex) = 0;
lb(powerIndex(2)) = eps;
...
for segmentIndex = 1:segmentCount
    ...
    overlaps = regionMinimum_deg(:, 1) <= sampleMaximum_deg(1) & ...
...
function certificate = certifyAllPlanes(control_deg, regions_deg, ...
        ...
        reserve_deg, target_deg, solverOptions)
% Verify every output span against every supplied convex exclusion region.
...
certificate = struct( "Kind", "staticDegreeOne", ...
    ...
    "Planes", planes, "RequiredGap_deg", target_deg + reserve_deg, ...
...
[candidate.SeedIndex, candidate.SeedCorridorBoundary_deg] = ...
    deal(seedIndex, corridorBoundary_deg);
[candidate.Message, candidate.SeedCorridorBoundary_deg] = ...
    deal("The static BMTP kernel was not run.", corridorBoundary_deg);
end
...
diagnostics = struct( "Identifier", "bmtpStaticDegree" + string(degree), ...
    ...
    "Representation", "C3CompositeBezier", ...
    "Attempted", true, "Accepted", false, "Degree", degree, ...
    "SubspansPerSeedEdge", splitCount, ...
    ...
    "IterationCount", 0, "Converged", false, "WorkLimitReached", false, ...
    "TaggedPairCount", 0, "TrajectorySocpCount", 0, "FinalCollisionPairCount", 0, ...
    "PlaneSocpCount", 0, "FinalTrajectoryExitFlag", NaN, ...
    "FailedPlaneSegmentIndex", 0, "FailedPlaneRegionIndex", 0, ...
    "FailedPlane", emptyPlane(), "WarmStartDuration_s", NaN, ...
    "BestDuration_s", NaN, "EndpointProjectionApplied", false, ...
    "TrialDuration_s", NaN(35, 1), ...
    "CollisionPairCountHistory", NaN(35, 1), ...
    "TrialWasCollisionFree", false(35, 1), ...
    "DilationScale", NaN, "MotionCertificate", struct(), ...
    "Coverage", struct(), "PlaneCertificate", struct(), ...
    "SolverMessage", "", "ElapsedTime_s", 0);
end
