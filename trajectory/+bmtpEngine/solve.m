...
for iterationIndex = 1:35
    ...
    if toc(totalTimer) >= maximumSolverTime_s
        [diagnostics.WorkLimitReached, solverMessage] = ...
    ...
    [trialControl_deg, trialTime_s, exitFlag, output, ...
        ...
        "earliestArrival", 0, ...
        feasibleSegmentTime_s, trajectoryOptions, trajectorySocpTemplate);
...
if options.GoalTimeMode ~= "earliestArrival"
    ...
    diagnostics.TravelRefinementTrialRates_deg_s = ...
        trialSavingsRates_deg_s;
    diagnostics.TravelRefinementPortfolioCount = ...
        numel(trialSavingsRates_deg_s);
    diagnostics.TravelRefinementCandidateLength_deg = ...
        NaN(size(trialSavingsRates_deg_s));
    diagnostics.TravelRefinementCandidateDuration_s = ...
        NaN(size(trialSavingsRates_deg_s));
    diagnostics.TravelRefinementCandidateCost_deg = ...
        NaN(size(trialSavingsRates_deg_s));
    for portfolioIndex = 1:numel(trialSavingsRates_deg_s)
        travelTemplate = struct([]);
        travelPlanes = planes;
        trialRate_deg_s = trialSavingsRates_deg_s(portfolioIndex);
        for refinementIndex = 1:8
            diagnostics.TravelRefinementIterationCount = ...
                diagnostics.TravelRefinementIterationCount + 1;
            [refinedControl_deg, refinedSegmentTime_s, ...
                travelExitFlag, travelOutput, travelTemplate] = ...
                solveTrajectorySocp( ...
                ...
                baseSegmentTime_s, trajectoryOptions, travelTemplate);
            diagnostics.TravelRefinementExitFlag = travelExitFlag;
            diagnostics.TravelRefinementMessage = string(travelOutput.message);
            ...
            refinedCollisionPairs = findSampledCollisionPairs( ...
                ...
                regionMaximum_deg, regionActiveBySegment, 1201);
            diagnostics.TravelRefinementCollisionPairCount = ...
                nnz(refinedCollisionPairs);
            if any(refinedCollisionPairs, "all")
                ...
                for newPairIndex = newPairIndices
                    ...
                    [travelPlane, planeExitFlag] = solveMaximumMarginPlane( ...
                        ...
                        regions_deg{regionIndex}, obstacleTarget_deg, ...
                        roundoffReserve_deg, planeOptions);
                    diagnostics.TravelRefinementPlaneSocpCount = ...
                        diagnostics.TravelRefinementPlaneSocpCount + 1;
            ...
            diagnostics.TravelRefinementCandidateLength_deg( ...
                portfolioIndex) = refinedLength_deg;
            diagnostics.TravelRefinementCandidateDuration_s( ...
                portfolioIndex) = segmentCount * refinedSegmentTime_s;
            diagnostics.TravelRefinementCandidateCost_deg( ...
                portfolioIndex) = refinedCost_deg;
            if options.GoalTimeMode == "fixedArrival"
                refinementIsBetter = refinedLength_deg < selectedLength_deg;
            else
                refinementIsBetter = refinedCost_deg < selectedCost_deg;
            end
            diagnostics.TravelRefinementSuccessfulCount = ...
                diagnostics.TravelRefinementSuccessfulCount + 1;
            if refinementIsBetter
                ...
                diagnostics.TravelRefinementSelectedRate_deg_s = ...
                    trialRate_deg_s;
            end
            break;
        end
    end
    if diagnostics.TravelRefinementAccepted
        bestControl_deg = selectedControl_deg;
        bestSegmentTime_s = selectedSegmentTime_s;
        bestDuration_s = segmentCount * bestSegmentTime_s;
        planes = selectedPlanes;
        taggedPairs = taggedPairs | reshape( ...
            [planes.Active], size(planes));
        diagnostics.TravelRefinementDuration_s = bestDuration_s;
    end
...
function [controlPoint_deg, segmentTime_s, exitFlag, output, template] = ...
        ...
        referenceSegmentTime_s, options, template)
% Reuse immutable SOCP rows while replacing planes, timing, and objective.
if isempty(template)
    controlCount = segmentCount * (degree + 1) * 2;
    powerIndex = controlCount + (1:4);
    usesTravelObjective = goalTimeMode ~= "earliestArrival";
    travelBoundCount = usesTravelObjective * segmentCount * degree;
    ...
    template = struct( ...
        ...
        "travelBoundIndex", travelBoundIndex, ...
...
[x, ~, exitFlag, output] = coneprog( ...
    f, template.cones, A, b, template.equality, ...
    template.equalityRightSide, lb, ub, options);
...
diagnostics = struct( "Identifier", "bmtpStaticDegree" + string(degree), ...
    ...
    "TravelRefinementPortfolioCount", 0, ...
    "TravelRefinementSuccessfulCount", 0, ...
    "TravelRefinementIterationCount", 0, ...
    "TravelRefinementPlaneSocpCount", 0, ...
    "TravelRefinementTrialRates_deg_s", zeros(1, 0), ...
    "TravelRefinementCandidateLength_deg", zeros(1, 0), ...
    "TravelRefinementCandidateDuration_s", zeros(1, 0), ...
    "TravelRefinementCandidateCost_deg", zeros(1, 0), ...
    "TravelRefinementSelectedRate_deg_s", NaN, ...
    "TravelRefinementExitFlag", NaN, ...
    "TravelRefinementDuration_s", NaN, ...
    "TravelRefinementCollisionPairCount", 0, ...
