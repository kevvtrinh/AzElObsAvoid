...
for iterationIndex = 1:35
    ...
    if ~any(collisionPairs, "all")
        ...
        if options.EnableStagnationStop
            if retainedBestImprovement_s > options.StagnationImprovementTolerance_s
                stagnationIterationCount = 0;
            else
                stagnationIterationCount = stagnationIterationCount + 1;
            end
            diagnostics.StagnationIterationCount = stagnationIterationCount;
            diagnostics.StagnationIterationCountHistory(iterationIndex) = ...
                stagnationIterationCount;
            if stagnationIterationCount >= options.StagnationIterationLimit
                diagnostics.StoppedOnStagnation = true;
                solverMessage = "The retained best duration stagnated for " + ...
                    string(stagnationIterationCount) + " collision-free trials.";
                break;
            end
        end
...
candidate.Message = "The static BMTP kernel was not run.";
end
...
diagnostics = struct( "Identifier", "bmtpStaticDegree" + string(degree), ...
    ...
    "IterationCount", 0, "Converged", false, "StoppedOnStagnation", false, ...
    "PlaneReuseApplied", false, "PlaneReuseCount", 0, ...
    "StagnationIterationCount", 0, "WorkLimitReached", false, ...
    ...
    "StagnationIterationCountHistory", NaN(35, 1), ...
