function [certificate, cache] = checkFinalMotion(request, preparedMotion, ...
        roundoffReserve_units, obstacleTarget_units, cache, stopOnFirstUnverified)
%% Section 0: Header & Readme
% SYNTAX
%   certificate = bmtpEngine.validation.checkFinalMotion(request, preparedMotion, ...
...
%**************************************************************************
% PURPOSE
%   - Certify every applicable curve-span and convex-obstacle pair.
%**************************************************************************
% INPUTS
...
%       Pair certificates for the prepared motion; Passed is false when any
    ...
    for axisIndex = 1:2
        for segmentIndex = 1:polynomial.SegmentCount
            ...
            certificate.WorkspacePassed = certificate.WorkspacePassed && ...
                bmtpEngine.validation.certifyPolynomialRange( ...
...
cache = struct( ...
    'Controls',     preparedMotion.CertifiedControlPoint_units, ...
    'Breaks',       spanBreaks_s, ...
...
function certificate = checkAllCurveObstaclePairs(controlPoint_units, regions_units, ...
        coverage, separatingLineGeometry, regionActiveBySegment, reserve_units, ...
    ...
    cacheMatchesRequest = ~isempty(cache) && ...
        isequaln(cache.Certificate.Regions_units, regions_units) && ...
    ...
    for segmentIndex = 1:segmentCount
        ...
        cachedSpanMatches = cachedSpanIndex > 0 && isequal( ...
            ...
            cache.Certificate.RegionActiveBySegment(cachedSpanIndex, :));
        if cachedSpanMatches
            activeRegions = regionActiveBySegment(segmentIndex, :);
            oldPlanes     = cache.Certificate.Planes(cachedSpanIndex, :);
        ...
        if staticGeometry && regionCount > 0 && segmentIndex > 1
            ...
            previousPlanes = bmtpEngine.separation.verifyStaticSeparatingLines( ...
                previousPlanes, trajectory_units, regions_units, reserve_units, target_units);
        ...
        for regionIndex = 1:regionCount
            ...
            if plane.Active && ~staticGeometry
                ...
                plane = bmtpEngine.separation.verifySeparatingLine( ...
                    plane, restricted_units, vertices_units, reserve_units, target_units);
            end
            if plane.Verified
                ...
                plane = bmtpEngine.separation.solveSeparatingLine( ...
                    restricted_units, vertices_units, target_units, reserve_units, geometry);
            end
