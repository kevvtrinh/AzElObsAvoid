...
corridor = struct('Available',available,'Intervals_units',zeros(0,4), ...
    ...
    'TerminationReason',"nonMonotoneClockGuide");
if ~available, return; end
coordinateScale_units = bmtpEngine.createCoordinateTolerances(request.InitialState.position_units, request.Regions_units);
boundaryRoundoff_units = 32 * eps(coordinateScale_units);
...
for slab = 1:numel(events_units)-1
    ...
    % A blocked chord is not a visibility guide. Boundary roundoff remains
    % eligible, but every retained facet still imposes its full clearance.
    if any(present & guideCoordinate_units > lower_units + boundaryRoundoff_units & guideCoordinate_units < upper_units - boundaryRoundoff_units)
        corridor.Available = false;
        corridor.TerminationReason = "guideIntersectsSourceInterior";
        return;
    end
...
corridor.TerminationReason = "sourceFacetsCompiled";
end
