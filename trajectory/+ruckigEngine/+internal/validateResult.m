...
if ~isstruct(result) || ~isscalar(result) || ...
    ...
    error("ruckigEngine:InvalidResult", ...
        "result must follow the Ruckig trajectory result format.");
end
...
historyIsFinite = ~isempty(result.time) && ...
    all(isfinite(result.time)) && ...
    all(isfinite(result.position), "all") && ...
    all(isfinite(result.velocity), "all") && ...
    all(isfinite(result.acceleration), "all") && ...
    all(isfinite(result.jerk), "all");
timeIsStrictlyIncreasing = numel(result.time) >= 2 && ...
    all(diff(result.time) > 0);
if historyIsFinite
    endpointErrors = [ ...
        max(abs(result.position(1, :) - initialState.position)), ...
        max(abs(result.velocity(1, :) - initialState.velocity)), ...
        max(abs(result.acceleration(1, :) - initialState.acceleration)), ...
        max(abs(result.position(end, :) - terminalState.position)), ...
        max(abs(result.velocity(end, :) - terminalState.velocity)), ...
        max(abs(result.acceleration(end, :) - ...
        terminalState.acceleration))];
else
...
[inequality, equality] = ...
    ...
    result.Polynomial, terminalState, limits, pathConstraints);
...
elseif ~endpointPassed
    ...
    message = sprintf( ...
        ...
        max(maximumInequalityViolation, maximumEqualityViolation), ...
        tolerance);
end
validation = struct( ...
    "Passed", passed, ...
