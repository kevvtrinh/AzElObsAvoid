...
function CoordinateFrame({ bounds, viewport }) {
...
function Plane({ scene, result, time, mode, onPoint, onVertexMove }) {
  const mount = useRef();
  ...
  useEffect(() => {
    ...
    const pointerDown = (event) => { if (mode === 'edit') drag = findVertex(event); }; const pointerMove = (event) => { if (drag) onVertexMove(drag, toPosition(event)); }; const pointerUp = (event) => { if (drag) { drag = null; return; } if (mode !== 'edit') onPoint(toPosition(event)); };
    ...
    return () => { renderer.domElement.removeEventListener('pointerdown', pointerDown); renderer.domElement.removeEventListener('pointermove', pointerMove); renderer.domElement.removeEventListener('pointerup', pointerUp); renderer.dispose(); mount.current?.replaceChildren(); };
  }, [scene, result, time, mode, onPoint, onVertexMove]);
  const bounds = sceneBounds(scene);
  return <div className="plane"><div className="canvas-mount" ref={mount} /><CoordinateFrame bounds={bounds} viewport={viewport} /></div>;
}

function App() {
  const [scene, setScene] = useState(initialScene); const [mode, setMode] = useState('polygon'); const [draft, setDraft] = useState([]); const [job, setJob] = useState(null); const [response, setResponse] = useState(); const [time, setTime] = useState(0); const [latency, setLatency] = useState();
  const point = (position_deg) => { if (mode === 'start') setScene((current) => ({ ...current, initialState: { ...current.initialState, position_deg } })); else if (mode === 'goal') setScene((current) => ({ ...current, goalState: { ...current.goalState, position_deg } })); else if (mode === 'polygon') setDraft((current) => [...current, position_deg]); };
  const moveVertex = ({ obstacleIndex, vertexIndex }, position_deg) => setScene((current) => ({ ...current, obstacles: current.obstacles.map((obstacle, index) => index !== obstacleIndex ? obstacle : { ...obstacle, slices: obstacle.slices.map((slice) => slice.vertices_deg.length > vertexIndex ? { ...slice, vertices_deg: slice.vertices_deg.map((vertex, index) => index === vertexIndex ? position_deg : vertex) } : slice) }) }));
