...
validateattributes(port, {'numeric'}, {'real', 'finite', 'scalar', 'integer', '>=', 1024, '<=', 65535}, "serveSandbox", "port");
port = double(port);
...
stopFilePath        = fullfile(tempdir, "offlineSandbox-stop-" + string(port) + ".txt");
bundleFilePath      = fullfile(tempdir, "offlineSandbox-bundle-" + string(port) + ".mat");
bundleRequestIdPath = fullfile(tempdir, "offlineSandbox-bundle-" + string(port) + ".txt");

%% Section 2: Bind The Loopback Server

loopbackAddress = java.net.InetAddress.getByName('127.0.0.1');
try
    serverSocket = java.net.ServerSocket(int32(port), int32(8), loopbackAddress);
catch exception
    error("serveSandbox:BindFailed", "Could not bind 127.0.0.1:%d: %s", port, exception.message);
end
...
while ~isfile(stopFilePath)
    ...
    try
        [wasPlanRequest, wasBundleRequest] = serveClient(clientSocket, pageBytes, sandboxFolder, port, bundleFilePath, bundleRequestIdPath);
        planRequestCount   = planRequestCount + double(wasPlanRequest);
        bundleRequestCount = bundleRequestCount + double(wasBundleRequest);
    catch exception
...
function [wasPlanRequest, wasBundleRequest] = serveClient(clientSocket, pageBytes, sandboxFolder, port, bundleFilePath, bundleRequestIdPath)
    ...
    elseif method == "GET" && path == "/"
        writeHttpResponse(clientSocket, 200, "OK", "text/html; charset=utf-8", pageBytes, strings(0, 1), corsOrigin);
    elseif method == "GET" && path == "/health"
        body = createHealthBody("ready", port, sandboxFolder);
        writeJsonResponse(clientSocket, 200, "OK", body, strings(0, 1), corsOrigin);
    elseif method == "POST" && path == "/plan"
        wasPlanRequest = true;
        servePlanningRequest(clientSocket, request.BodyBytes, corsOrigin, bundleFilePath, bundleRequestIdPath);
    elseif method == "POST" && path == "/run-bundle"
        wasPlanRequest = true;
        serveBundleReplayRequest(clientSocket, request.BodyBytes, corsOrigin, bundleFilePath, bundleRequestIdPath);
...
function servePlanningRequest(clientSocket, requestBytes, corsOrigin, bundleFilePath, bundleRequestIdPath)
    ...
    temporaryCleanup = onCleanup(@() deleteTemporaryFiles(requestFilePath, resultFilePath));
    planningTimer    = tic;
    try
        writeFileBytes(requestFilePath, requestBytes);
        activeRequestId = previewRequestId(requestBytes);
        [response, diagnosisBundle] = offlineSandbox.runPlanningRequest(requestFilePath, resultFilePath);
        resultBytes = readFileBytes(resultFilePath);
        cacheDiagnosisBundle(bundleFilePath, bundleRequestIdPath, activeRequestId, diagnosisBundle);
    catch exception
        if isUserInterruption(exception)
            rethrow(exception);
        end
...
function serveBundleReplayRequest(clientSocket, bundleBytes, corsOrigin, bundleFilePath, bundleRequestIdPath)
    ...
    resultFilePath     = string(tempname) + ".json";
    temporaryCleanup   = onCleanup(@() deleteTemporaryFiles(uploadedBundlePath, resultFilePath));
    planningTimer      = tic;
    try
        writeFileBytes(uploadedBundlePath, bundleBytes);
        [response, reproducedBundle] = offlineSandbox.replayDiagnosisBundle(uploadedBundlePath, resultFilePath);
        resultBytes = readFileBytes(resultFilePath);
        cacheDiagnosisBundle(bundleFilePath, bundleRequestIdPath, string(response.requestId), reproducedBundle);
    catch exception
        if isUserInterruption(exception)
            rethrow(exception);
        end
...
function cacheDiagnosisBundle(bundleFilePath, bundleRequestIdPath, requestId, diagnosisBundle)
    ...
    cacheCleanup           = onCleanup(@() deleteTemporaryFiles(temporaryBundlePath, temporaryRequestIdPath));
    save(char(temporaryBundlePath), 'diagnosisBundle', '-v7.3');
    writeFileBytes(temporaryRequestIdPath, unicode2native(char(requestId), "UTF-8"));
    [bundleMoved, bundleMessage] = movefile(temporaryBundlePath, bundleFilePath, "f");
    if ~bundleMoved || ~isfile(bundleFilePath)
        error("serveSandbox:BundleCacheFailed", "Could not cache the diagnosis bundle: %s", bundleMessage);
    end
    [requestIdMoved, requestIdMessage] = movefile(temporaryRequestIdPath, bundleRequestIdPath, "f");
    if ~requestIdMoved || ~isfile(bundleRequestIdPath)
        deleteFileIfPresent(bundleFilePath);
        error("serveSandbox:BundleCacheFailed", "Could not cache the bundle request identifier: %s", requestIdMessage);
    end
    clear cacheCleanup;
end
...
function request = readHttpRequest(clientSocket, readTimeout_ms)
    ...
    while headerByteCount < maximumHeaderBytes
        ...
        if value < 0
            error("serveSandbox:TruncatedHeaders", "The HTTP connection ended before the headers were complete.");
        end
        headerByteCount = headerByteCount + 1;
        headerBytes(headerByteCount) = uint8(value);
        if headerByteCount >= 4 && isequal(headerBytes(headerByteCount - 3:headerByteCount), uint8([13 10 13 10]))
            headerComplete = true;
            break;
        end
    end
    if ~headerComplete
        error("serveSandbox:HeadersTooLarge", "HTTP headers exceed the 32768-byte limit.");
    end

    headerText   = native2unicode(headerBytes(1:headerByteCount), "UTF-8");
    ...
    if numel(requestParts) ~= 3 || ~startsWith(requestParts{3}, 'HTTP/')
        error("serveSandbox:InvalidRequestLine", "The HTTP request line is invalid.");
    end
    method = upper(string(requestParts{1}));
    target = string(requestParts{2});
    if ~startsWith(target, "/")
        error("serveSandbox:InvalidRequestTarget", "The HTTP request target must be an absolute path.");
    end
    ...
    for lineIndex = 2:numel(lines)
        ...
        if isempty(colonIndex)
            error("serveSandbox:InvalidHeader", "Each HTTP header must contain a colon.");
        end
        ...
        if name == "transfer-encoding" && lower(value) ~= "identity"
            error("serveSandbox:UnsupportedTransferEncoding", "Chunked or encoded HTTP request bodies are not supported.");
        elseif name == "content-length"
            if hasContentLength || isempty(regexp(char(value), '^\d+$', 'once'))
                error("serveSandbox:InvalidContentLength", "Content-Length must be one nonnegative decimal integer.");
            end
            contentLength    = str2double(value);
            hasContentLength = true;
        elseif name == "origin"
            if hasOrigin
                error("serveSandbox:DuplicateOrigin", "An HTTP request may contain only one Origin header.");
            end
            origin    = value;
            hasOrigin = true;
        end
    ...
    if ~isfinite(contentLength) || contentLength > maximumBodyBytes
        error("serveSandbox:PayloadTooLarge", "HTTP request bodies may not exceed %d bytes.", maximumBodyBytes);
    end
    ...
    if contentLength == 0
        ...
        while bodyBuffer.hasRemaining()
            ...
            if readByteCount < 0
                error("serveSandbox:TruncatedBody", "The HTTP body ended before Content-Length bytes arrived.");
            end
        end
        signedBody = int8(bodyBuffer.array());
        bodyBytes  = typecast(reshape(signedBody, 1, []), 'uint8');
    end
    request = struct("Method", method, ...
        "Path", path, ...
        "BodyBytes", bodyBytes, ...
        "Origin", origin, ...
        "HasOrigin", hasOrigin);
...
function writeJsonResponse(clientSocket, statusCode, reason, value, headers, corsOrigin)
...
function writeHttpResponse(clientSocket, statusCode, reason, contentType, bodyBytes, additionalHeaders, corsOrigin)
...
function requestId = previewRequestId(requestBytes)
    ...
    if (isstring(candidate) && isscalar(candidate)) || (ischar(candidate) && isrow(candidate))
...
function isTimeout = isSocketTimeout(exception)
    ...
    isTimeout = contains(string(exception.message), "java.net.SocketTimeoutException");
end
...
function isInterruption = isUserInterruption(exception)
    ...
    isInterruption = contains(identifier, "OperationTerminated") || contains(message, "Operation terminated by user");
end
...
function bytes = readFileBytes(filePath)
    ...
    if fileIdentifier < 0
        error("serveSandbox:FileOpenFailed", "Could not open '%s': %s", filePath, openMessage);
    end
    try
    ...
    if closeStatus ~= 0
        error("serveSandbox:FileCloseFailed", "Could not close '%s' after reading.", filePath);
    end
end
...
function writeFileBytes(filePath, bytes)
    ...
    if fileIdentifier < 0
        error("serveSandbox:FileOpenFailed", "Could not create '%s': %s", filePath, openMessage);
    end
    try
    ...
    if writtenByteCount ~= numel(bytes) || closeStatus ~= 0
        error("serveSandbox:FileWriteFailed", "Could not write all request bytes to '%s'.", filePath);
    end
end
...
function closeServer(serverSocket, stopFilePath, bundleFilePath, bundleRequestIdPath)
