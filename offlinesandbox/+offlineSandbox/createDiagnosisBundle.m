...
plannerInputs.goalState = request.goalState;
plannerInputs.limits = request.limits;
if isfield(plannerInputs, "options"), plannerInputs = rmfield(plannerInputs, "options"); end

%% Section 2: Preserve Browser And Canonical Scene Geometry

rawObstacleStrokes_units       = cell(numel(request.obstacles), 1);
polygonObstaclePositions_units = cell(numel(request.obstacles), 1);
...
for obstacleIndex = 1:numel(request.obstacles)
    ...
    rawObstacleStrokes_units{obstacleIndex} = keyframes(1).vertices_units;
    polygonObstaclePositions_units{obstacleIndex} = keyframes(1).vertices_units;
end

scene = struct("StartPosition_units", request.initialState.position_units, ...
    "GoalPosition_units", request.goalState.position_units, ...
    "RawObstacleStrokes_units", {rawObstacleStrokes_units}, ...
    "LineObstaclePositions_units", {cell(0, 1)}, ...
    "PolygonObstaclePositions_units", ...
        {polygonObstaclePositions_units}, ...
    "PolygonMotionVectors_units", NaN(numel(request.obstacles), 2), ...
...
%% Section 3: Assemble The Reproducible Bundle

exportRequest = struct("HasCompleteScene", true, ...
