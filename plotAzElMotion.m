...
if options.ShowWorkspace
    ...
    partialTrajectory = bestPartialTrajectory(result.SearchDiagnostics);
    if ~result.Success && ~isempty(partialTrajectory.time_s)
        plot(workspaceAxes, partialTrajectory.position_deg(:, 1), ...
            partialTrajectory.position_deg(:, 2), "-", ...
            "Color", [0.85 0.33 0.10], "LineWidth", 1.5, ...
            "DisplayName", "Failed HS3 candidate");
    end
...
if options.ShowVisibilityGraphs
    ...
    if result.Success
        ...
        partialTrajectory = bestPartialTrajectory( ...
            result.SearchDiagnostics);
        if ~isempty(partialTrajectory.time_s)
            plot3(visibilityAxes, ...
                partialTrajectory.position_deg(:, 1), ...
                partialTrajectory.position_deg(:, 2), ...
                partialTrajectory.time_s, "-", ...
                "Color", [0.85 0.33 0.10], "LineWidth", 1.5, ...
                "DisplayName", "Failed HS3 candidate");
        end
    end
...
function partial = bestPartialTrajectory(searchDiagnostics)
% PURPOSE
%   - Read a retained failed trajectory without reconstructing solver state.
partial = struct("time_s", zeros(0, 1), ...
    "position_deg", zeros(0, 2));
if isfield(searchDiagnostics, "BestPartialTrajectory")
    partial = searchDiagnostics.BestPartialTrajectory;
end
end
