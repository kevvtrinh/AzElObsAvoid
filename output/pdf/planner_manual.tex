...
\end{minipage}
\vfill
\large Recorded-example guide; see the appendix for maintained interfaces
\end{titlepage}
...
right; the other tips it up or down. The software calls them \key{x} and
\key{y}. For planning, it lays those two angles flat on a map. A dot on
...
\fill[blue] (-5,0) circle (3pt) node[above right,align=center]{start\\direction};
...
\node[rotate=90] at (-6.55,-.1){y};
\node at (0,-3.55){x};
\end{tikzpicture}
\caption{One direction is one place on the flat x--y map.}
\label{fig:map}
\end{figure}
...
xlabel={time (seconds)},ylabel={fastest coordinate speed (coordinate units per second)},
...
within 0.0131 coordinate units per second of the limit; it does not claim to touch it.}
\label{fig:speed}
\end{figure}
...
smooth motion, the safety buffer, and collision freedom. The planner reports success only after that checker says the candidate passed.
Rejected motion may remain available in a failure result for inspection.
