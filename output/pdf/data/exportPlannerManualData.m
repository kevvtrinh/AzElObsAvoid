...
selectedSummary = diagnosis.Attempts( ...
    [diagnosis.Attempts.SeedIndex] == diagnosis.SelectedAttemptIndex);
...
writematrix(protectedPolygon_deg, fullfile(dataDirectory, "protected_obstacle.dat"), ...
    "Delimiter", "tab");
writematrix(result.Route_deg, fullfile(dataDirectory, "selected_seed.dat"), ...
    "Delimiter", "tab");
...
solver = diagnosis.SolverDetails(diagnosis.SolverDetails.Attempt == ...
    diagnosis.SelectedAttemptIndex, {'Field','Value'});
...
repeatData = [iterationIndex, trialDuration_s(validTrial), ...
    collisionPairCountHistory(validTrial), ...
    double(trialWasCollisionFree(validTrial))];
...
summary = struct( ...
    "SelectedSeedIndex", diagnosis.SelectedAttemptIndex, ...
...
fprintf(fileIdentifier, "\\newcommand{\\WalkDirectDuration}{%.12f s}\n", ...
    detailValue(directAttempt, "MotionDuration_s"));
