...
function testEquivalentRingOrderDoesNotCreateMotion(testCase)
    ...
    for queryTime_s = [0.5 1.5]
        ...
        verifyEqual(testCase, geometry.VertexSpeedBound_deg_s, 0, "AbsTol", 1e-12);
    end
end
...
function testRotatingRectangleUsesLinearVertexMotion(testCase)
    ...
    obstacle  = obstacleAvoidance.obstacles.createObstacle("sampled rotating rectangle", [0; 2], {lower_deg(:, 1); upper_deg(:, 1)}, {lower_deg(:, 2); upper_deg(:, 2)}, 0);
    [~, geometry] = obstacleAvoidance.obstacles.shapeAtTime(obstacle, 1, true);

    verifyEqual(testCase, [geometry.azimuth_deg, geometry.elevation_deg], 0.5 * (lower_deg + upper_deg), "AbsTol", 1e-12);
...
function testUnprovenCorrespondenceContainsSweptGap(testCase)
    ...
    obstacle       = obstacleAvoidance.obstacles.createObstacle("unproven correspondence", [0; 2], {left_deg(:, 1); right_deg(:, 1)}, {left_deg(:, 2); right_deg(:, 2)}, 0);
...
function testNestedTopologyChangePreservesLargerConcavity(testCase)
    ...
    obstacle          = obstacleAvoidance.obstacles.createObstacle("nested topology transition", [0; 2], {largerL_deg(:, 1); smallerSquare_deg(:, 1)}, {largerL_deg(:, 2); smallerSquare_deg(:, 2)}, 0);
...
function testVisibilityStatusIsConservativeMetadata(testCase)
    ...
    obstacle     = obstacleAvoidance.obstacles.createObstacle("status metadata", [0; 1], boundary_deg(:, 1), boundary_deg(:, 2), 0);
...
function testStaticQueryPreservesActivitySpan(testCase)
    ...
    boundary_deg   = [-1 -1; 1 -1; 1 1; -1 1];
    finiteHistory  = obstacleAvoidance.obstacles.createObstacle("finite static history", [1; 3], boundary_deg(:, 1), boundary_deg(:, 2), 0);
    ...
    singleSample   = obstacleAvoidance.obstacles.createObstacle("single static sample", 2, boundary_deg(:, 1), boundary_deg(:, 2), 0);
