...
function testBundlePreservesExactResultAndRemovesCallback(testCase)
    ...
    verifyFalse(testCase, isfield(bundle.PlannerOptions, "TransientHandle"));
...
function testPageAndServerExposeMatchedBundleDownload(testCase)
    ...
    verifyTrue(testCase, contains(htmlText, 'Missing edges are not evidence of rejection.'));
    ...
    verifyTrue(testCase, contains(htmlText, 'requestJson(`${state.serverOrigin}/save-bundle`'));
...
function testOptionalBundlePreservesFileAndMotionOutputs(testCase)
    ...
    verifyEqual(testCase, rmfield(single.result,'ElapsedTime_s'), rmfield(paired.result,'ElapsedTime_s'));
