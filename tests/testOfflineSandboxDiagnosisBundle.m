...
selectedSummary = reproducedBundle.Diagnosis.Attempts( ...
    reproducedBundle.Diagnosis.SelectedAttemptIndex);
verifyLessThan(testCase, selectedSummary.MotionLength_deg + selectedSummary.TrajectoryDuration_s, 217, ...
    "Preserve the historical travel-plus-duration regression bound.");
if ~isempty(fieldnames(reproducedBundle.Diagnosis.Search))
    verifyLessThan(testCase, reproducedBundle.Diagnosis.Search. ...
        RouteShorteningCandidateCount, 1000, ...
        "Same-class route cleanup must not restore the discarded candidate sweep.");
    verifyTrue(testCase, reproducedBundle.Diagnosis.Search. ...
        RouteClassSearchTruncated, ...
        "The bounded route-class search must report its completeness limit.");
else
    % A validated exact motion needs no route-class search.
    verifyEqual(testCase, reproducedBundle.Diagnosis. ...
        AttemptedCount, 1);
end
clear cleanup;
end
