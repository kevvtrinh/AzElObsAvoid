...
edgeDuration_s = 4 * candidate.Polynomial.SegmentDuration_s(1:4:end);
% Check each interior waypoint because every polyline corner must be a complete stop.
...
options = plannerOptions("earliestArrival");
seed = geometricSeed([0 0; 4 0]);
validCandidate = azElPlannerMethods.hs3.internal.motion.buildStopWaypointMotion( ...
    [], initialState, goalState, limits, options, seed);
initialState.velocity_deg_s = [0.1 0];
earlyFailure = azElPlannerMethods.hs3.internal.motion.buildStopWaypointMotion( ...
    [], initialState, goalState, limits, options, seed);
