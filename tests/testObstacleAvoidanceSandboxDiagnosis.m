...
function testSuccessfulBundleRoundTripsAndReproduces(testCase)
    ...
    verifyEqual(testCase, bundle.PlannerInputs.goalState, goalState);
...
function testSandboxDefaultsBoundInteractivePlannerWork(testCase)
    ...
    verifyFalse(testCase, isfield(plannerOptions, "MaximumSeedCount"));
    verifyFalse(testCase, isfield(plannerOptions, "ArrivalTimeTolerance_s"));
end
