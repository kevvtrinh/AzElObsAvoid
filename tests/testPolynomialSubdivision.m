...
function testCompletePolynomialEdgeAdapterIsLossless(testCase)
    ...
    [candidate,diagnostics]=bmtpEngine.solve(seed,cell(0,1), ...
        struct('Passed',true),base.Inputs.initialState, ...
        base.Inputs.goalState,base.Limits,base.Options);
    verifyTrue(testCase,candidate.Success);
    verifyEqual(testCase,diagnostics.Identifier,"completePolynomialSeed");
    verifyEqual(testCase,diagnostics.ConstraintRepresentation, ...
        "completePolynomialEdges");
    verifyEqual(testCase,candidate.Polynomial.positionPower_units, ...
        base.Polynomial.positionPower_units);
    verifyEqual(testCase,candidate.Polynomial.SegmentDuration_s, ...
        base.Polynomial.SegmentDuration_s);
    verifyTrue(testCase,candidate.PlaneCertificate.Passed);
    distantRegion_units=[20,20;21,20;21,21;20,21];
    timedCoverage=struct('Passed',true, ...
        'ActiveTimeInterval_s',[initial.time_s,goal.time_s], ...
        'EndRegions_units',{{distantRegion_units}});
    [timedCandidate,timedDiagnostics]=bmtpEngine.solve(seed, ...
        {distantRegion_units},timedCoverage,base.Inputs.initialState, ...
        base.Inputs.goalState,base.Limits,base.Options);
    verifyTrue(testCase,timedCandidate.Success);
    verifyEqual(testCase,timedDiagnostics.Identifier, ...
        "completePolynomialSeed");
    verifyEqual(testCase,timedCandidate.Polynomial.positionPower_units, ...
        base.Polynomial.positionPower_units);
    verifyTrue(testCase,timedCandidate.PlaneCertificate.Passed);
    alteredEdges=edges;
