...
scene = obstacleAvoidance.obstacles.preparePlanningScene( ...
    request.obstacles, request.initialState, request.goalState);
...
verifyEqual(testCase, scene.preparedObstacles(2).InternalPreparation.IntervalGeometryModel, ...
    "linearCorrespondingVertices");

proposal = obstacleAvoidance.search.createRouteSearchGeometry( ...
    request.initialState, request.goalState, request.options, ...
    scene);
...
seedSet = obstacleAvoidance.search.createPathGuesses( ...
    request.initialState, request.goalState, request.limits, request.options, ...
    routeSet, proposal.shape.Vertices);
...
recoveredRouteSet = obstacleAvoidance.search.searchRoutes( ...
    request.initialState, request.goalState, request.limits, request.options, ...
    scene, proposal, visibilityGraph, routeSet);
...
[route_deg, routeTime_s] = ...
    ...
    nodePosition_deg, edgeCost_deg, obstacleAvoidance.obstacles.prepareObstacles(struct.empty(0, 1)), initialState, ...
...
[route_deg, routeTime_s, record] = ...
    obstacleAvoidance.search.timeExpandedVisibilitySearch( ...
    nodePosition_deg, edgeCost_deg, obstacleAvoidance.obstacles.prepareObstacles(struct.empty(0, 1)), ...
...
[route_deg, routeTime_s, record] = ...
    obstacleAvoidance.search.timeExpandedVisibilitySearch( ...
    nodePosition_deg, zeros(4), obstacleAvoidance.obstacles.prepareObstacles(obstacles), ...
...
[route_deg, routeTime_s, record] = ...
    obstacleAvoidance.search.timeExpandedVisibilitySearch( ...
    nodePosition_deg, edgeCost_deg, obstacleAvoidance.obstacles.prepareObstacles(barrier), initialState, goalState, ...
...
[route_deg, routeTime_s] = ...
    obstacleAvoidance.search.timeExpandedVisibilitySearch( ...
    nodePosition_deg, edgeCost_deg, obstacleAvoidance.obstacles.prepareObstacles(obstacles), initialState, goalState, ...
...
verifyTrue(testCase, ...
    resultDiagnosis.SearchCoverage.TimedSearchInitialDeferred);
verifyTrue(testCase, ...
    resultDiagnosis.SearchCoverage.TimedSearchRecoveryAttempted);
verifyTrue(testCase, ...
    resultDiagnosis.SearchCoverage.TimedSearchAttempted);
end
...
verifyEqual(testCase, candidate.ArrivalTime_s, goalState.time_s, ...
    "AbsTol", 1e-12);
end
...
verifyEqual(testCase, resultDiagnosis.Attempts(1).SeedSource, ...
    "directRestToRest");
...
twoSeedOptions = struct( ...
    "GoalTimeMode", "earliestArrival", "MaximumSeedCount", 2);
[twoSeedResult, twoSeedResultDiagnosis] = obstacleAvoidance.planTrajectory( ...
...
verifyEqual(testCase, ...
    twoSeedResultDiagnosis.AttemptedCount, 2);

recoveryOptions = twoSeedOptions;
recoveryOptions.MaximumSeedCount = 5;
[recoveredResult, recoveredResultDiagnosis] = obstacleAvoidance.planTrajectory( ...
...
obstacle = obstacleAvoidance.obstacles.createObstacle( ...
    "earliest direct wait", obstacleTime_s, ...
...
directWaitIndex = find( ...
    string({resultDiagnosis.Routes.Source}) == "directWait", 1);
verifyTrue(testCase, result.Success, result.Message);
...
verifyLessThan(testCase, testSupport.diagnosisValue(diagnostics, "FinalWaitTime_s"), ...
    testSupport.diagnosisValue(diagnostics, "InitialWaitTime_s"));
end
...
verifyFalse(testCase, any(startsWith( ...
    diagnostics.Field, "TravelRefinement")));
...
verifyEqual(testCase, testSupport.diagnosisValue(diagnostics, "SolverMessage"), ...
...
collisionFreeTrials_s = trialDurations_s( ...
    testSupport.diagnosisValue(diagnostics, "TrialWasCollisionFree"));
verifyEqual(testCase, testSupport.diagnosisValue(diagnostics, "RetainedBestTrialDuration_s"), ...
...
verifyLessThanOrEqual(testCase, testSupport.diagnosisValue(diagnostics, "TravelRefinement.FinalLength_deg"), ...
    testSupport.diagnosisValue(diagnostics, "TravelRefinement.InitialLength_deg"));
verifyEqual(testCase, result.TrajectoryDuration_s, 12.5, ...
    "AbsTol", 1e-9);
verifyEqual(testCase, result.Route_deg, result.position_deg);
verifyEqual(testCase, resultDiagnosis.Routes(1).Length_deg, ...
...
verifyLessThan(testCase, resultDiagnosis.Routes(1).Length_deg, ...
    1.0125 * directLength_deg, ...
    "The exact-clock detour contains unnecessary joint travel.");
end
...
verifyEqual(testCase, result.TrajectoryDuration_s, 12.5, ...
    "AbsTol", 1e-9);
verifyLessThan(testCase, sum(vecnorm(diff(result.position_deg), 2, 2)), 20.55, ...
    "The exact-clock detour contains unnecessary joint travel.");
...
verifyEqual(testCase, ...
    resultDiagnosis.Selection.JerkRole, ...
    "hardConstraintOnly");
end
...
for mode = ["fixedArrival", "earliestArrival"]
    selection = obstacleAvoidance.planner.selectValidatedCandidate( ...
        summary, struct("GoalTimeMode", mode));
...
selection = obstacleAvoidance.planner.selectValidatedCandidate( ...
    summary, struct("GoalTimeMode", "earliestArrival"));
...
verifyEmpty(testCase, result.time_s);
verifyTrue(testCase, isfield(resultDiagnosis.Search, ...
    "ExpandedCount"));
verifyGreaterThanOrEqual(testCase, ...
    resultDiagnosis.AttemptedCount, 1);
end
...
[result, resultDiagnosis] = obstacleAvoidance.planMovingTargetIntercept( ...
...
verifyEqual(testCase, result.Options.GoalTimeMode, ...
    "fixedArrival");
verifyEqual(testCase, testSupport.diagnosisValue(resultDiagnosis.InterceptSearch, "OptimalityStatus"), ...
    "notAnOptimization");
...
[asapResult, asapResultDiagnosis] = obstacleAvoidance.planMovingTargetIntercept( ...
...
verifyEqual(testCase, testSupport.diagnosisValue(asapResultDiagnosis.InterceptSearch, "OptimalityStatus"), ...
    "resolutionBounded");
verifyEqual(testCase, testSupport.diagnosisValue(asapResultDiagnosis.InterceptSearch, "MaximumCoarseStep_s"), ...
    0.5, "AbsTol", 1e-12);
...
[certifiedResult, certifiedResultDiagnosis] = obstacleAvoidance.planMovingTargetIntercept( ...
    restState(0, [0 0]), certifiedTarget, physicalLimits(), ...
    certifiedOptions);
verifyTrue(testCase, certifiedResult.Success, certifiedResult.Message);
verifyEqual(testCase, testSupport.diagnosisValue(certifiedResultDiagnosis.InterceptSearch, "OptimalityStatus"), ...
    "certifiedEarliest");
end
