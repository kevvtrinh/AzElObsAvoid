...
requiredFields = {'GoalTimeMode', 'SampleTime_s', 'MaximumSeedCount', ...
    'MaximumWaitRefinementIterations', 'EnableStagnationStop', ...
    'StagnationIterationLimit', 'StagnationImprovementTolerance_s', ...
...
function testStagnationStopResetsCounterAndRetainsBestTrial(testCase)
% Exercise retained-best stagnation without changing the returned candidate.
repositoryRoot = fileparts(fileparts(mfilename("fullpath")));
addpath(fullfile(repositoryRoot, "examples"));
overrides = struct( ...
    "CollisionClearanceTolerance_deg", 1e-4, ...
    "EnableStagnationStop", true, ...
    "EnablePlaneReuse", false, ...
    "StagnationIterationLimit", 3, ...
    "StagnationImprovementTolerance_s", 1e-3, ...
    "MaximumSeedCount", 2, "FigureVisible", "off", ...
    "PlotOutputs", false, "ShowAnimation", false, ...
...
verifyTrue(testCase, diagnostics.StoppedOnStagnation);
stagnationHistory = diagnostics.StagnationIterationCountHistory;
stagnationHistory = stagnationHistory(~isnan(stagnationHistory));
verifyTrue(testCase, any(stagnationHistory == 0));
verifyTrue(testCase, any(stagnationHistory(2:end) == 0));
verifyEqual(testCase, stagnationHistory(end), ...
    overrides.StagnationIterationLimit);
...
overrides = struct( ...
    "CollisionClearanceTolerance_deg", 1e-4, ...
    "EnablePlaneReuse", true, "MaximumSeedCount", 2, ...
