...
adapterText = string(fileread(fullfile( ...
    plannerRoot, "solveStaticBmtpTrajectory.m")));
...
exactStageText = string(fileread(fullfile( ...
    plannerRoot, "tryDirectAndFixedTimeMotions.m")));
fixedClockText = string(fileread(fullfile( ...
    plannerRoot, "tryFixedTimeDetour.m")));
recoveryText = string(fileread(fullfile( ...
    plannerRoot, "tryAdditionalPathGuesses.m")));
...
verifyTrue(testCase, contains(plannerText, ...
    "obstacleAvoidance.planner.tryDirectAndFixedTimeMotions("));
...
stageCalls = [ ...
    "resolvePlannerOptions(", "normalizePlannerRequest(", ...
    "preparePlanningScene(", ...
    "validatePlannerEndpoints(", "tryDirectAndFixedTimeMotions(", ...
    "createRouteSearchGeometry(", "createVisibilityGraph(", ...
    "searchRoutes(", "createPathGuesses(", "solvePathGuess(", ...
    "tryAdditionalPathGuesses(", ...
    "selectValidatedCandidate("];
...
for stageIndex = 1:numel(stageCalls)
    ...
    if stageCalls(stageIndex) == "tryDirectAndFixedTimeMotions("
...
seedLocations = strfind(plannerExecutable, "createPathGuesses(");
verifyEqual(testCase, numel(routeLocations), 1, ...
    "The planner trunk must own one initial route-search call.");
verifyEqual(testCase, numel(seedLocations), 1, ...
    "The planner must create the direct guess and searched routes together.");
verifyLessThan(testCase, seedLocations, ...
    stageLocations(stageCalls == "solvePathGuess("), ...
    "Initial guesses must be created before solving.");
verifyEqual(testCase, ...
    numel(strfind(recoveryText, ...
    "obstacleAvoidance.search.searchRoutes(")), 1, ...
    "Later-seed recovery must own its only resumable search call.");
verifyEqual(testCase, ...
    numel(strfind(recoveryText, ...
    "obstacleAvoidance.search.createRoutePathGuesses(")), 1, ...
    "Later-seed recovery must own its only deferred seed conversion.");
verifyEqual(testCase, ...
    numel(strfind(recoveryText, ...
    "obstacleAvoidance.planner.solvePathGuess(")), 1, ...
    "Later-seed recovery must use the maintained single-seed solver.");
repositoryRoot = fileparts(fileparts(mfilename("fullpath")));
routeSearchSource = fileread(fullfile(repositoryRoot, ...
    "+obstacleAvoidance", "+search", "searchRoutes.m"));
verifyEqual(testCase, ...
    numel(strfind(routeSearchSource, "searchDistinctSpatialRoutes(")), 1, ...
    "The route coordinator must have only one spatial-search call site.");
end
