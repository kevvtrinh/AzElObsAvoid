...
function testPlannerStagesUseOnlyTheirInputs(testCase)
    ...
    stages = ["obstacles.createObstaclePlanningData", "search.addRouteSearchGeometry", ...
...
function testObstacleAvoidancePackagesMatchResponsibilities(testCase)
    ...
    for sourceIndex = 1:numel(plottingSources)
        ...
        verifyEmpty(testCase, regexp(sourceText, forbiddenCall, "once"), sprintf("Plotting source reruns planner work: %s", sourcePath));
    end
    ...
    for sourceIndex = 1:numel(searchSources)
        ...
        sourceText = string(fileread(sourcePath));
        verifyFalse(testCase, contains(sourceText, "obstacleAvoidance.planner."), sprintf("Search source depends on planner adapters: %s", sourcePath));
    end
end
...
function testRuckigEngineHasNoObstaclePlannerDependency(testCase)
    ...
    for sourceIndex = 1:numel(sourceRecords)
        ...
        forbiddenPattern = "obstacleavoidance|bmtp|fmincon|coneprog|quadprog|optimoptions";
        verifyEmpty(testCase, regexp(sourceText, forbiddenPattern, "once"), sprintf("Ruckig source crosses its engine boundary: %s", sourcePath));
    end
end
...
function testBmtpEngineHasNoObstaclePlannerDependency(testCase)
    ...
    for sourceIndex = 1:numel(sourceRecords)
        sourcePath = fullfile(sourceRecords(sourceIndex).folder, sourceRecords(sourceIndex).name);
        sourceText = string(fileread(sourcePath));
        verifyFalse(testCase, contains(sourceText, "obstacleAvoidance."), sprintf("BMTP source depends on obstacle planning: %s", sourcePath));
    end
end
...
function testNumericalSolverCallsRemainInsideEngine(testCase)
    ...
    verifyEmpty(testCase, offendingPaths, sprintf("Numerical solver call outside BMTP engine: %s", strjoin(offendingPaths, ", ")));
end
