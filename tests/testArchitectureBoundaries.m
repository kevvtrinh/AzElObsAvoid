...
recoveryText = string(fileread(fullfile( ...
    plannerRoot, "recoverAdditionalSeeds.m")));
...
stageCalls = [ ...
    ...
    "validatePlannerEndpoints(", "solveExactCandidates(", ...
    "createRouteSearchGeometry(", "createVisibilityGraph(", ...
    "searchRoutes(", "createSeeds(", "solveOneSeed(", ...
...
verifyLessThan(testCase, seedLocations, ...
    stageLocations(stageCalls == "solveOneSeed("), ...
...
verifyEqual(testCase, ...
    numel(strfind(recoveryText, ...
    "obstacleAvoidance.search.createSeeds(")), 1, ...
    "Later-seed recovery must own its only deferred seed conversion.");
verifyEqual(testCase, ...
    numel(strfind(recoveryText, ...
    "obstacleAvoidance.planner.solveOneSeed(")), 1, ...
