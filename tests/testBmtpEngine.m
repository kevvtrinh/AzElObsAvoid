...
function testPublicWrapperRetainsDeprecatedRestartForms(testCase)
% Keep old arities callable while restart state is retired for one release.
initialState = createState(0, [-2 0]);
goalState = createState(10, [2 0]);
seed = struct("Index", 1, "Source", "unitDirect", ...
    "tau", [0; 1], "position_deg", [-2 0; 2 0], ...
...
regions_deg = {[3 3; 4 3; 4 4; 3 4]};
coverage = struct("Passed", true, "RegionCount", 1);
[candidate, diagnostics] = planTrajBmtp( ...
    seed, regions_deg, coverage, initialState, goalState, ...
    testCase.TestData.Limits, testCase.TestData.Options);
verifyTrue(testCase, candidate.Success, candidate.Message);
verifyTrue(testCase, diagnostics.Accepted);
verifyWarning(testCase, @() callDeprecatedRestartOutput( ...
    seed, regions_deg, coverage, initialState, goalState, ...
    testCase.TestData.Limits, testCase.TestData.Options), ...
    "planTrajBmtp:DeprecatedRestart");
verifyWarning(testCase, @() callDeprecatedRestartInput( ...
    seed, regions_deg, coverage, initialState, goalState, ...
    testCase.TestData.Limits, testCase.TestData.Options), ...
    "planTrajBmtp:DeprecatedRestart");
[candidate, diagnostics, restart] = callWithoutRestartWarning( ...
    seed, regions_deg, coverage, initialState, goalState, ...
    testCase.TestData.Limits, testCase.TestData.Options, struct());
verifyTrue(testCase, candidate.Success, candidate.Message);
verifyTrue(testCase, diagnostics.Accepted);
verifyEmpty(testCase, restart.ControlPoint_deg);
verifyTrue(testCase, isnan(restart.SegmentTime_s));
end

function testLegacyRestartIsIgnored(testCase)
% Prove a supplied legacy record resolves to the maintained cold solve.
initialState = createState(0, [-2 0]);
goalState = createState(10, [2 0]);
seed = struct("Index", 1, "Source", "unitDirect", ...
    "tau", [0; 1], "position_deg", [-2 0; 2 0], ...
...
coverage = struct("Passed", true, "RegionCount", 1);
options = createOptions("fixedArrival");
[coldCandidate, coldDiagnostics] = planTrajBmtp( ...
    seed, regions_deg, coverage, initialState, goalState, ...
    testCase.TestData.Limits, options);
[legacyCandidate, legacyDiagnostics, emptyRestart] = ...
    callWithoutRestartWarning( ...
    seed, regions_deg, coverage, initialState, goalState, ...
    testCase.TestData.Limits, options, ...
    struct("ControlPoint_deg", zeros(3, 17, 2), "SegmentTime_s", 1));

verifyTrue(testCase, coldCandidate.Success, coldCandidate.Message);
verifyTrue(testCase, coldDiagnostics.Accepted);
verifyTrue(testCase, legacyCandidate.Success, legacyCandidate.Message);
verifyTrue(testCase, legacyDiagnostics.Accepted);
verifyEqual(testCase, legacyCandidate.position_deg, ...
    coldCandidate.position_deg, "AbsTol", 1e-12);
verifyEqual(testCase, legacyCandidate.velocity_deg_s, ...
    coldCandidate.velocity_deg_s, "AbsTol", 1e-12);
verifyEqual(testCase, legacyCandidate.acceleration_deg_s2, ...
    coldCandidate.acceleration_deg_s2, "AbsTol", 1e-12);
verifyEqual(testCase, legacyCandidate.MotionDuration_s, ...
    coldCandidate.MotionDuration_s, "AbsTol", 1e-12);
verifyEmpty(testCase, emptyRestart.ControlPoint_deg);
verifyTrue(testCase, isnan(emptyRestart.SegmentTime_s));
end
...
function callDeprecatedRestartOutput( ...
        seed, regions_deg, coverage, initialState, goalState, limits, options)
% Request the retired third output so its migration warning is testable.
[~, ~, ~] = planTrajBmtp( ...
    seed, regions_deg, coverage, initialState, goalState, limits, options);
end

function callDeprecatedRestartInput( ...
        seed, regions_deg, coverage, initialState, goalState, limits, options)
% Supply the retired eighth input so its migration warning is testable.
planTrajBmtp( ...
    seed, regions_deg, coverage, initialState, goalState, limits, ...
    options, struct());
end

function [candidate, diagnostics, restart] = callWithoutRestartWarning( ...
        seed, regions_deg, coverage, initialState, goalState, limits, ...
        options, legacyRestart)
% Suppress the already-verified one-release restart warning.
warningState = warning("off", "planTrajBmtp:DeprecatedRestart");
