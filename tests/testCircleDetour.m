...
function testExactSavedCircleAvoidsLateOvershoot(testCase)
    ...
    baselineLength_units = obstacleAvoidance.geometry.routeLength(bundle.Result.position_units);
    % Require the path-first improvement, including removal of the jerk-cost veto.
    verifyLessThan(testCase, obstacleAvoidance.geometry.routeLength(result.position_units), 0.92 * baselineLength_units);
    verifyEqual(testCase, result.TrajectoryDuration_s, bundle.Result.TrajectoryDuration_s, 'AbsTol', 1e-9);
end

function testEarlyRectangleWithYGoverningClock(testCase)
    ...
    limits.xInterval_units    = [-40 40];
    limits.yInterval_units  = [-40 80];
    ...
    verifyLessThan(testCase, obstacleAvoidance.geometry.routeLength(result.position_units), 109);
end
...
function testPrescribedTurnPreservesMotionAndDefaultCompatibility(testCase)
    ...
    ordinary = bmtpEngine.createOffsetSplineMotion(direct, knots_s, offsets_units, 2, initial, options.SampleTime_s, 'turnRegression');
    emptyOverride = bmtpEngine.createOffsetSplineMotion(direct, knots_s, offsets_units, 2, initial, options.SampleTime_s, 'turnRegression', []);
...
function value = restState(time_s, position_units)
    value = struct();
    value.time_s = time_s;
    value.position_units = position_units;
    value.velocity_units_s = [0 0];
    value.acceleration_units_s2 = [0 0];
end
