...
function testConvexRectangleCoarsensToOneCell(testCase)
    ...
    shape = polyshape([0 4 4 0], [0 0 2 2], "Simplify", false, "KeepCollinearPoints", true);
...
function testConcaveNotchRetainsSmallTriangulation(testCase)
    ...
    shape         = polyshape(vertices, "Simplify", false, "KeepCollinearPoints", true);
...
function testCollinearBoundaryAndInputOrderAreDeterministic(testCase)
    ...
    firstShape    = polyshape(vertices, "Simplify", false, "KeepCollinearPoints", true);
    secondShape   = polyshape(flipud(circshift(vertices, 2, 1)), "Simplify", false, "KeepCollinearPoints", true);
    ...
    verifyEqual(testCase, regionSignatures(firstRegions), regionSignatures(repeatRegions));
    verifyEqual(testCase, regionSignatures(firstRegions), regionSignatures(secondRegions));
...
function areaTwice = signedArea(vertices)
    ...
    areaTwice    = sum(vertices(:, 1) .* nextVertices(:, 2) - vertices(:, 2) .* nextVertices(:, 1));
end
