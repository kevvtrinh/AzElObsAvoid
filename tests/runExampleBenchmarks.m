...
for caseIndex = 1:numel(caseNames)
    ...
    record.MeetsHistoricalQuality = record.Valid && (~logical(row{10}) || ...
    ...
    record.MeetsHistoricalSeedLength = ~logical(row{10}) || ~record.HasHistoricalReference || ...
        record.RouteLength_units <= record.ReferenceRouteLength_units+1e-8;
    record.MeetsHistoricalRuntime = record.Valid && (~record.HasHistoricalReference || ...
        record.WallTime_s <= record.ReferenceWallTime_s);
    record.MeetsAllHistorical = record.MeetsHistoricalQuality && record.MeetsHistoricalRuntime;
    records = [records; record]; %#ok<AGROW>
    fprintf('%s: valid=%d, duration=%.6g, length=%.6g, wall=%.4g, all=%d, %s\n', ...
        name, record.Valid, record.Duration_s, record.Length_units,record.WallTime_s, ...
        record.MeetsAllHistorical,message);
