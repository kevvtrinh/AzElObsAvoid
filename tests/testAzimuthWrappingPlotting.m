...
function testDisplayPathSplitsBothSeamDirections(testCase)
    ...
    increasingDisplay_deg = obstacleAvoidance.plotting.createWrappedSpatialPath(increasing_deg, [-180 180], true);
    decreasingDisplay_deg = obstacleAvoidance.plotting.createWrappedSpatialPath(decreasing_deg, [-180 180], true);
    verifyEqual(testCase, increasingDisplay_deg, [179 0; 180 1; NaN NaN; -180 1; -179 2]);
    verifyEqual(testCase, decreasingDisplay_deg, [-179 0; -180 1; NaN NaN; 180 1; 179 2]);
end
...
function testWrappedResultCreatesPeriodicAndContinuousViews(testCase)
    ...
    verifyEqual(testCase, continuousMotionHandle.XData, result.position_deg(:, 1).');
