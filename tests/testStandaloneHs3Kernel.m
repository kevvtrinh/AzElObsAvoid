...
function testFixedTimeShortensNonzeroBoundaryMotion(testCase)
% Verify fixed-time path ranking improves on the feasible zero-jerk motion.
initialState = struct( ...
    "time", 1, "position", [0 1], ...
...
verifyLessThan(testCase, returnedLength, zeroJerkLength - 1e-4);
end
...
function testDirectJerkSwitchingMatchesIndependentMinimum(testCase)
% Verify a jerk-limited multi-axis line matches the closed-form lower bound.
initialState = struct( ...
    "time", 0, "position", [0 0], ...
...
expectedRampTime = nthroot(1 / 2, 3);
expectedDuration = 4 * expectedRampTime;
verifyTrue(testCase, trajectory.Success, trajectory.Message);
verifyTrue(testCase, trajectory.Validation.Passed, ...
    trajectory.Validation.Message);
verifyEqual(testCase, trajectory.Duration, expectedDuration, ...
    "AbsTol", 1e-10);
verifyEqual(testCase, trajectory.Diagnostics.Output.algorithm, ...
    "closedFormJerkSwitching");
verifyEqual(testCase, trajectory.Polynomial.SegmentCount, 4);
verifyEqual(testCase, ...
    sum(vecnorm(diff(trajectory.position), 2, 2)), sqrt(5), ...
    "AbsTol", 1e-10);
end
