...
% Add the trajectory product folder for direct test runs.
...
function testFixedTimePreservesZeroJerkBoundaryMotion(testCase)
% Verify pure HS3 retains the minimum-jerk zero-control fixed-time motion.
initialState = struct( ...
    "time", 1, "position", [0 1], ...
...
options = struct( ...
    ...
    "SegmentCount", 5);
trajectory = planTrajHs3(initialState, terminalState, limits, options);
verifyTrue(testCase, trajectory.Success, trajectory.Message);
relativeTime = trajectory.time - initialState.time;
zeroJerkPosition = initialState.position + ...
    relativeTime .* initialState.velocity + ...
    0.5 * relativeTime .^ 2 .* initialState.acceleration;
returnedLength = sum(vecnorm(diff(trajectory.position, 1, 1), 2, 2));
zeroJerkLength = sum(vecnorm(diff(zeroJerkPosition, 1, 1), 2, 2));
verifyEqual(testCase, returnedLength, zeroJerkLength, "AbsTol", 1e-10);
end
...
function testEarliestHs3ProgressIsFeasibleAndLocallyTimeMinimal(testCase)
% Verify pure HS3 remains close to the direct lower bound and locally timed.
initialState = struct( ...
...
verifyGreaterThanOrEqual(testCase, motionLength + 1e-10, straightLength);
verifyLessThan(testCase, motionLength - straightLength, 0.01);

earlierOptions = options;
earlierOptions.TimeMode = "fixed";
earlierOptions.FinalTime = trajectory.FinalTime - 2e-3;
earlier = hs3Engine.solve( ...
...
function testHs3DoesNotReportClosedFormSwitching(testCase)
% Verify an eligible request still runs pure collocation when HS3 is explicit.
initialState = struct( ...
    "time", 0, "position", [0 0], ...
...
verifyTrue(testCase, trajectory.Validation.Passed, ...
    trajectory.Validation.Message);
verifyNotEqual(testCase, string(trajectory.Diagnostics.Output.algorithm), ...
    "closedFormJerkSwitching");
verifyEqual(testCase, trajectory.Polynomial.SegmentCount, 10);
verifyGreaterThanOrEqual(testCase, trajectory.Duration, ...
    4 * nthroot(1 / 2, 3));
end
...
trajectory = hs3Engine.solve(initialState, terminalState, limits, struct( ...
...
[~, position] = hs3Engine.polynomial.evaluateTrajectoryPolynomial( ...
...
[expectedSegment, expectedMap] = hs3Engine.polynomial.createSubintervalBernsteinMap( ...
    tauStart, tauEnd, 5, 6);
[actualSegment, actualMap] = hs3Engine.polynomial.createSubintervalBernsteinMap( ...
    tauStart, tauEnd, 5, 6);
...
defaults = hs3Engine.solve();
verifyEqual(testCase, defaults, hs3Engine.defaultOptions());
[initialState, terminalState, limits] = fixedRestToRestProblem(2);
trajectory = hs3Engine.solve(initialState, terminalState, limits, struct( ...
    "TimeMode", "fixed", "FinalTime", terminalState.maximumTime, ...
...
verifyEqual(testCase, ...
        trajectory.Options.SegmentCount, defaults.SegmentCount);
...
verifyWarning(testCase, @() hs3Engine.solve( ...
    initialState, terminalState, limits, overrides), ...
    "hs3Engine:UnknownOptions");
lastwarn("");
trajectory = hs3Engine.solve(initialState, terminalState, limits, overrides);
...
function testSuccessAndFailureReturnTheSameFormat(testCase)
% Verify expected infeasibility does not drop public result fields.
[initialState, terminalState, limits] = fixedRestToRestProblem(1);
success = hs3Engine.solve(initialState, terminalState, limits, struct( ...
    "TimeMode", "fixed", "FinalTime", terminalState.maximumTime));
terminalState.position = 10;
terminalState.maximumTime = 0.1;
failure = hs3Engine.solve(initialState, terminalState, limits, struct( ...
    "TimeMode", "fixed", "FinalTime", terminalState.maximumTime));
...
hs3Root = fullfile(repositoryRoot, "trajectory", "+hs3Engine");
