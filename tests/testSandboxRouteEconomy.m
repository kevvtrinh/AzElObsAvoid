...
function testStaticCircleHasOneEconomicalDetour(testCase)
    ...
    obstacles  = obstacleAvoidance.obstacles.createObstacle("static circle", [0; request.goalState.time_s], circle_deg(:, 1), circle_deg(:, 2), safetyMargin_deg);

    [result, resultDiagnosis] = runAndValidate(testCase, obstacles, request);
    protectedRadius_deg     = circleRadius_deg + safetyMargin_deg;
    halfChord_deg           = 0.5 * norm(request.goalState.position_deg - request.initialState.position_deg);
    tangentLength_deg       = sqrt(halfChord_deg^2 - protectedRadius_deg^2);
    arcAngle_rad            = 2 * asin(protectedRadius_deg / halfChord_deg);
    geometricLowerBound_deg = 2 * tangentLength_deg + protectedRadius_deg * arcAngle_rad;
...
function testIrregularStaticObstacleAvoidsRepeatedJointMotion(testCase)
    ...
    obstacles = obstacleAvoidance.obstacles.createObstacle("irregular static obstacle", [0; request.goalState.time_s], star_deg(:, 1), star_deg(:, 2), 0.25);

    result             = runAndValidate(testCase, obstacles, request);
    directDistance_deg = norm(request.goalState.position_deg - request.initialState.position_deg);
    verifyLessThanOrEqual(testCase, motionLength(result), 1.04 * directDistance_deg, "The irregular static detour adds over 4%% joint travel.");
...
function testMovingIrregularObstacleAvoidsRepeatedJointMotion(testCase)
    ...
    obstacles  = obstacleAvoidance.obstacles.createObstacle("irregular moving obstacle", [0; 10; request.goalState.time_s], {start_deg(:, 1); finish_deg(:, 1); finish_deg(:, 1)}, {start_deg(:, 2); finish_deg(:, 2); finish_deg(:, 2)}, 0.25);

    result             = runAndValidate(testCase, obstacles, request);
    directDistance_deg = norm(request.goalState.position_deg - request.initialState.position_deg);
    verifyLessThanOrEqual(testCase, motionLength(result), 1.02 * directDistance_deg, "The moving-obstacle detour adds over 2%% joint travel.");
...
function request = createRequest()
    ...
    request.limits       = struct("azimuthInterval_deg", [-10 10], ...
...
function count = lateralReversalCount(result)
    ...
    velocityTolerance_deg_s = 1e-8 * max(1, max(abs(lateralVelocity_deg_s)));
    significantSign         = sign(lateralVelocity_deg_s(abs(lateralVelocity_deg_s) > velocityTolerance_deg_s));
