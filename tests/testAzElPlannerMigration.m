...
function testAlternatingTwoSlotGateExampleSupportsAllCounts(testCase)
%% Section 0: Header & Readme
% Exercise the shared-plan scenario for every documented arrangement count.
options = struct( ...
    "FigureVisible", "off", ...
    "ShowAnimation", false, ...
    ...
    "ShowSweptSurfaces", false, ...
    "Verbose", false);
for arrangementCount = 1:5
    result = exampleAlternatingTwoSlotGatesAzElTimeSpace( ...
        arrangementCount, options);
    diagnostics = result.timedSlopePath.ConstraintDiagnostics;
    testCase.verifyTrue(result.Success);
    testCase.verifyTrue(result.Validation.Passed);
    testCase.verifyTrue(result.ScenarioValidation.Passed);
    testCase.verifyEqual(result.numberOfArrangements, arrangementCount);
    testCase.verifyEqual(result.obstacleField.ObstacleCount, 2);
    testCase.verifyTrue(result.SlotDiagnostics.SelectedSlotFullyClosed);
    testCase.verifyTrue(result.SlotDiagnostics.OppositeSlotRemainsOpen);
    testCase.verifyGreaterThan(result.timedSlopePath.WaitDuration_s, 0);
    testCase.verifyGreaterThanOrEqual( ...
        result.timedSlopePath.MotionStartTime_s, ...
        result.blockerPhaseStep_s - 1e-9);
    testCase.verifyTrue(diagnostics.FiniteJerkNumericallyVerified);
    testCase.verifyFalse(diagnostics.FiniteJerkCertified);
    testCase.verifyFalse(diagnostics.ContinuousJerkCertified);
    testCase.verifyTrue(diagnostics.VelocityCarriedAcrossG3Joins);
    testCase.verifyFalse(any(result.bestAttemptBlocked));
    closeTestFigures();
end
end
