...
function testTimedPathCollisionRejectsClearEndpointCornerCut(testCase)
%% Section 0: Header & Readme
% Verify an occupied chord cannot hide behind two clear samples.
[azimuth_deg, elevation_deg] = squareBoundary([0 0]);
obstacle = makeAzElObstacleData( ...
    "corner cut", [0; 10], azimuth_deg, elevation_deg, 0);
obstacleField = buildAzElTimeObstacleField(obstacle);
position_deg = [-1.2 0.8; 0.8 -1.2];
time_s = [4; 6];

sampleOccupied = queryAzElTimeObstacle( ...
    obstacleField, position_deg(:, 1), position_deg(:, 2), time_s, ...
    struct("BoundaryIsOccupied", false));
[pathOccupied, details] = queryAzElTimedPathCollision( ...
    obstacleField, time_s, position_deg, ...
    struct("BoundaryIsOccupied", false));

testCase.verifyFalse(any(sampleOccupied), ...
    "The fixture endpoints must stay outside the obstacle.");
testCase.verifyEqual(pathOccupied, [false; true]);
testCase.verifyTrue(details.SegmentOccupied);
testCase.verifyEqual(double(details.SegmentBlockingObstacleIndex), 1);
testCase.verifyGreaterThan(double(details.SegmentBlockingSliceIndex), 0);

tangentPosition_deg = [-2 -1; 2 -1];
freeTangent = queryAzElTimedPathCollision( ...
    obstacleField, 5, tangentPosition_deg, ...
    struct("BoundaryIsOccupied", false));
blockedTangent = queryAzElTimedPathCollision( ...
    obstacleField, 5, tangentPosition_deg, ...
    struct("BoundaryIsOccupied", true));
testCase.verifyFalse(any(freeTangent));
testCase.verifyTrue(any(blockedTangent));
end
...
function testGeographicExtremeRegionSequenceAvoidsEveryPolygon(testCase)
%% Section 0: Header & Readme
% Verify Hawaii, Croatia, and Philippines each return a continuously clear
% extreme-visibility trajectory in the documented sequence.
result = exampleUSOutlineExtremeVisibility(struct( ...
    "FigureVisible", "off", ...
    "ShowAnimation", false, ...
...
testCase.verifyEqual( ...
    result.RegionSequenceSummary.Region, ...
    ["Hawaii"; "Croatia"; "Philippines"]);
testCase.verifyTrue(result.RegionSequencePassed);
testCase.verifyTrue(all(result.RegionSequenceSummary.Passed));
testCase.verifyGreaterThan( ...
    result.RegionSequenceSummary.TestVertexCount, ...
    1000 * ones(3, 1));
testCase.verifyGreaterThan( ...
    sum(result.RegionSequenceSummary.TestVertexCount), 7500);
testCase.verifyEqual(numel(result.RegionSequenceResults), 3);
for regionIndex = 1:numel(result.RegionSequenceResults)
    regionResult = result.RegionSequenceResults{regionIndex};
    testCase.verifyTrue(regionResult.Success, ...
        sprintf("Region %s failed: %s", ...
        result.RegionSequenceSummary.Region(regionIndex), ...
        regionResult.Message));
    testCase.verifyTrue(regionResult.ExampleValidation.Passed);
    testCase.verifyTrue(any(regionResult.directBlocked));
    continuousCollision = queryAzElTimedPathCollision( ...
        regionResult.obstacleField, ...
        regionResult.timedSlopePath.time_s, ...
        regionResult.timedSlopePath.position_deg, struct( ...
        "TimePaddingSamples", 1, "BoundaryIsOccupied", false));
    testCase.verifyFalse(any(continuousCollision), ...
        sprintf("Region %s trajectory crossed protected polygon geometry.", ...
        result.RegionSequenceSummary.Region(regionIndex)));
end
end
