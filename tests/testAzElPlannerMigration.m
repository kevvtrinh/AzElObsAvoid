...
function testAnimationRejectsDuplicatePathTimes(testCase)
%% Section 0: Header & Readme
% Keep the public animation contract aligned with planner time assembly.
timedPath = struct( ...
    "Success", true, ...
    "time_s", [0; 1; 1], ...
    "position_deg", [0 0; 1 0; 1 0], ...
    "velocity_deg_s", zeros(3, 2));
testCase.verifyError(@() animateAzElTimedSlopePath( ...
    timedPath, [], struct("FigureVisible", "off")), ...
    "animateAzElTimedSlopePath:NonIncreasingTime");
end

function testOpposingUJerkAnimationUsesStrictTime(testCase)
%% Section 0: Header & Readme
% Reproduce the opposing-U finite-jerk animation path that exposed equal
% absolute timestamps at analytic profile events.
testCase.addTeardown(@closeTestFigures);
options = struct( ...
    "FigureVisible", "off", ...
    "ShowAnimation", true, ...
    "ShowKinematicPlot", false, ...
    "ShowSweptSurfaces", false, ...
    "AnimationFrameStride", 100000, ...
    "AnimationPause_s", 0, ...
    "Verbose", false, ...
    "UseParallel", "off", ...
...
testCase.verifyTrue(result.ExampleValidation.TimeIsStrictlyIncreasing);
testCase.verifyTrue(all(diff(result.timedSlopePath.time_s) > 0));
testCase.verifyTrue(isgraphics(result.animation.Figure));
end

function testExampleJerkToggleRejectsNonbinaryNumeric(testCase)
%% Section 0: Header & Readme
% Reject ambiguous numeric values before an example starts heavy planning.
testCase.verifyError(@() resolveAzElExampleOptions( ...
    struct("EnableJerkConstraint", 2), struct(), [2.5 2.5]), ...
    "resolveAzElExampleOptions:InvalidEnableJerkConstraint");
end
...
%% Section 0: Header & Readme
% Verify both maintained examples accept nonlinear sampled target tracks.
options = plannerTestOptions();
...
testCase.verifyFalse(earliestResult.SearchDiagnostics.EarliestCertified);
testCase.verifyTrue( ...
    earliestResult.SearchDiagnostics.EarliestNumericallyResolved);
testCase.verifyEqual( ...
    earliestResult.SearchDiagnostics.SearchMethod, ...
    "sampledPathFirstTransitionBisection");
testCase.verifyEqual(earliestResult.TargetMotion.ModelType, ...
    "sampledTrajectory");
testCase.verifyLessThanOrEqual( ...
    earliestResult.InterceptValidation.PositionError_deg, 1e-7);
testCase.verifyLessThanOrEqual( ...
    earliestResult.InterceptValidation.RequestedVelocityError_deg_s, 1e-7);
testCase.verifyFalse( ...
    earliestResult.InterceptValidation.VelocityMatchRequired);
...
testCase.verifyEqual(specifiedResult.TargetMotion.ModelType, ...
    "sampledTrajectory");
testCase.verifyFalse( ...
    specifiedResult.InterceptValidation.VelocityMatchRequired);
...
function testMovingTargetTrajectoryValidationAndLinearCompatibility(testCase)
%% Section 0: Header & Readme
% Verify sampled paths reject corrupt time bases and legacy linear input
% remains compatible with velocity-matched specified-time interception.
initialState = struct( ...
    "time_s", 0, ...
    "position_deg", [0 0], ...
    "velocity_deg_s", [0 0], ...
...
plannerOptions = plannerTestOptions();
plannerOptions.MotionType = "velocityCarrying";
interceptOptions = struct( ...
    "InterceptMode", "specifiedTime", ...
    "SpecifiedInterceptTime_s", 12, ...
...
linearResult = planAzElMovingTargetIntercept( ...
    initialState, linearTarget, limits, interceptOptions);
testCase.verifyTrue(linearResult.Success);
testCase.verifyTrue(linearResult.InterceptValidation.Passed);
testCase.verifyEqual(linearResult.TargetMotion.ModelType, ...
    "constantVelocity");
testCase.verifyEqual(linearResult.TargetVelocityAtIntercept_deg_s, ...
    linearTarget.velocity_deg_s, "AbsTol", 1e-12);

sampledVelocityMatchTarget = struct( ...
    "time_s", [0; 6; 12], ...
    "position_deg", [8 -3; 9 0; 10 0], ...
    "InterpolationMethod", "pchip");
sampledVelocityMatchResult = planAzElMovingTargetIntercept( ...
    initialState, sampledVelocityMatchTarget, limits, interceptOptions);
testCase.verifyTrue(sampledVelocityMatchResult.Success);
testCase.verifyTrue( ...
    sampledVelocityMatchResult.InterceptValidation.TargetVelocityMatched);
testCase.verifyEqual( ...
    sampledVelocityMatchResult.TargetVelocityAtIntercept_deg_s(2), 0, ...
    "AbsTol", 1e-12);
testCase.verifyGreaterThan( ...
    sampledVelocityMatchResult.TargetVelocityAtIntercept_deg_s(1), 0);

invalidTarget = struct( ...
    "time_s", [0; 2; 2], ...
    "position_deg", [1 0; 2 1; 3 0]);
testCase.verifyError(@() planAzElMovingTargetIntercept( ...
    initialState, invalidTarget, limits, interceptOptions), ...
    "planAzElMovingTargetIntercept:InvalidTargetTime");
outsideTarget = struct( ...
    "time_s", [0; 5; 10], ...
    "position_deg", [1 0; 2 1; 3 0]);
testCase.verifyError(@() planAzElMovingTargetIntercept( ...
    initialState, outsideTarget, limits, interceptOptions), ...
    "planAzElMovingTargetIntercept:TargetTimeOutsideDomain");
end
...
for motionIndex = 1:numel(motionTypes)
    for sampleIndex = 1:numel(sampleTimes_s)
        ...
        testCase.verifyFalse(diagnostics.TimeOptimizationAttempted);
        testCase.verifyFalse(diagnostics.TimeOptimizationSucceeded);
        testCase.verifyTrue(isnan(diagnostics.TimeOptimizationExitFlag));
        testCase.verifyEqual( ...
            diagnostics.TimeOptimizationMeshIntervalCount, 0);
        testCase.verifyEqual(diagnostics.TimeOptimizationReduction_s, 0);
        testCase.verifyEmpty( ...
            diagnostics.TimeOptimizationDerivativeBounds);
        testCase.verifyEqual( ...
...
options = plannerTestOptions();
options.MotionType = "velocityCarrying";
% Hold route generation fixed so this test isolates optimizer convergence;
% adaptive candidate behavior is covered by its dedicated regression.
options.PolygonCandidateMode = "extreme";
result = planAzElMotion( ...
    obstacle, initialState, goalState, limits, options);
timedPath = result.timedSlopePath;
...
testCase.verifyNotEmpty(geometryBounds);
testCase.verifyTrue(all([geometryBounds.SampledBoundsWithinCertificate]));
testCase.verifyTrue(all(vertcat(geometryBounds.NumericalTangentByAxis) <= ...
    vertcat(geometryBounds.CertifiedTangentByAxis) + 1e-10, "all"));
testCase.verifyTrue(all(vertcat( ...
    geometryBounds.NumericalSecondDerivativeByAxis_deg_inv) <= ...
    vertcat(geometryBounds.CertifiedSecondDerivativeByAxis_deg_inv) + ...
    1e-10, "all"));
testCase.verifyTrue(all(vertcat( ...
    geometryBounds.NumericalThirdDerivativeByAxis_deg_inv2) <= ...
    vertcat(geometryBounds.CertifiedThirdDerivativeByAxis_deg_inv2) + ...
    1e-10, "all"));
testCase.verifyGreaterThan( ...
...
if timedPath.ConstraintDiagnostics.TimeOptimizationAttempted
    testCase.verifyTrue( ...
        timedPath.ConstraintDiagnostics.TimeOptimizationAttempted);
    if timedPath.ConstraintDiagnostics.TimeOptimizationSucceeded
        testCase.verifyGreaterThan( ...
            timedPath.ConstraintDiagnostics.TimeOptimizationExitFlag, 0);
        testCase.verifyEqual( ...
            timedPath.ConstraintDiagnostics.ExecutedMotionProfileCount, ...
            timedPath.ConstraintDiagnostics.TimeOptimizationMeshIntervalCount);
        testCase.verifyLessThanOrEqual( ...
            timedPath.ConstraintDiagnostics.TimeOptimizationMaximumViolation, ...
            2e-6);
        testCase.verifyLessThan(timedPath.MinimumMotionDuration_s, 30);
        testCase.verifyGreaterThan( ...
            timedPath.ConstraintDiagnostics.TimeOptimizationReduction_s, 10);
        testCase.verifyEqual( ...
        ...
        testCase.verifyNumElements( ...
            timedPath.ConstraintDiagnostics.TimeOptimizationDerivativeBounds, ...
            timedPath.ConstraintDiagnostics.TimeOptimizationMeshIntervalCount);
        testCase.verifyEqual(timedPath.MinimumMotionDuration_s, ...
            timedPath.ConstraintDiagnostics.OptimizedMotionDuration_s, ...
            "AbsTol", 1e-8);
        testCase.verifyNotEmpty(regexpi( ...
            char(timedPath.RetimerType), "meshOptimized", "once"));
        profileStartAcceleration_deg_s2 = ...
            vertcat(timedPath.SegmentProfiles.PhaseStartAcceleration_deg_s2);
        profileDuration_s = ...
            vertcat(timedPath.SegmentProfiles.PhaseDuration_s);
        profileJerk_deg_s3 = ...
            vertcat(timedPath.SegmentProfiles.PhaseJerk_deg_s3);
        profileEndAcceleration_deg_s2 = ...
            profileStartAcceleration_deg_s2 + ...
            profileJerk_deg_s3 .* profileDuration_s;
        testCase.verifyEqual(profileEndAcceleration_deg_s2(1:end - 1), ...
            profileStartAcceleration_deg_s2(2:end), "AbsTol", 1e-8);
        profileStartSpeed_deg_s = ...
            vertcat(timedPath.SegmentProfiles.StartSpeed_deg_s);
        profileEndSpeed_deg_s = ...
            vertcat(timedPath.SegmentProfiles.EndSpeed_deg_s);
        integratedEndSpeed_deg_s = profileStartSpeed_deg_s + ...
            profileStartAcceleration_deg_s2 .* profileDuration_s + ...
            0.5 * profileJerk_deg_s3 .* profileDuration_s.^2;
        testCase.verifyLessThanOrEqual(abs( ...
            integratedEndSpeed_deg_s - profileEndSpeed_deg_s), ...
            3e-7 * max(1, max(limits.maxVelocity_deg_s)));
        integratedLength_deg = profileStartSpeed_deg_s .* ...
            profileDuration_s + 0.5 * profileStartAcceleration_deg_s2 .* ...
            profileDuration_s.^2 + profileJerk_deg_s3 .* ...
            profileDuration_s.^3 / 6;
        profileLength_deg = ...
            vertcat(timedPath.SegmentProfiles.Length_deg);
        testCase.verifyLessThanOrEqual(abs( ...
            integratedLength_deg - profileLength_deg), ...
            3e-7 * max(1, profileLength_deg));
        testCase.verifyEqual( ...
            profileEndSpeed_deg_s(1:end - 1), ...
            profileStartSpeed_deg_s(2:end), ...
            "AbsTol", 1e-10);
        testCase.verifyEqual([profileStartAcceleration_deg_s2(1); ...
            profileEndAcceleration_deg_s2(end)], [0; 0], ...
            "AbsTol", 1e-8);
        profileStartS_deg = ...
            vertcat(timedPath.SegmentProfiles.StartArcLength_deg);
        profileEndS_deg = ...
            vertcat(timedPath.SegmentProfiles.EndArcLength_deg);
        testCase.verifyEqual(profileEndS_deg(1:end - 1), ...
            profileStartS_deg(2:end), "AbsTol", 1e-10);
        testCase.verifyEqual(sum(profileDuration_s), ...
            timedPath.MinimumMotionDuration_s, "AbsTol", 1e-10);
        testCase.verifyTrue(all(isfinite( ...
            [timedPath.SegmentProfiles.PeakSpeed_deg_s])));
        testCase.verifyTrue(all(isnan( ...
            [timedPath.SegmentProfiles.MaxSpeed_deg_s])));
        testCase.verifyTrue(all(isnan( ...
            [timedPath.SegmentProfiles.MaxAcceleration_deg_s2])));
        testCase.verifyTrue(all(isnan( ...
            [timedPath.SegmentProfiles.MaxJerk_deg_s3])));
        directProfiles = result.candidateTimedPaths{1}.SegmentProfiles;
        testCase.verifyNotEmpty(directProfiles);
        testCase.verifyEqual(fieldnames(timedPath.SegmentProfiles), ...
            fieldnames(directProfiles));
        sampledVelocityPeak_deg_s = max(abs( ...
            timedPath.velocity_deg_s), [], 1);
        sampledAccelerationPeak_deg_s2 = max(abs( ...
            timedPath.acceleration_deg_s2), [], 1);
        sampledJerkPeak_deg_s3 = max(abs(timedPath.jerk_deg_s3), [], 1);
        profileVelocityBound_deg_s = max(vertcat( ...
            timedPath.SegmentProfiles.PeakVelocityByAxis_deg_s), [], 1);
        profileAccelerationBound_deg_s2 = max(vertcat( ...
            timedPath.SegmentProfiles.PeakAccelerationByAxis_deg_s2), [], 1);
        profileJerkBound_deg_s3 = max(vertcat( ...
            timedPath.SegmentProfiles.PeakJerkByAxis_deg_s3), [], 1);
        testCase.verifyLessThanOrEqual(sampledVelocityPeak_deg_s, ...
            profileVelocityBound_deg_s + 1e-10);
        testCase.verifyLessThanOrEqual(sampledAccelerationPeak_deg_s2, ...
            profileAccelerationBound_deg_s2 + 1e-10);
        testCase.verifyLessThanOrEqual(sampledJerkPeak_deg_s3, ...
            profileJerkBound_deg_s3 + 1e-10);
        testCase.verifyLessThanOrEqual(profileVelocityBound_deg_s, ...
            limits.maxVelocity_deg_s + 1e-8);
        testCase.verifyLessThanOrEqual(profileAccelerationBound_deg_s2, ...
            limits.maxAcceleration_deg_s2 + 1e-8);
        testCase.verifyLessThanOrEqual(profileJerkBound_deg_s3, ...
        ...
        testCase.verifyFail( ...
            "The deterministic rounded-path time optimization did not " + ...
            "converge in an environment with Optimization Toolbox.");
        testCase.verifyLessThan(timedPath.MinimumMotionDuration_s, 60);
        testCase.verifyNotEmpty(regexpi( ...
            char(timedPath.ConstraintDiagnostics.TimeOptimizationMessage), ...
            "fallback", "once"));
    end
else
    testCase.verifyFalse( ...
...
function testQuinticCertificateDominatesTightRadiusOffGridPeaks(testCase)
%% Section 0: Header & Readme
% Verify the production Bernstein certificate encloses dense off-grid
% derivatives for tight supported turns that defeated sampled envelopes.
turnAngles_deg = [30 60 90 120 150];
radius_deg = 0.02;
parameter = linspace(0, 1, 20001).';
incomingDirection = [1 0];
for angleIndex = 1:numel(turnAngles_deg)
    turnAngle_deg = turnAngles_deg(angleIndex);
    outgoingDirection = [cosd(turnAngle_deg), sind(turnAngle_deg)];
    tangentScale = (384 / 125) * sind(turnAngle_deg / 2) / ...
        cosd(turnAngle_deg / 2)^2;
    tangentDistance_deg = radius_deg * tangentScale;
    controlPoints_deg = [ ...
        -tangentDistance_deg * incomingDirection; ...
        -0.5 * tangentDistance_deg * incomingDirection; ...
        0 0; 0 0; ...
        0.5 * tangentDistance_deg * outgoingDirection; ...
        tangentDistance_deg * outgoingDirection];
    certificate = azElInternal.certifyQuinticArcDerivatives( ...
        controlPoints_deg, [0 1]);
    firstParameterDerivative_deg = evaluateBernsteinPolynomial( ...
        5 * diff(controlPoints_deg, 1, 1), parameter);
    secondParameterDerivative_deg = evaluateBernsteinPolynomial( ...
        20 * diff(controlPoints_deg, 2, 1), parameter);
    thirdParameterDerivative_deg = evaluateBernsteinPolynomial( ...
        60 * diff(controlPoints_deg, 3, 1), parameter);
    speed_deg = vecnorm(firstParameterDerivative_deg, 2, 2);
    speedSquared_deg2 = speed_deg.^2;
    firstSecondDot_deg2 = sum(firstParameterDerivative_deg .* ...
        secondParameterDerivative_deg, 2);
    secondNormSquared_deg2 = sum( ...
        secondParameterDerivative_deg.^2, 2);
    firstThirdDot_deg2 = sum(firstParameterDerivative_deg .* ...
        thirdParameterDerivative_deg, 2);
    tangent = firstParameterDerivative_deg ./ speed_deg;
    secondDerivative_deg_inv = secondParameterDerivative_deg ./ ...
        speedSquared_deg2 - firstParameterDerivative_deg .* ...
        firstSecondDot_deg2 ./ speed_deg.^4;
    thirdDerivative_deg_inv2 = thirdParameterDerivative_deg ./ ...
        speed_deg.^3 - 3 * secondParameterDerivative_deg .* ...
        firstSecondDot_deg2 ./ speed_deg.^5 - ...
        firstParameterDerivative_deg .* (secondNormSquared_deg2 + ...
        firstThirdDot_deg2) ./ speed_deg.^5 + ...
        4 * firstParameterDerivative_deg .* firstSecondDot_deg2.^2 ./ ...
        speed_deg.^7;
    observedBounds = [ ...
        max(abs(tangent), [], 1); ...
        max(abs(secondDerivative_deg_inv), [], 1); ...
        max(abs(thirdDerivative_deg_inv2), [], 1)];
    certifiedBounds = [ ...
        certificate.TangentByAxis; ...
        certificate.SecondDerivativeByAxis_deg_inv; ...
        certificate.ThirdDerivativeByAxis_deg_inv2];
    allowance = 1e-10 * max(1, certifiedBounds);
    testCase.verifyLessThanOrEqual( ...
        observedBounds, certifiedBounds + allowance, ...
        sprintf("Certificate missed a dense peak at %g degrees.", ...
        turnAngle_deg));
    testCase.verifyGreaterThan( ...
        certificate.ParameterSpeedLowerBound_deg, 0);
    testCase.verifyGreaterThan(certificate.SubdivisionCount, 0);
end
end
...
function testAdaptiveCandidatesImproveTwoOpposingURoute(testCase)
%% Section 0: Header & Readme
% Keep moderate polygon corners available so the spatial jerk retimer can
% use a shorter route without disabling bounded dense-region reduction.
options = struct( ...
    "FigureVisible", "off", ...
    "ShowAnimation", false, ...
    "ShowKinematicPlot", false, ...
    ...
    "EnableJerkConstraint", true);
result = exampleTwoOpposingUVisibilityGraph(options);
smoothLength_deg = result.timedSlopePath.CurveArcLength_deg(end);
% Same headless jerk-enabled request before this change at commit a3c4c67.
referenceLength_deg = 31.847132061;
referenceDuration_s = 67.912173473;
reductionMode = string( ...
    {result.spaceView.CandidateReductionDiagnostics.Mode});

testCase.verifyTrue(result.Success);
testCase.verifyTrue(result.ExampleValidation.Passed);
testCase.verifyEqual(result.Options.PolygonCandidateMode, "adaptive");
testCase.verifyTrue(any(reductionMode == "adaptiveAllCorners"));
testCase.verifyLessThan(smoothLength_deg, referenceLength_deg);
testCase.verifyLessThan( ...
    result.timedSlopePath.MinimumMotionDuration_s, referenceDuration_s);
testCase.verifyTrue(result.Validation.ProtectedGeometryClear);
testCase.verifyTrue(result.ExampleValidation.CollisionFree);
testCase.verifyTrue(result.Validation.JerkWithinLimits);
end
...
testCase.verifyEqual(spaceView.Options.PolygonCandidateMode, "adaptive");
testCase.verifyEqual(diagnostic.Mode, "adaptiveExtreme");
testCase.verifyLessThanOrEqual( ...
...
testCase.verifyTrue(jerkOnResult.Success, jerkOnResult.Message);
testCase.verifyTrue(jerkOnResult.ExampleValidation.Passed, ...
    jerkOnResult.ExampleValidation.Message);
testCase.verifyTrue(jerkOffResult.Success, jerkOffResult.Message);
testCase.verifyTrue(jerkOffResult.ExampleValidation.Passed, ...
    jerkOffResult.ExampleValidation.Message);
testCase.verifyTrue(all(diff( ...
    jerkOnResult.timedSlopePath.time_s) > 0));
testCase.verifyTrue(all(diff( ...
    jerkOffResult.timedSlopePath.time_s) > 0));
...
function testMovingExampleJerkToggleKeepsPhysicalInputs(testCase)
%% Section 0: Header & Readme
% The moving-circle toggle must change only jerk, never its obstacle rate,
% time history, state request, non-jerk limits, or planner options.
testCase.addTeardown(@closeTestFigures);
baseOptions = struct( ...
    "FigureVisible", "off", ...
    "ShowAnimation", false, ...
    ...
    "Verbose", false, ...
    "UseParallel", "off");
noJerkOptions = baseOptions;
noJerkOptions.EnableJerkConstraint = false;
noJerkResult = exampleMovingCircleNoAzimuthWrap(noJerkOptions);
jerkOptions = baseOptions;
jerkOptions.EnableJerkConstraint = true;
jerkResult = exampleMovingCircleNoAzimuthWrap(jerkOptions);

testCase.verifyTrue(noJerkResult.Success, noJerkResult.Message);
testCase.verifyTrue(jerkResult.Success, jerkResult.Message);
testCase.verifyEqual(noJerkResult.azElData, jerkResult.azElData);
testCase.verifyEqual(noJerkResult.initialState, jerkResult.initialState);
testCase.verifyEqual(noJerkResult.goalState, jerkResult.goalState);
testCase.verifyEqual(noJerkResult.Options, jerkResult.Options);
noJerkLimits = rmfield(noJerkResult.limits, "maxJerk_deg_s3");
jerkLimits = rmfield(jerkResult.limits, "maxJerk_deg_s3");
testCase.verifyEqual(noJerkLimits, jerkLimits);
testCase.verifyTrue(all(isinf( ...
    noJerkResult.limits.maxJerk_deg_s3)));
testCase.verifyEqual(jerkResult.limits.maxJerk_deg_s3, [2.5 2.5]);
end
...
duplicateTimeResult = result;
duplicateTimeResult.timedSlopePath.time_s(2) = ...
    duplicateTimeResult.timedSlopePath.time_s(1);
duplicateTimeValidation = validateAzElExampleResult( ...
    duplicateTimeResult, "duplicate-time direct regression");
testCase.verifyFalse(duplicateTimeValidation.Passed);
testCase.verifyTrue(duplicateTimeValidation.TimeIsNondecreasing);
testCase.verifyFalse(duplicateTimeValidation.TimeIsStrictlyIncreasing);
testCase.verifyTrue(any(duplicateTimeValidation.Issues == ...
    "timeIsStrictlyIncreasing"));
end
...
function values = evaluateBernsteinPolynomial(control, parameter)
%% Section 0: Header & Readme
% SYNTAX
%   values = evaluateBernsteinPolynomial(control, parameter)
%**************************************************************************
% PURPOSE
%   - Independently evaluate vector Bernstein controls for certificate tests.
%**************************************************************************
% INPUTS
%   - control (N-by-D finite numeric matrix)
%       Bernstein controls of degree N-1.
%   - parameter (M-by-1 numeric column)
%       Evaluation parameters in [0, 1].
%**************************************************************************
% OUTPUTS
%   - values (M-by-D numeric matrix)
%       Polynomial values at each parameter.
%**************************************************************************
% UNITS
%   - Parameters are dimensionless; values retain the control units.
%**************************************************************************
degree = size(control, 1) - 1;
values = zeros(numel(parameter), size(control, 2));
for controlIndex = 0:degree
    basis = nchoosek(degree, controlIndex) .* ...
        parameter.^controlIndex .* ...
        (1 - parameter).^(degree - controlIndex);
    values = values + basis .* control(controlIndex + 1, :);
end
end
