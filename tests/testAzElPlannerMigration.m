...
for motionIndex = 1:numel(motionTypes)
    ...
    result = planAzElMotion( ...
        obstacle, initialState, goalState, limits, options);
    testCase.verifyFalse(result.Success);
    testCase.verifyFalse(result.diagnosticTimedPathAvailable);
    testCase.verifyNotEmpty(result.bestAttemptPosition_deg);
    testCase.verifyEqual(result.ProgressLog(end).Event, "PlanningFailed");
    closeTestFigures();
end
end
...
%% Section 0: Header & Readme
% Verify the maintained spatial retimer has sample-invariant direct timing.
...
testCase.verifyEqual(arrivalTimes_s, ...
    7.5 * ones(numel(motionTypes), numel(sampleTimes_s)), ...
    "AbsTol", 1e-10);
end
...
options = plannerTestOptions();
options.MotionType = "velocityCarrying";
% Hold route generation fixed; adaptive candidates have a separate test.
...
testCase.verifyGreaterThan( ...
    ...
    numel(result.smoothPath.Primitives) - 1);
...
%% Section 0: Header & Readme
% Return a short unobstructed spatial-retiming request.
