...
testCase.verifyTrue( ...
    timedPath.ConstraintDiagnostics.SpatiallyVaryingLimits);
testCase.verifyGreaterThan( ...
    timedPath.ConstraintDiagnostics.SpatialRetimingCellCount, ...
    numel(result.smoothPath.Primitives));
testCase.verifyEqual( ...
    timedPath.ConstraintDiagnostics.ExecutedMotionProfileCount, ...
    numel(result.smoothPath.Primitives));
testCase.verifyLessThan( ...
    timedPath.ConstraintDiagnostics.ExecutedMotionProfileCount, ...
    timedPath.ConstraintDiagnostics.SpatialRetimingCellCount);
testCase.verifyGreaterThan(max(timedPath.SampleSpeed_deg_s), 1.5);
testCase.verifyLessThan( ...
    min([timedPath.SegmentProfiles.MaxSpeed_deg_s]), 0.75);
testCase.verifyLessThan(timedPath.MinimumMotionDuration_s, 60);
testCase.verifyTrue(result.Validation.JerkWithinLimits);
...
testCase.verifyLessThanOrEqual(max(abs( ...
    timedPath.acceleration_deg_s2), [], 1), ...
    limits.maxAcceleration_deg_s2 + 1e-8);
end
