...
initial = struct('time_s', 0, 'position_deg', [0 0]);
goal = struct('time_s', 10, 'position_deg', [1 0]);
...
obstacle = obstacleAvoidance.obstacles.createObstacle( ...
    'blocked start', [0;10], [-1;1;1;-1], [-1;-1;1;1], 0);
[failure, failureDiagnosis] = obstacleAvoidance.planTrajectory( ...
    obstacle, initial, goal, limits);
...
verifyFalse(testCase, any(isfield(r, {'Seeds','SeedSummaries', ...
    'SearchDiagnostics','SelectedSeedIndex','GoalHorizon_s'})));
end

function testStableSuccessAndFailureSchemas(testCase)
verifyEqual(testCase, fieldnames(testCase.TestData.Result), ...
    fieldnames(testCase.TestData.Failure));
verifyEqual(testCase, fieldnames(testCase.TestData.Diagnosis), ...
    fieldnames(testCase.TestData.FailureDiagnosis));
...
options = struct('FigureVisible','off','ShowKinematics',false, ...
    'ShowAnimation',false);
...
handles = obstacleAvoidance.plotting.plotTrajectory( ...
    testCase.TestData.Failure, options, testCase.TestData.FailureDiagnosis);
verifyTrue(testCase, isgraphics(handles.WorkspaceAxes));
end
