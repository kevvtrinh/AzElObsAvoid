...
function testUsableConicIteratePolicy(testCase)
    verifyTrue(testCase,bmtpEngine.optimization.hasUsableConicIterate(0,1));
    verifyTrue(testCase,bmtpEngine.optimization.hasUsableConicIterate(0,-7));
    verifyFalse(testCase,bmtpEngine.optimization.hasUsableConicIterate([],1));
    verifyFalse(testCase,bmtpEngine.optimization.hasUsableConicIterate(NaN,1));
    verifyFalse(testCase,bmtpEngine.optimization.hasUsableConicIterate(0,0));
end
...
function testDifferentTimeWindowsDoNotConflict(testCase)
    ...
    [controls,durations,flag,output] = bmtpEngine.optimization.solveTrajectoryStep(1,5,initial,goal,limits,planes,1e-8,6,options,1,true);
    ...
    first = bmtpEngine.motion.restrictBezier(squeeze(controls),[0,0.1]);
    last = bmtpEngine.motion.restrictBezier(squeeze(controls),[0.9,1]);
    ...
    [~,~,flag,output] = bmtpEngine.optimization.solveTrajectoryStep(1,5,initial,goal,limits,planes,1e-8,6,options,1,true);
...
function testTravelRefinementAddsNewCollisionPlanes(testCase)
    ...
    request = bmtpEngine.pipeline.createSolveRequest(seed, {box_units}, struct('Passed', true), ...
...
function testSingleSpanTimedInfeasibilityReturnsNoMotion(testCase)
    ...
    [controls,duration_s,flag] = bmtpEngine.optimization.solveTimedTrajectoryStep( ...
...
function testSingleSpanTimedMotionIsIndependentlyValid(testCase)
    ...
    for degree = [5,8]
        for horizon_s = [5,6,8,10]
            [controls,duration_s,flag,output] = bmtpEngine.optimization.solveTimedTrajectoryStep( ...
...
function testSolveRequestCarriesMinimumArrivalBound(testCase)
    ...
    request=bmtpEngine.pipeline.createSolveRequest(seed,cell(0,1),coverage, ...
        normalized.Inputs.initialState,normalized.Inputs.goalState, ...
        normalized.Limits,normalized.Options);
...
function testTimedProfileDoesNotDependOnSeedSource(testCase)
    ...
    firstRequest=bmtpEngine.pipeline.createSolveRequest(firstSeed,cell(0,1),coverage, ...
        normalized.Inputs.initialState,normalized.Inputs.goalState, ...
        normalized.Limits,normalized.Options);
    secondRequest=bmtpEngine.pipeline.createSolveRequest(secondSeed,cell(0,1),coverage, ...
        normalized.Inputs.initialState,normalized.Inputs.goalState, ...
        normalized.Limits,normalized.Options);
    firstWarm=bmtpEngine.pipeline.createWarmStart(firstRequest);
    secondWarm=bmtpEngine.pipeline.createWarmStart(secondRequest);
...
function testVariableClockUsesActualMovingCellOverlap(testCase)
    ...
    [planes,activePairs,complete,statistics]= ...
        bmtpEngine.separation.createTimeScopedPlanes(controls,[3;3],request,1e-6,1e-8);
...
function testUnequalSpanClockWithFullEndpointStates(testCase)
    ...
    [controls,durations_s,flag] = bmtpEngine.optimization.solveTrajectoryStep( ...
    ...
    endpointControls=bmtpEngine.motion.imposeEndpointControls(controls,durations_s,initial,goal);
    ...
    polynomial = bmtpEngine.motion.createPowerPolynomial(controls,durations_s,0);
    [~,position,velocity,acceleration] = bmtpEngine.motion.evaluatePolynomial(polynomial,[0;8]);
    ...
    [~,~,~,~,leftJerk] = bmtpEngine.motion.evaluatePolynomial(polynomial,[2;6],[1;2]);
    [~,~,~,~,rightJerk] = bmtpEngine.motion.evaluatePolynomial(polynomial,[2;6],[2;3]);
...
function testConstraintGenerationWithTransformedFixedClock(testCase)
    ...
    [controls,durations_s,flag,output]=bmtpEngine.optimization.solveTrajectoryStep( ...
    ...
    fourth=bmtpEngine.motion.restrictBezier(squeeze(controls(4,:,:)),[0.2,0.8]);
...
function testConstraintGenerationClosesSeveralPairsInOneSpan(testCase)
    ...
    [controlPoint_units,~,exitFlag,output]= ...
        bmtpEngine.optimization.solveTrajectoryStep(1,8,initial,goal,limits,planes, ...
        0,20,options,1,true);
...
function [request, warmStart, diagnostics, target_units, reserve_units] = ...
    ...
    request = bmtpEngine.pipeline.createSolveRequest(seed, {blockingRegion_units}, coverage, ...
        normalized.Inputs.initialState, normalized.Inputs.goalState, ...
        normalized.Limits, normalized.Options);
    warmStart = bmtpEngine.pipeline.createWarmStart(request);
    diagnostics = struct( ...
...
function [request, warmStart, diagnostics, target_units, reserve_units] = ...
    ...
    request = bmtpEngine.pipeline.createSolveRequest(seed, {firstRegion_units}, coverage, ...
        normalized.Inputs.initialState, normalized.Inputs.goalState, ...
        normalized.Limits, normalized.Options);
    warmStart = bmtpEngine.pipeline.createWarmStart(request);
    diagnostics = struct( ...
