...
function testDifferentTimeWindowsDoNotConflict(testCase)
    ...
    request = stepRequest(5,initial,goal,limits,options);
    [controls,durations,flag,output,constraintBase] = ...
        bmtpEngine.optimization.solveTrajectoryStep(request, ...
        trajectoryStep(1,planes,1e-8,6));
    ...
    [~,~,flag,output] = bmtpEngine.optimization.solveTrajectoryStep(request, ...
        trajectoryStep(1,planes,1e-8,6));
    ...
    reducedStep = trajectoryStep(1,planes(1),1e-8,6);
    reducedStep.ConstraintBase = constraintBase;
    [~,~,~,~,reducedConstraintBase] = ...
        bmtpEngine.optimization.solveTrajectoryStep(request,reducedStep);
    verifyNotEqual(testCase,reducedConstraintBase.Key.VariableCount, ...
        constraintBase.Key.VariableCount);
    verifyEqual(testCase,size(reducedConstraintBase.A,2), ...
        reducedConstraintBase.Key.VariableCount);
end
...
function testTravelRefinementAddsNewCollisionPlanes(testCase)
    ...
    request = bmtpEngine.prepareRequest(seed, ...
        struct('regions_units', {{box_units}}, ...
    ...
    alternating = bmtpEngine.optimization.createOptimizationResult( ...
        "A complete time-scoped feasible iterate was retained.", "", "", false, ...
        controls_units, [4; 4; 4], repmat(plane, 3, 1), false(3, 1));
    ...
    [refined, diagnostics] = bmtpEngine.optimization.refineTravel( ...
    ...
    prepared = struct('ControlPoint_units', refined.ControlPoint_units, ...
...
function testSingleSpanTimedInfeasibilityReturnsNoMotion(testCase)
    ...
    request = stepRequest(8,struct('position_units',[-1,0]),struct('position_units',[1,0.5]),limits,options);
    [controls,duration_s,flag] = bmtpEngine.optimization.solveTrajectoryStep( ...
        request,timedStep(1,planes,1e-8,0.1));
...
function testSingleSpanTimedMotionIsIndependentlyValid(testCase)
    ...
    for degree = [5,8]
        for horizon_s = [5,6,8,10]
            request = stepRequest(degree,struct('position_units',[-1,0]),struct('position_units',[1,0.5]),limits,options);
            [controls,duration_s,flag,output] = bmtpEngine.optimization.solveTrajectoryStep( ...
                request,timedStep(1,planes,1e-8,horizon_s));
            ...
            prepared = bmtpEngine.motion.createMotion(request,controls,duration_s);
            ...
            motionOutput = bmtpEngine.createMotionOutput(struct(),request,prepared);
            ...
            result.Diagnostics.SeparationProof = bmtpEngine.validation.checkFinalMotion( ...
                request,prepared,roundoffReserve_units,target_units);
...
function testSolveRequestCarriesMinimumArrivalBound(testCase)
    ...
    seed=struct('position_units',[-1,0;1,0.5],'tau',[0;1], ...
        'UsesVariableClock',true);
    coverage=struct('Passed',true,'ExactRegionCount',0,'MinimumMotionDuration_s',4, ...
...
function testTimedTrajectoryStepRetainsSuppliedClock(testCase)
    ...
    request=stepRequest(5,struct('position_units',[-1,0]),struct('position_units',[1,0]),limits,options);
    [controls,duration_s,flag]=bmtpEngine.optimization.solveTrajectoryStep( ...
        request,timedStep(2,planes,1e-8,[2;6]));
    verifyTrue(testCase,flag>0 || flag==-7);
...
function testUnequalSpanClockWithFullEndpointStates(testCase)
    ...
    [controls,durations_s,flag] = bmtpEngine.optimization.solveTrajectoryStep( ...
        stepRequest(5,initial,goal,limits,options),trajectoryStep(3,planes,1e-8,[2;4;2]));
...
function testConstraintGenerationWithTransformedFixedClock(testCase)
    ...
    [controls,durations_s,flag,output]=bmtpEngine.optimization.solveTrajectoryStep( ...
        stepRequest(5,initial,goal,limits,options),trajectoryStep(9,planes,1e-8,2 * ones(9,1)));
    assertTrue(testCase,flag>0 || flag==-7);
...
function testVariationPassDoesNotConsumeFeasibilityBudget(testCase)
    % One allowed corridor iteration must still be followed by the mandatory
    % full integrated-snap pass on the selected fixed clock.
    options = planner();
    ...
    region_units = [-0.5,-1;0.5,-1;0.5,1;-0.5,1];
    seed = struct( ...
        'position_units',[-4,0;0,2;4,0], ...
        'tau',[0;0.5;1], ...
        'UsesTimeScopedSolver',true, ...
        'MaximumAlternatingIterations',1);
    coverage = struct( ...
        'Passed',true, ...
        'ExactRegionCount',1, ...
    ...
    [candidate,diagnostics] = bmtpEngine.solve(seed, ...
        struct('regions_units', {{region_units}}, 'coverage', coverage), ...
        struct('initialState', initial, ...
        'goalState', goal, ...
        'limits', limits, ...
    ...
    verifyEqual(testCase,diagnostics.IterationCount,2);
    verifyTrue(testCase,diagnostics.IntrinsicJerkVariation);
end
...
function testConstraintGenerationClosesSeveralPairsInOneSpan(testCase)
    ...
    [controlPoint_units,~,exitFlag,output]= ...
        ...
        stepRequest(8,initial,goal,limits,options),trajectoryStep(1,planes,0,20));
...
function testSavedMovingDetourEarliestArrival(testCase)
    ...
    assertTrue(testCase,result.Success,result.Message);
    verifyTrue(testCase,obstacleAvoidance.validateTrajectory(result).Passed);
    verifyEqual(testCase,result.Diagnostics.GoalArrivalWindow_s(2),180,'AbsTol',1e-8);
    verifyGreaterThanOrEqual(testCase,result.ArrivalTime_s, ...
        result.Diagnostics.GoalArrivalWindow_s(1)-1e-8);
    verifyLessThanOrEqual(testCase,result.ArrivalTime_s, ...
        result.Diagnostics.GoalArrivalWindow_s(2)+1e-8);
    verifyEqual(testCase,result.Diagnostics.VisibilityGraph.SearchKind,"timeExpandedVisibilityGraph");
    verifyEqual(testCase,result.Diagnostics.SolverDiagnostics.TaggedPairCount, ...
        result.Diagnostics.SolverDiagnostics.ApplicablePairCount);
    verifyEqual(testCase,result.Diagnostics.SolverDiagnostics.FinalCollisionPairCount,0);
end
...
function [request, warmStart, diagnostics, target_units, roundoffReserve_units] = ...
    ...
    coverage = struct( ...
        ...
        'SeedMotionDuration_s',    6, ...
        'ActiveTimeInterval_s',    [0, 10], ...
        'EndRegions_units',        {{blockingRegion_units}});
    request = bmtpEngine.prepareRequest(seed, ...
        struct('regions_units', {{blockingRegion_units}}, 'coverage', coverage), ...
        struct('initialState', normalized.Inputs.initialState, ...
        'goalState', normalized.Inputs.goalState, ...
        'limits', normalized.Diagnostics.Limits, ...
        'options', normalized.Options));
...
function request = stepRequest(degree, initialState, goalState, limits, options)
    % The part of an engine solve request that a trajectory step reads.
    request = struct( ...
        'Degree',                 degree, ...
        ...
        'TimedTrajectoryOptions', options);
end

function step = trajectoryStep(segmentCount, planes, roundoffReserve_units, segmentTime_s)
    % One fixed physical clock, supplied directly rather than rebuilt from ratios.
    step = struct( ...
...
function step = timedStep(segmentCount, planes, roundoffReserve_units, segmentTime_s)
    % One fixed scaled clock, supplied directly with no second duration source.
    step = struct( ...
