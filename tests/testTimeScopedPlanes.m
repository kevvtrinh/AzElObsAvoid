...
function testDifferentTimeWindowsDoNotConflict(testCase)
    ...
    [controls,durations,flag,output,constraintBase] = ...
        ...
        trajectoryStep(1,planes,1e-8,6,1,true));
    ...
    reducedStep = trajectoryStep(1,planes(1),1e-8,6,1,true);
...
function testSingleSpanTimedInfeasibilityReturnsNoMotion(testCase)
    ...
    [controls,duration_s,flag] = bmtpEngine.optimization.solveTimedTrajectoryStep( ...
        request,timedStep(1,planes,1e-8,0.1,"fixedArrival",0,[]));
    ...
    for degree = [5,8]
        for horizon_s = [5,6,8,10]
            ...
            [controls,duration_s,flag,output] = bmtpEngine.optimization.solveTimedTrajectoryStep( ...
                request,timedStep(1,planes,1e-8,horizon_s,"fixedArrival",0,[]));
...
function testTimedTrajectoryStepRetainsRelativeClock(testCase)
    ...
    [controls,duration_s,flag]=bmtpEngine.optimization.solveTimedTrajectoryStep( ...
        request,timedStep(2,planes,1e-8,8,"fixedArrival",0,[1;3]));
    verifyTrue(testCase,flag>0 || flag==-7);
...
function testUnequalSpanClockWithFullEndpointStates(testCase)
    ...
    [controls,durations_s,flag] = bmtpEngine.optimization.solveTrajectoryStep( ...
        stepRequest(5,initial,goal,limits,options),trajectoryStep(3,planes,1e-8,8,[1,2,1],true));
...
function testConstraintGenerationWithTransformedFixedClock(testCase)
    ...
    [controls,durations_s,flag,output]=bmtpEngine.optimization.solveTrajectoryStep( ...
        stepRequest(5,initial,goal,limits,options),trajectoryStep(9,planes,1e-8,18,ones(9,1),true));
    assertTrue(testCase,flag>0 || flag==-7);
...
function testConstraintGenerationClosesSeveralPairsInOneSpan(testCase)
    ...
    [controlPoint_units,~,exitFlag,output]= ...
        ...
        stepRequest(8,initial,goal,limits,options),trajectoryStep(1,planes,0,20,1,true));
...
function step = trajectoryStep(segmentCount, planes, roundoffReserve_units, ...
        maximumMotionDuration_s, segmentRatio, fixedClock)
    % One fixed- or variable-clock step with the tie-break on and no reused base.
    step = struct( ...
        'SegmentCount',              segmentCount, ...
        'Planes',                    planes, ...
        'RoundoffReserve_units',     roundoffReserve_units, ...
        'MaximumMotionDuration_s',   maximumMotionDuration_s, ...
...
function step = timedStep(segmentCount, planes, roundoffReserve_units, ...
        maximumMotionDuration_s, goalTimeMode, minimumMotionDuration_s, segmentRatio)
    % One timed step with no reused base.
    step = struct( ...
        'SegmentCount',            segmentCount, ...
        'Planes',                  planes, ...
        'RoundoffReserve_units',   roundoffReserve_units, ...
