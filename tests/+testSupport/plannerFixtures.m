...
function limits = physicalLimits(velocity_deg_s, acceleration_deg_s2, jerk_deg_s3)
    ...
    limits = struct("maxVelocity_deg_s", velocity_deg_s, ...
...
function trajectory = constantJerkTrajectory(duration_s)
    ...
    jerk_deg_s3         = [1 0; 1 0];
    trajectory          = trajectoryRecord(time_s, position_deg, velocity_deg_s, acceleration_deg_s2, jerk_deg_s3, positionPower_deg, velocityPower_deg_s, accelerationPower_deg_s2, jerkPower_deg_s3, duration_s);
end
...
function trajectory = linearTrajectory(initialState, goalState)
    ...
    accelerationPower_deg_s2 = zeros(1, 2, 4);
    jerkPower_deg_s3         = zeros(1, 2, 3);
    trajectory               = trajectoryRecord([initialState.time_s; goalState.time_s], [initialState.position_deg; goalState.position_deg], [initialState.velocity_deg_s; goalState.velocity_deg_s], zeros(2, 2), zeros(2, 2), positionPower_deg, velocityPower_deg_s, accelerationPower_deg_s2, jerkPower_deg_s3, duration_s);
end
...
function trajectory = interiorVelocityPeakTrajectory()
    ...
    trajectory = trajectoryRecord([0; 1], [0 0; 2 / 3 0], zeros(2, 2), [4 0; -4 0], [-8 0; -8 0], positionPower_deg, velocityPower_deg_s, accelerationPower_deg_s2, jerkPower_deg_s3, duration_s);
end

function trajectory = trajectoryRecord(time_s, position_deg, velocity_deg_s, acceleration_deg_s2, jerk_deg_s3, positionPower_deg, velocityPower_deg_s, accelerationPower_deg_s2, jerkPower_deg_s3, duration_s)
