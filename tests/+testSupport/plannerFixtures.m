function fixtures = plannerFixtures()
%% Section 0: Header & Readme
% SYNTAX
%   fixtures = testSupport.plannerFixtures()
%**************************************************************************
% PURPOSE
%   - Provide one shared set of deterministic planner-test fixture builders.
%**************************************************************************
% INPUTS
%   - None.
%**************************************************************************
% OUTPUTS
%   - fixtures (scalar struct of function handles)
%       Builders for states, limits, obstacles, and trajectory records.
%**************************************************************************
% UNITS
%   - Fixture fields retain their documented degree and second suffixes.
%**************************************************************************

%% Section 1: Build Fixture Interface

% Expose small deterministic constructors to all planner tests. Keep units in
% field names. A fixture change affects many tests, so change it only when the
% shared meaning of the test input changes.

fixtures = struct("State", @state, ...
    "PhysicalLimits", @physicalLimits, ...
    "RectangleObstacle", @rectangleObstacle, ...
    "ConstantJerkTrajectory", @constantJerkTrajectory, ...
    "LinearTrajectory", @linearTrajectory, ...
    "InteriorVelocityPeakTrajectory", @interiorVelocityPeakTrajectory, ...
    "TrajectoryRecord", @trajectoryRecord);
end

%% Section 2: Local Functions

function value = state(time_s, position_units, velocity_units_s, acceleration_units_s2)
    % Construct one complete endpoint state.
    value = struct("time_s", time_s, "position_units", position_units, ...
        "velocity_units_s", velocity_units_s, "acceleration_units_s2", acceleration_units_s2);
end

function limits = physicalLimits(velocity_units_s, acceleration_units_s2, jerk_units_s3)
    % Construct one complete physical-limit record.
    limits = struct("maxVelocity_units_s", velocity_units_s, ...
        "maxAcceleration_units_s2", acceleration_units_s2, ...
        "maxJerk_units_s3", jerk_units_s3, ...
        "xInterval_units", [-180 180], ...
        "yInterval_units", [-90 90]);
end

function obstacle = rectangleObstacle(time_s, bounds_units, margin_units)
    % Construct a static rectangle from [minX maxX minY maxY].
    x_units   = bounds_units([1 2 2 1]).';
    y_units = bounds_units([3 3 4 4]).';
    obstacle      = obstacleAvoidance.obstacles.createObstacle("rectangle", time_s(:), x_units, y_units, margin_units);
end

function trajectory = constantJerkTrajectory(duration_s)
    % Build analytic q=t^3/6, v=t^2/2, a=t, and j=1 on the first axis.
    positionPower_units        = zeros(1, 2, 6);
    velocityPower_units_s      = zeros(1, 2, 5);
    accelerationPower_units_s2 = zeros(1, 2, 4);
    jerkPower_units_s3         = zeros(1, 2, 3);
    positionPower_units(1, 1, 4) = duration_s^3 / 6;
    velocityPower_units_s(1, 1, 3) = duration_s^2 / 2;
    accelerationPower_units_s2(1, 1, 2) = duration_s;
    jerkPower_units_s3(1, 1, 1) = 1;
    time_s              = [0; duration_s];
    position_units        = [0 0; duration_s^3 / 6 0];
    velocity_units_s      = [0 0; duration_s^2 / 2 0];
    acceleration_units_s2 = [0 0; duration_s 0];
    jerk_units_s3         = [1 0; 1 0];
    trajectory          = trajectoryRecord(time_s, position_units, velocity_units_s, acceleration_units_s2, jerk_units_s3, positionPower_units, velocityPower_units_s, accelerationPower_units_s2, jerkPower_units_s3, duration_s);
end

function trajectory = linearTrajectory(initialState, goalState)
    % Build one exact constant-velocity segment sampled only at its endpoints.
    duration_s        = goalState.time_s - initialState.time_s;
    positionPower_units = zeros(1, 2, 6);
    positionPower_units(1, :, 1) = initialState.position_units;
    positionPower_units(1, :, 2) = goalState.position_units - initialState.position_units;
    velocityPower_units_s = zeros(1, 2, 5);
    velocityPower_units_s(1, :, 1) = initialState.velocity_units_s;
    accelerationPower_units_s2 = zeros(1, 2, 4);
    jerkPower_units_s3         = zeros(1, 2, 3);
    trajectory               = trajectoryRecord([initialState.time_s; goalState.time_s], [initialState.position_units; goalState.position_units], [initialState.velocity_units_s; goalState.velocity_units_s], zeros(2, 2), zeros(2, 2), positionPower_units, velocityPower_units_s, accelerationPower_units_s2, jerkPower_units_s3, duration_s);
end

function trajectory = interiorVelocityPeakTrajectory()
    % Build v=4s(1-s), which peaks between clear endpoint samples.
    duration_s        = 1;
    positionPower_units = zeros(1, 2, 6);
    positionPower_units(1, 1, 3) = 2;
    positionPower_units(1, 1, 4) = -4 / 3;
    velocityPower_units_s = zeros(1, 2, 5);
    velocityPower_units_s(1, 1, 2) = 4;
    velocityPower_units_s(1, 1, 3) = -4;
    accelerationPower_units_s2 = zeros(1, 2, 4);
    accelerationPower_units_s2(1, 1, 1) = 4;
    accelerationPower_units_s2(1, 1, 2) = -8;
    jerkPower_units_s3 = zeros(1, 2, 3);
    jerkPower_units_s3(1, 1, 1) = -8;
    trajectory = trajectoryRecord([0; 1], [0 0; 2 / 3 0], zeros(2, 2), [4 0; -4 0], [-8 0; -8 0], positionPower_units, velocityPower_units_s, accelerationPower_units_s2, jerkPower_units_s3, duration_s);
end

function trajectory = trajectoryRecord(time_s, position_units, velocity_units_s, acceleration_units_s2, jerk_units_s3, positionPower_units, velocityPower_units_s, accelerationPower_units_s2, jerkPower_units_s3, duration_s)
    % Assemble one manual trajectory with the required polynomial fields.
    polynomial = struct("SegmentCount", 1, "SegmentStartTime_s", time_s(1), ...
        "SegmentDuration_s", duration_s, "FinalTime_s", time_s(end), ...
        "positionPower_units", positionPower_units, ...
        "velocityPower_units_s", velocityPower_units_s, ...
        "accelerationPower_units_s2", accelerationPower_units_s2, ...
        "jerkPower_units_s3", jerkPower_units_s3, "TerminalState", struct());
    trajectory = struct("time_s", time_s, "position_units", position_units, ...
        "velocity_units_s", velocity_units_s, ...
        "acceleration_units_s2", acceleration_units_s2, ...
        "jerk_units_s3", jerk_units_s3, ...
        "Polynomial", polynomial);
end
