...
if timeIsFinite
    goalPosition_deg = azElInternal.goalPositionAtTime( goalState, time_s(end));
else
...
if timeIsStrictlyIncreasing && historyIsFinite && continuousBounds.Valid
    [seedCorridorCertified, seedCorridorClearance_deg] = azElInternal.validation.certifySeedCorridor( ...
...
goalPosition_deg = azElInternal.goalPositionAtTime( goalState, polynomial.FinalTime_s);
...
[~, polynomialPosition_deg, polynomialVelocity_deg_s, ...
    polynomialAcceleration_deg_s2, polynomialJerk_deg_s3] = azElInternal.motion.evaluatePolynomial(polynomial, time_s);
...
for segmentIndex = 1:polynomial.SegmentCount
    ...
    for splitIndex = 1:numel(splitTimes_s)
        [~, splitPoint_deg] = azElInternal.motion.evaluatePolynomial( ...
            polynomial, splitTimes_s(splitIndex), segmentIndex);

        for obstacleIndex = 1:numel(obstacles)
            shape = azElInternal.obstacles.shapeAtTime( obstacles(obstacleIndex), splitTimes_s(splitIndex));
    ...
    while ~isempty(stackStart_s)
        ...
        [~, point_deg] = azElInternal.motion.evaluatePolynomial( polynomial, intervalMid_s, segmentIndex);
        intervalResolved = true;
        intervalClearanceLowerBound_deg = Inf;

        for obstacleIndex = 1:numel(obstacles)
            [shape, geometry] = azElInternal.obstacles.shapeAtTime( obstacles(obstacleIndex), intervalMid_s);
