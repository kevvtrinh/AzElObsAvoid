...
\begin{tikzpicture}[remember picture,overlay]
  \fill[navy] (current page.north west) rectangle
    ([yshift=-3.25in]current page.north east);
  \fill[teal] ([yshift=-3.25in]current page.north west) rectangle
    ([yshift=-3.38in]current page.north east);
\end{tikzpicture}
...
X/Y obstacle transformation, third-order Hermite-Simpson
...
\textbf{Coordinate convention} & Euclidean $[\mathrm{x},\mathrm{y}]$ in coordinate units
\end{tabular}
...
{\small Terminology edition: 2026-09-07. Coordinate names and units follow
the generic x/y API. The algorithm discussion remains historical; consult
the repository README for the current planner and independent wrapping options.
Both archived PDF filenames are regenerated from this common source.}
...
in an x/y plane into a dynamically feasible, independently
validated trajectory. It covers two related products:

\begin{itemize}
  \item \term{Obstacle-aware X/Y planner}: the public
...
validation belong to the X/Y layer. This is an architectural boundary, not a
...
\node[prod] (api) {Public X/Y API\\\file{planTrajectory}};
...
\node[prod,below left=of kernel] (valid) {Independent X/Y\\trajectory validation};
...
\begin{scope}[on background layer]
  \node[draw=navy,dashed,rounded corners=3pt,fit=(api)(input)(search)(planner)(valid)(result),
    inner sep=5mm,label={[navy]above left:X/Y product}] {};
...
neutral HS3 source. The public neutral wrapper and the X/Y planner share the
...
\begin{itemize}
  ...
  \file{position_units}; optional endpoint velocity and acceleration default to
  zero.
...
Retain the direct seed; construct protected swept geometry, visibility/route class
...
interval using its Bernstein hull. The X/Y planner constructs the equivalent
...
For x/y work, coordinate 1 can be x and coordinate 2 can be
y. The neutral HS3 function does not assign those meanings. The caller
...
\section{From an X/Y obstacle to HS3 input}\label{sec:obstacles}

\subsection{Canonical obstacle representation}

Obstacle coordinates are treated as Euclidean $[\mathrm{x},
\mathrm{y}]$ coordinates in coordinate units. Distances and path lengths are
therefore planar degree distances, not spherical or geodesic distances.

The constructor retains two histories:

\begin{itemize}
  \item \file{originalX_units}/\file{originalY_units}: source geometry;
  \item \file{x_units}/\file{y_units}: protected geometry used by planning and
  collision checking.
\end{itemize}
...
After construction, \file{prepareObstacles.m} caches shapes, bounds, interval
...
\begin{scope}[shift={(0,0)}]
  \draw[->] (-0.5,0)--(7.2,0);
  \node[below,font=\scriptsize] at (6.1,0) {x};
  \draw[->] (0,-2.6)--(0,3.2) node[above,font=\scriptsize]{y (units)};
...
\begin{scope}[shift={(9.6,0)}]
  \draw[->] (-0.4,0)--(7.0,0);
  \node[below,font=\scriptsize] at (5.9,0) {x};
  \draw[->] (0,-2.6)--(0,3.2) node[above,font=\scriptsize]{y};
...
The graph state is augmented with a bounded two-dimensional winding/route class
...
\begin{scope}[shift={(8.0,.15)}]
  ...
  \draw[->] (0,0)--(0,4.0) node[above,font=\scriptsize]{$\beta_r$ (units)};
...
\subsection{Coordinate wrapping limitation}

The current coordinate interface has independent \file{WrapX} and
\file{WrapY} flags. Obstacle-free fixed-position requests select the nearest
equivalent goal on each enabled axis using that axis's workspace interval
width as its period. Half-period ties select positive displacement. Returned
motion remains continuous; spatial plots split at either periodic seam.
Obstacles and moving goals remain unsupported when either flag is enabled.
Graphics must therefore not be read as a periodic obstacle transformation.
...
$(2,2)$ units/s, acceleration limit $(1,1)$ units/s$^2$, and jerk limit
$(2,2)$ units/s$^3$.
...
\begin{equation}
[0\ \ 1]q(\tau)\ge1.5001,
\quad\text{or}\quad 1.5001-\mathrm{y}(\tau)\le0.
\end{equation}
...
If the same original rectangle translates 2 coordinate units in x between
$t=0$ and $t=10$ seconds, the protected vertices translate identically. At
$t=5$ seconds, $\lambda=0.5$ and

\begin{equation}
P_m(5)=[4.5,7.5]\times[-1.5,1.5],\qquad v_{obs,max}=0.2\ \mathrm{units/s}.
\end{equation}

For a validation interval $\Delta t=0.1$ second and path speed bound
$\lVert v_{path,max}\rVert_2=2$ units/s, Equation~\eqref{eq:collisioncertificate}
...
\file{WrapX}, \file{WrapY} & false & Independent periodic axes; periods are workspace interval widths. \\
\file{MaximumSeedCount} & 5 & Bounded topology proposal count. \\
\file{DirectOnly} & false & Permit visibility, route class, and timed seeds. \\
\file{ObstacleClusterDistance_units} & 0 & Disable nearby-region seed clustering. \\
...
\file{CollisionClearanceTolerance_units} & $10^{-7}$ & Clearance tolerance for validation. \\
...
and rejected edges, time layers, parent/cost relationships, route class/timed
...
\begin{itemize}
  ...
  association times.
  \item Wrapping either axis with obstacles or moving goals is unsupported.
  ...
  \item Geographic and X/Y geometry is planar in coordinate units, not spherical.
\end{itemize}
...
NaN-separated x/y vectors to an unsimplified \file{polyshape}.
...
\scriptentry{+obstacleAvoidance/+obstacles/prepareObstacles.m}{Precomputes sample
...
adds a timed route for changing history, enumerates distinct route class classes,
...
a forward layered graph over every supplied planning time; each state is a time
layer plus visibility node. It supports waiting and motion arcs, uses sampled
edge-collision filtering, and returns route positions, absolute route times,
parents, costs, and work counts.}
...
\scriptentry{examples/exampleMovingCircleNoWrap.m}{Demonstrates an
immediate detour around a rising circle with periodic x explicitly
...
dense U.S. outline that grows, deforms, rotates 180 coordinate units, and disappears,
...
a protected static U cavity through visibility/route class proposals without
hidden waypoints.}
...
visibility and distinct route class choices around two opposing protected U
obstacles.}
...
direct-line preservation, topology/route class/timed seeds, clustering/envelopes,
...
\begin{itemize}
  ...
  \item \file{citation.md}: references for route class search, direct collocation,
...
How is dynamic geometry evaluated? & \file{prepareObstacles.m},
...
What finally authorizes success? & \file{validateTrajectory.m} for the X/Y
