...
for obstacleIndex = 1:numel(inflatedAzElData)
    ...
    for sampleIndex = 1:sampleCount
        ...
        reportProgress = sampleIndex == 1 || sampleIndex == sampleCount || ...
...
templatePosition_deg = [double(templateAzimuth_deg(:)), ...
    double(templateElevation_deg(:))];
samplePosition_deg = [double(sampleAzimuth_deg(:)), ...
    double(sampleElevation_deg(:))];
translation_deg = [0 0];
isTranslatedCopy = isequal(size(templatePosition_deg), ...
    size(samplePosition_deg));
if ~isTranslatedCopy
    return;
end
...
isTranslatedCopy = max(abs(offset_deg - translation_deg), [], "all") <= ...
