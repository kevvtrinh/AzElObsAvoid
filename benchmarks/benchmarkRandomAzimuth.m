...
% OUTPUTS
%   - runs (table)
%       Per-case timing and motion-quality data. Planner
...
for caseIndex = reshape(caseIndices, 1, [])
    for includeStaticObstacle = [false, true]
        ...
        if includeStaticObstacle
            ...
            staticSkeleton = obstacleAvoidance.search.createVisibilitySkeleton( ...
                staticScene, result.Limits, result.Options);
            staticGraph = obstacleAvoidance.search.createVisibilityGraph( ...
                staticSkeleton, scenario.InitialState.position_units, ...
        ...
        rows{rowIndex} = table(caseIndex, includeStaticObstacle, ...
            ...
            'VariableNames', {'CaseIndex', 'StaticObstacle', ...
        ...
        fprintf(['Case %d static=%d: %s, %.3f s, ' ...
            'length %.6f deg, ratio %.6f\n'], caseIndex, ...
            includeStaticObstacle, result.TerminationReason, ...
