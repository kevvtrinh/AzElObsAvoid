...
certifies two spans, [2770,2910] and [2910,3000] s. Each span has one conforming
273-triangle partition. Its restrictions give the same certified face indices
...
partition and simplicity certification. The canonicalizer therefore carries
the existing source-interval alignment into its span certificate. It never
...
The first swept construction used axis-aligned margin squares of half-width
...
swept union, and a rotated unit square with 0.1-unit protection reproduces
...
swept cells now use half-width `sqrt(2)*safetyMargin_units`. With that
...
counterexample for half-width `d` and the certificate for `d*sqrt(2)`.
...
`vertexCorrespondence` as `sourceIndex` and the swept cells follow the
caller's transform. The swept union of a coastline outline is then covered by
...
set; 5524 cells became 2680 for the whole scene, 1582 of them U.S. swept
...
`sweptCorrespondingConvexCells`. The same run planned the Vietnam slew in
