...
% SYNTAX
%   runs = benchmarkRandomGoalVisibilityWindows()
...
%     whose protected goal is visible, then hidden, then visible again.
%   - Run the public planner and independent validator on every accepted variant.
%**************************************************************************
% INPUTS
%   - requestFilePath (string scalar, optional; default repository fixture)
%       Path to an offlineSandboxRequest/v1 JSON template.
%   - caseCount (positive integer scalar, optional; default 50)
%       Number of accepted deterministic cases.
%   - outputFolder (string scalar, optional; default "")
%       Existing or new folder for compact MAT and CSV results.
%**************************************************************************
% OUTPUTS
%   - runs (table)
%       One row per run with validity, quality, timing, and random parameters.
%   - cases (caseCount-by-1 cell array)
%       Generated request structs.
%   - results (caseCount-by-1 cell array)
%       Complete planner results. Ordinary failures remain Success = false;
%       invalid input throws unless the measured planner call is captured.
%**************************************************************************
% UNITS
...
for caseIndex = 1:caseCount
    ...
    rows{caseIndex} = table(caseIndex, parameters.Seed, ...
        ...
        terminationReason, arrivalWindow, ...
        ...
        'EarliestPossibleArrival_s', 'FirstWindowKinematicallyPossible', ...
        'Success', 'ValidationPassed', 'TerminationReason', ...
        'ArrivalWindow', 'ArrivalTime_s', 'TrajectoryDuration_s', ...
...
function [request, parameters] = randomizeRequest(template, caseIndex, caseSeed)
    % Randomize one template while preserving its visibility-window structure.
    request           = template;
    ...
    parameters = struct( ...
        ...
        'TimeScale',           timeScale, ...
        'StaticSizeScale',     staticSizeScale, ...
        'OccluderSizeScale',   movingSizeScale, ...
...
function points_units = transformPoints(points_units, pivot_units, scale, coordinateMap, translation_units)
    % Apply the sampled case's scale, axis map, pivot, and translation.
...
function limit = mapDerivativeLimit(sourceLimit, coordinateMap, scale)
    % Map a scalar or per-axis derivative limit into transformed coordinates.
...
function bounds = transformBounds(limits, pivot_units, scale, coordinateMap, translation_units)
    % Transform all workspace corners and recover enclosing axis intervals.
    corners_units = [ ...
        limits.xInterval_units(1), limits.yInterval_units(1); ...
    ...
    bounds = [ ...
        min(corners_units(:, 1)), max(corners_units(:, 1)); ...
...
function state = normalizeState(stateInput)
    % Normalize JSON numeric arrays to the planner's row-vector state shape.
    state                       = stateInput;
    state.time_s                = double(stateInput.time_s);
    state.position_units        = reshape(double(stateInput.position_units), 1, 2);
    state.velocity_units_s      = reshape(double(stateInput.velocity_units_s), 1, 2);
    state.acceleration_units_s2 = reshape(double(stateInput.acceleration_units_s2), 1, 2);
end

function [passed, window] = characterizeGoalWindows(obstacles, initialState, goalState)
    % Sample goal occupancy and require one visible-hidden-visible sequence.
...
function waitDuration_s = routeWaitDuration(result)
    ...
    segmentIsStationary = vecnorm(diff(route_units, 1, 1), 2, 2) <= 1e-10;
    waitDuration_s      = sum(diff(routeTime_s) .* segmentIsStationary);
end

function arrivalWindow = classifyArrival(arrivalTime_s, window)
    % Classify arrival relative to the sampled hidden interval.
    arrivalWindow = "unavailable";
    if ~isfinite(arrivalTime_s)
        return
    end
...
function value = numericField(container, fieldName)
    % Read one finite-shape numeric scalar field when it is available.
    value = NaN;
    if isstruct(container) && isfield(container, fieldName) && ...
            isnumeric(container.(fieldName)) && isscalar(container.(fieldName))
        value = double(container.(fieldName));
    end
end

function value = stringField(container, fieldName, defaultValue)
    % Read one scalar text field or retain the caller's default value.
    value = string(defaultValue);
    if isstruct(container) && isfield(container, fieldName)
        candidate = string(container.(fieldName));
...
function createSummaryPlot(runs, outputFolder)
    % Save timing, arrival-window, and path-length benchmark summaries.
    caseIndex       = runs.CaseIndex;
    caseIsControl   = ~runs.ReopenChallenge;
    caseIsChallenge = runs.ReopenChallenge;
    runIsValid      = runs.Success & runs.ValidationPassed;
    ...
    scatter(caseIndex(caseIsControl & runIsValid), normalizedArrival(caseIsControl & runIsValid), ...
        34, [0.12, 0.47, 0.71], 'filled');
    scatter(caseIndex(caseIsChallenge & runIsValid), normalizedArrival(caseIsChallenge & runIsValid), ...
        42, [0.42, 0.24, 0.68], 'filled');
    scatter(caseIndex(~runIsValid), 0.5 * ones(sum(~runIsValid), 1), ...
    ...
    nexttile;
    hold on;
    scatter(caseIndex(caseIsControl), runs.WallTime_s(caseIsControl), ...
        34, [0.12, 0.47, 0.71], 'filled');
    scatter(caseIndex(caseIsChallenge & runIsValid), runs.WallTime_s(caseIsChallenge & runIsValid), ...
        42, [0.42, 0.24, 0.68], 'filled');
    scatter(caseIndex(caseIsChallenge & ~runIsValid), runs.WallTime_s(caseIsChallenge & ~runIsValid), ...
        54, [0.8, 0.1, 0.1], 'x', 'LineWidth', 1.5);
    set(gca, 'YScale', 'log');
    ...
    nexttile;
    hold on;
    scatter(caseIndex(caseIsControl & runIsValid), runs.PathLength_units(caseIsControl & runIsValid), ...
        34, [0.12, 0.47, 0.71], 'filled');
    scatter(caseIndex(caseIsChallenge & runIsValid), runs.PathLength_units(caseIsChallenge & runIsValid), ...
        42, [0.42, 0.24, 0.68], 'filled');
    scatter(caseIndex(~runIsValid), zeros(sum(~runIsValid), 1), ...
        54, [0.8, 0.1, 0.1], 'x', 'LineWidth', 1.5);
    ...
    title(layout, sprintf(['%d-case visible-hidden-visible benchmark: ' ...
        '%d/%d validated'], height(runs), sum(runIsValid), height(runs)));
