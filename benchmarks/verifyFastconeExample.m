...
evidence=findEvidence(result); hasConic=false; native=false; matlab=false; recovery=false;
for k=1:numel(evidence)
    hasConic=hasConic || evidence{k}.CallCount>0;
    native=native || evidence{k}.NativeAcceptedCount>0;
    matlab=matlab || evidence{k}.MatlabAcceptedCount>0;
...
metrics=struct('Example',string(name),'Planner',result.Success,'Independent',validation.Passed, ...
    ...
    'HasConicCalls',hasConic,'NativeExecuted',native,'MatlabExecuted',matlab,'RecoveryUsed',recovery, ...
...
notes=sprintf('fastcone integration; %s diagnostic wall time; conic=%d native=%d matlab=%d recovery=%d; expectedFailure=%d', ...
    suffix,hasConic,native,matlab,recovery,expectedFailure);
row=struct('RunDate',string(datetime('now','Format','yyyy-MM-dd')), ...
    'SourceCommit',"36512e2+matlab-fastcone",'Branch',"bmtp-cleanup-codex", ...
    ...
    'PlannerSuccess',result.Success,'ValidationPassed',validation.Passed, ...
