...
function printExampleResult(exampleName, result, checkResult, elapsedTime_s)
    ...
    if result.Success
        ...
        motionLength_deg   = sum(vecnorm(diff(result.position_deg), 2, 2));
        polylineLength_deg = sum(vecnorm(diff(result.Route_deg, 1, 1), 2, 2));
        ...
        reportFormat       = "REFACTOR_EXAMPLE name=%s jerk=enabled success=%d check=%d validator=%d " + "reason=%s polyline_deg=%.15g smooth_deg=%.15g " + "duration_s=%.15g collision=%d kinematic=%d certificate=%d " + "runtime_s=%.6f\n";
        fprintf(reportFormat, exampleName, result.Success, checkResult.Passed, motionCheck.Passed, result.TerminationReason, polylineLength_deg, motionLength_deg, result.TrajectoryDuration_s, motionCheck.CollisionFree, kinematicPassed, certificatePassed, elapsedTime_s);
    else
        reportFormat = "REFACTOR_EXAMPLE name=%s jerk=enabled success=%d check=%d validator=%d " + "reason=%s polyline_deg=NaN smooth_deg=NaN duration_s=NaN " + "collision=0 kinematic=NaN certificate=NaN runtime_s=%.6f\n";
