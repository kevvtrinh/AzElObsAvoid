...
for exampleIndex = 1:exampleCount
    ...
    exampleTimer = tic;
    [result, diagnosis] = feval(exampleName, struct( ...
        "PlotOutputs", false, ...
        "FigureVisible", "off", ...
        "Verbose", false));
    ...
    checkResult = validateExampleResult(result, exampleName, ...
        struct("ExpectedSuccess", expectedSuccess(exampleIndex)), diagnosis);
...
if result.Success
    ...
    polylineLength_deg = sum(vecnorm( ...
        diff(result.Route_deg, 1, 1), 2, 2));
    kinematicPassed = motionCheck.VelocityWithinLimits && ...
        motionCheck.AccelerationWithinLimits && ...
        motionCheck.JerkWithinLimits;
    certificatePassed = motionCheck.CollisionFree && motionCheck.CollisionResolved && ...
        motionCheck.PolynomialFormatValid && motionCheck.DynamicsConsistent && ...
        motionCheck.InitialStateMatched && motionCheck.TerminalStateMatched;
    reportFormat = ...
        "REFACTOR_EXAMPLE name=%s jerk=enabled success=%d check=%d validator=%d " + ...
        "reason=%s polyline_deg=%.15g smooth_deg=%.15g " + ...
        "duration_s=%.15g collision=%d kinematic=%d certificate=%d " + ...
        "runtime_s=%.6f\n";
    fprintf(reportFormat, ...
        exampleName, result.Success, checkResult.Passed, motionCheck.Passed, ...
        result.TerminationReason, polylineLength_deg, ...
        motionLength_deg, result.TrajectoryDuration_s, ...
        motionCheck.CollisionFree, kinematicPassed, certificatePassed, ...
        elapsedTime_s);
else
    reportFormat = ...
        "REFACTOR_EXAMPLE name=%s jerk=enabled success=%d check=%d validator=%d " + ...
        "reason=%s polyline_deg=NaN smooth_deg=NaN duration_s=NaN " + ...
        "collision=0 kinematic=NaN certificate=NaN runtime_s=%.6f\n";
    fprintf(reportFormat, ...
        exampleName, result.Success, checkResult.Passed, false, ...
