...
| Periodic image resolution | `testPeriodicWrapUsesNearestImage`, `testPeriodicYAndDualAxisWrap` | Nearest unwrapped endpoint, validated motion; periodic obstacles and targets remain unsupported | Required coordinate policy |
...
| Fixed static detour | `testPlanningCore/testDetourAndTampering`, `planner()` defaults | Exhaustive exact spatial visibility graph and static BMTP | Retained |
...
| Earliest zero-delay C3 chord | `testSparseDynamicZeroWaitDeparture` | The certified chord attains the physical travel lower bound and cannot be beaten; it returns immediately with no timed or chronological search | Retained analytic boundary |
...
| Earliest chronological search | `testEarliestMovingTargetUsesChronologicalClock`, `testEarliestStaticNonrestUsesPhysicalClockTrials` | Fixed-arrival trials on declared physical clocks only after the analytic family and the timed profile both fail; moving targets and non-rest endpoints change the goal state with the clock | Retained; honest incomplete search |
