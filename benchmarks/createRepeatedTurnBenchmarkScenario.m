...
validateattributes(turnCount, {'numeric'}, ...
...
requiredNames = [ ...
    ...
    "maxJerk_deg_s3", "elevationInterval_deg"];
if ~isstruct(constants) || ~isscalar(constants) || ...
        ~all(isfield(constants, requiredNames))
...
for fieldName = positiveScalarNames
    validateattributes(constants.(fieldName), {'numeric'}, ...
        {'real', 'finite', 'scalar', 'positive'});
end
validateattributes(constants.safetyMargin_deg, {'numeric'}, ...
...
centerAzimuth_deg = constants.barrierSpacing_deg * ...
    (barrierIndices - (turnCount + 1) / 2);
centerElevation_deg = constants.barrierCenterMagnitude_deg * ...
    (-1).^(barrierIndices - 1);
...
for obstacleIndex = 1:turnCount
    ...
    obstacle = makeAzElObstacleData( ...
        ...
        rectangle_deg(:, 1), rectangle_deg(:, 2), ...
        constants.safetyMargin_deg);
...
initialState = struct( ...
    "time_s", 0, "position_deg", [startAzimuth_deg 0], ...
    "velocity_deg_s", [0 0], "acceleration_deg_s2", [0 0]);
goalState = struct( ...
    "time_s", goalTime_s, "position_deg", [goalAzimuth_deg 0], ...
    "velocity_deg_s", [0 0], "acceleration_deg_s2", [0 0]);
limits = struct( ...
    ...
    "azimuthInterval_deg", ...
    [startAzimuth_deg - 1, goalAzimuth_deg + 1], ...
    "elevationInterval_deg", constants.elevationInterval_deg);
end
