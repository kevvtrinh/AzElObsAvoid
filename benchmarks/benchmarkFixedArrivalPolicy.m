...
%**************************************************************************
% PURPOSE
%   - Measure the fixed-arrival shortcut and fallback policy on four
...
cases(4) = struct('Name', "Recurrent curtain timed fallback", ...
    'obstacles', curtain, ...
...
for caseIndex = 1:numel(cases)
    ...
    for repetition = 0:repetitions
        ...
        if ~isempty(attempts)
            ...
            timedFallbackUsed = any([attempts.Role] == "fallback");
            totalAlternatingIterations = sum([attempts.IterationCount]);
            attemptSequence = strjoin([attempts.Kind] + ":" + [attempts.Outcome], " > ");
        end
        fprintf(['%s repetition %d: %.6f s; valid %d; arrival %.9f; ' ...
            'length %.12f; guide %s; attempts %s; %s\n'], caseRequest.Name, repetition, ...
        ...
        rows{rowIndex} = struct( ...
            ...
            'TimedFallbackUsed',     timedFallbackUsed, ...
