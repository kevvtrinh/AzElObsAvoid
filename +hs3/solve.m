...
% SYNTAX
%   options = hs3.solve()
%   trajectory = hs3.solve(initialState, terminalState, limits)
%   trajectory = hs3.solve(initialState, terminalState, limits, ...
%       optionOverrides)
%   trajectory = hs3.solve(initialState, terminalState, limits, ...
...
%       Partial hs3.defaultOptions overrides; empty fields use defaults.
...
if nargin == 0
    trajectory = hs3.defaultOptions();
    return;
end
if nargin < 3
    error("solve:NotEnoughInputs", ...
...
if any(pathConstraints.TauEnd > ...
    ...
    error("solve:CrossSegmentPathInterval", ...
        "Each path-constraint interval must lie inside one HS3 segment.");
end
hs3.subintervalHullMap(pathConstraints.Tau, pathConstraints.TauEnd, ...
...
trajectory.IntegratedSquaredJerk = hs3.integratedSquaredJerk( ...
    solverResult.Decision, false, solverResult.FinalTime, ...
    options.SegmentCount, startTime, dimensionCount);
trajectory.Validation = hs3.validate(trajectory);
...
if ~isstruct(initialState) || ~isscalar(initialState) || ...
    ...
    error("solve:InvalidInitialState", ...
...
if ~isstruct(terminalState) || ~isscalar(terminalState) || ...
    ...
    error("solve:InvalidTerminalState", ...
...
if numel(state.velocity) ~= dimensionCount || ...
    ...
    error("solve:StateDimensionMismatch", ...
...
if ~isstruct(pathConstraints) || ~isscalar(pathConstraints)
    error("solve:InvalidPathConstraints", ...
