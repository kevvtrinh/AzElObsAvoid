function [isOccupied, blockingObstacleIndex, queryDetails] = queryTimeObstacle(obstacles, azimuth_deg, elevation_deg, ...
        queryTime, optionOverrides)
%% Section 0: Header & Readme
% SYNTAX
%   options = azElPlannerMethods.corridor.queryTimeObstacle()
%   isOccupied = azElPlannerMethods.corridor.queryTimeObstacle( ...
%       obstacles, azimuth_deg, elevation_deg, queryTime)
%   [isOccupied, blockingObstacleIndex, queryDetails] = azElPlannerMethods.corridor.queryTimeObstacle(obstacles, azimuth_deg, elevation_deg, ...
%       queryTime, optionOverrides)
%**************************************************************************
% PURPOSE
...
options.BoundaryIsOccupied = azElPlannerMethods.corridor.internal.normalizeLogicalScalar( ...
...
for queryIndex = 1:queryCount
    ...
    for obstacleIndex = 1:numel(obstacles)
        shape = azElPlannerMethods.corridor.internal.obstacles.shapeAtTime( obstacles(obstacleIndex), queryTime_s(queryIndex));
...
for timeIndex = 1:numel(uniqueTime_s)
    ...
    for obstacleIndex = 1:numel(obstacles)
        ...
        if hasOneRing
            ...
            if isempty(shape)
                shape = azElPlannerMethods.corridor.internal.obstacles.shapeAtTime( obstacles(obstacleIndex), uniqueTime_s(timeIndex));
            end
...
for pointIndex = 1:pointCount
    clearance_deg = azElPlannerMethods.corridor.internal.geometry.pointPolygonClearance( shape, points_deg(pointIndex, :));
