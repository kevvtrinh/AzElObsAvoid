function [bestMotion, bestValidation, diagnostics] = solveCompactC3( ...
        seed, upperDuration_s, obstacles, initialState, goalState, limits, plannerOptions)
%% Section 0: Header & Readme
% SYNTAX
%   [bestMotion, bestValidation, diagnostics] = azElPlannerMethods.corridor.internal.motion.solveCompactC3(seed, upperDuration_s, ...
%       obstacles, initialState, goalState, limits, plannerOptions)
%**************************************************************************
% PURPOSE
%   - Improve one already feasible short topology with a fixed-size C3 spline.
...
%   - diagnostics (scalar struct), duration and quadratic-solve evidence.
%**************************************************************************
% UNITS
...
% Eight spans cap optimizer size independently of the seed's sampling density.
% Endpoint controls are fixed; the least-squares fit initializes only interior
% controls from the seed's normalized route parameter.
spanCount = 8;
degree = 5;
...
[~, fitBasis] = azElPlannerMethods.corridor.internal.motion.evaluatePolynomial( basisPolynomial, fitTime);
...
controlPoint_deg(interiorIndex, :) = fitBasis(:, interiorIndex) \ (desiredPosition_deg - fixedContribution_deg);
...
duration_s = physicalLowerDuration_s + 0.1 * (upperDuration_s - physicalLowerDuration_s);
...
% Probe at most eight durations, retaining only trials that pass independent validation.
for trialIndex = 1:8
    [trialMotion, trialControlPoint_deg, lastExitFlag, trialQpCount] = solveDuration(controlPoint_deg, duration_s, obstacles, ...
        initialState, limits, plannerOptions, interiorIndex, fixedIndex);
    qpCount = qpCount + trialQpCount;
    trialCount = trialIndex;
    validation = azElPlannerMethods.corridor.validateTrajectory( trialMotion, obstacles, initialState, goalState, limits, plannerOptions);
...
diagnostics = struct("Attempted", true, "Accepted", ~isempty(bestMotion), ...
    ...
    "InitialDuration_s", initialDuration_s, "BestDuration_s", upperDuration_s);
end


function [motion, controlPoint_deg, exitFlag, qpCount] = solveDuration( ...
        controlPoint_deg, duration_s, obstacles, initialState, limits, plannerOptions, interiorIndex, fixedIndex)
...
affinePolynomial = azElPlannerMethods.corridor.internal.motion.convertBsplineToPolynomial( ...
    eye(size(controlPoint_deg, 1)), 5, initialState.time_s, spanDuration_s);
sampleTime_s = linspace(initialState.time_s, initialState.time_s + duration_s, 257).';
[~, positionBasis, velocityBasis, accelerationBasis, jerkBasis] = azElPlannerMethods.corridor.internal.motion.evaluatePolynomial(affinePolynomial, sampleTime_s);
decision = [controlPoint_deg(interiorIndex, 1); controlPoint_deg(interiorIndex, 2)];
quadraticOptions = optimoptions("quadprog", "Display", "off", "Algorithm", "active-set");
exitFlag = NaN;
qpCount = 0;

% Re-linearize obstacle barriers around each accepted control-point update.
for iterationIndex = 1:6
    fixedPosition_deg = positionBasis(:, fixedIndex) * controlPoint_deg(fixedIndex, :);
    [barrierMatrix, barrierBound] = obstacleRows( ...
        positionBasis(:, interiorIndex), fixedPosition_deg, decision, sampleTime_s, obstacles);
    ...
    [trialDecision, ~, exitFlag] = quadprog( ...
        ...
        [kinematicBound; barrierBound], [], [], decision - 6, decision + 6, decision, quadraticOptions);
    qpCount = qpCount + 1;
    if exitFlag <= 0
        break;
    end
    ...
    controlPoint_deg(interiorIndex, :) = [ decision(1:numel(interiorIndex)), decision(numel(interiorIndex) + 1:end)];
    if step_deg <= 1e-4
        break;
    end
end
polynomial = azElPlannerMethods.corridor.internal.motion.convertBsplineToPolynomial( controlPoint_deg, 5, initialState.time_s, spanDuration_s);
...
[outputTime_s, position_deg, velocity_deg_s, acceleration_deg_s2, ...
    jerk_deg_s3] = azElPlannerMethods.corridor.internal.motion.evaluatePolynomial( polynomial, outputTime_s);
motion = struct("time_s", outputTime_s, "position_deg", position_deg, ...
    ...
    "ControlPoint_deg", controlPoint_deg, "SpanDuration_s", spanDuration_s);
end

function [matrix, bound] = obstacleRows(interiorBasis, fixedPosition_deg, decision, time_s, obstacles)
...
for obstacleIndex = 1:obstacleCount
    ...
    if stationaryHistory
        ...
        [clearance_deg, nearestPoint_deg] = azElPlannerMethods.corridor.internal.geometry.pointPolygonClearance( ...
        ...
        for nearIndex = 1:numel(nearTimeIndex)
            ...
            [clearance_deg(nearIndex), nearestPoint_deg(nearIndex, :)] = azElPlannerMethods.corridor.internal.geometry.pointPolygonClearance( ...
...
function [matrix, bound] = kinematicRows(basisByOrder, controlPoint_deg, interiorIndex, fixedIndex, limits)
