...
% Prepare each obstacle independently so its reusable interpolation data stays
% aligned with the original history record.
for obstacleIndex = 1:numel(obstacles)
    ...
    % Convert every source slice once and accumulate one conservative bound for
    % broad-phase rejection across the obstacle's complete history.
    ...
    % Compare each adjacent slice pair to decide whether exact vertex
    % interpolation is valid or a conservative union must represent the span.
    for intervalIndex = 1:intervalCount
        ...
        matchingTopology(intervalIndex) = numel(lowerAzimuth_deg) == ...
            numel(upperAzimuth_deg) && ...
        ...
        if matchingTopology(intervalIndex)
            deltaAzimuth_deg{intervalIndex} = upperAzimuth_deg - ...
                lowerAzimuth_deg;
            deltaElevation_deg{intervalIndex} = upperElevation_deg - ...
                lowerElevation_deg;
    ...
    % Give each source slice the worst speed bound of its neighboring intervals
    % so exact-time collision checks remain conservative at slice boundaries.
    for sampleIndex = 1:sampleCount
