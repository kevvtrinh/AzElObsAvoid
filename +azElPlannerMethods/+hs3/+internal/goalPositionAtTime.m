...
% SYNTAX
%   position_deg = azElPlannerMethods.hs3.internal.goalPositionAtTime(goalState, time_s)
%**************************************************************************
% PURPOSE
...
% A moving goal is sampled with the interpolation rule resolved by the planner.
if isfield(goalState, "targetTime_s") && ~isempty(goalState.targetTime_s)
    ...
    % A fixed goal does not depend on query time, so return its stored position.
