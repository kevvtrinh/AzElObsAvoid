...
% SYNTAX
%   [clusteredShape, record] = azElPlannerMethods.hs3.internal.search.clusterSeedShape( ...
%       sweptShape, clusterDistance_deg, protectedPoints_deg)
%**************************************************************************
% PURPOSE
...
if ~isa(sweptShape, "polyshape") || ~isscalar(sweptShape)
    error("azElInternal:clusterAzElSeedShape:InvalidShape", ...
...
for expandedRegionIndex = 1:numel(expandedRegions)
    ...
    % Collect the finite vertices of every member before forming their single
    % convex hull.
...
% Retain every region that was not absorbed into a cluster so the seed-only
% geometry remains conservative and complete.
