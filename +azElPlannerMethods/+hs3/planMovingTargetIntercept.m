...
% Normalize both terminal-derivative policy switches through the shared
% scalar-logical contract before constructing the goal state.
...
% Convert the requested intercept policy into the planner's fixed- or earliest-arrival mode.
if options.InterceptMode == "specifiedTime"
    ...
    interceptTime_s = min(targetTime_s(end), ...
        initialTime_s + options.MaximumSearchDuration_s);
    goalTimeMode = "earliestArrival";
end

% Sample the target state at the horizon supplied to the single planner call.
terminalPosition_deg = interp1( ...
    targetTime_s, targetPosition_deg, interceptTime_s, interpolationMethod);
[terminalVelocity_deg_s, terminalAcceleration_deg_s2] = targetDerivatives( ...
...
% Package the sampled target history so the planner can evaluate a moving goal internally.
goalState = struct( ...
...
% Forward all method options while overriding only the goal-time mode chosen above.
...
% Report the target position at the achieved intercept, or stable NaNs on failure.
if result.Success
