...
% SYNTAX
%   azElData = azElPlannerMethods.hs3.normalizeTimeObstacleData(inputData)
%**************************************************************************
% PURPOSE
...
% Validate every protected slice independently because vertex counts may vary
% over time even though the history length is fixed.
for sampleIndex = 1:sampleCount
    ...
    [azimuthSlices_deg{sampleIndex}, ...
        elevationSlices_deg{sampleIndex}, removedRegionCount] = normalizeBoundarySliceTopology( ...
        azimuthSlices_deg{sampleIndex}, ...
        elevationSlices_deg{sampleIndex}, ...
...
if hasOriginalBoundaries
    ...
    % Apply the same topology rules to every original-geometry slice so
    % provenance remains structurally aligned with protected geometry.
    for sampleIndex = 1:sampleCount
...
function [azimuth_deg, elevation_deg, removedRegionCount] = normalizeBoundarySliceTopology( ...
        azimuth_deg, elevation_deg, ...
        sampleIndex, boundaryRole)
% Reject invalid separators and short regions, removing only two-vertex rings.
...
% Classify the finite boundary regions.
...
% Rebuild only the regions that can enclose area.
...
% Copy each retained ring before allocating the final NaN-separated vectors.
...
% Write the retained rings in their original order with one separator between
% consecutive regions.
for retainedIndex = 1:numel(retainedRegionIndex)
