function validation = validateTrajectory(trajectory, obstacles, ...
...
% Verify every protected obstacle retains the original-geometry margin provenance.
...
% Collect the name of each failed validation check for the public diagnostic message.
...
% Confirm every derivative-order coefficient array has its required finite shape.
...
% Validate every polynomial segment over its full normalized time interval.
for segmentIndex = 1:segmentCount

    % Check azimuth and elevation coefficients independently against their axis limits.
    for axisIndex = 1:2
        % Read this segment-axis polynomial and each of its exact derivatives.
        ...
        % Use Bernstein convex-hull bounds to cover the full continuous segment.
        ...
        % Differentiate the stored power coefficients and compare adjacent derivative records.
        ...
        % Adjacent segments must agree in position, velocity, and acceleration at their knot.
...
% Summarize the continuous physical bounds and exact derivative consistency.
...
% Compare the polynomial's first and last states with the requested endpoint states.
...
% Re-evaluate the polynomial at every returned sample to validate the public histories.
[~, polynomialPosition_deg, polynomialVelocity_deg_s, ...
    polynomialAcceleration_deg_s2, polynomialJerk_deg_s3] = azElPlannerMethods.hs3.internal.motion.evaluatePolynomial( ...
    polynomial, time_s);
...
% The polynomial is valid only when its schema and every independent consistency check pass.
...
bernstein = azElPlannerMethods.hs3.internal.powerToBernstein(powerCoefficient);
...
% Certify each polynomial segment independently across obstacle history breakpoints.
for segmentIndex = 1:polynomial.SegmentCount
    ...
    % Add every obstacle geometry-change time that falls inside this segment.
    for obstacleIndex = 1:numel(obstacles)
    ...
    % Check every breakpoint explicitly because interval bounds meet at these times.
    for splitIndex = 1:numel(splitTimes_s)
        [~, splitPoint_deg] = azElPlannerMethods.hs3.internal.motion.evaluatePolynomial( ...
            polynomial, splitTimes_s(splitIndex), segmentIndex);

        % Measure the breakpoint against every obstacle active at that exact time.
        for obstacleIndex = 1:numel(obstacles)
    ...
    % Repeatedly bisect unresolved intervals until clearance is proved or resolution is exhausted.
    while ~isempty(stackStart_s)
        ...
        [~, point_deg] = azElPlannerMethods.hs3.internal.motion.evaluatePolynomial( ...
            polynomial, intervalMid_s, segmentIndex);
        intervalResolved = true;
        intervalClearanceLowerBound_deg = Inf;

        % Bound relative motion against every obstacle over the current half-interval.
        for obstacleIndex = 1:numel(obstacles)
