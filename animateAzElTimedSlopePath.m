...
%       Global frame and obstacle-slice budgets bound display work.
...
budgetStride = ceil((numel(time_s) - 1) / ...
    (options.MaximumAnimationFrameCount - 1));
effectiveFrameStride = max(options.FrameStride, max(1, budgetStride));
frameIndices = unique([ ...
    (1:effectiveFrameStride:numel(time_s)).'; numel(time_s)]);
...
for frameCursor = 1:numel(frameIndices)
    ...
    if hasObstacleField && options.ShowObstacles
        [obstacle2DHandles, obstacleHandlesWereRebuilt] = ...
            updateObstacleSnapshot(axes2D, obstacleField, ...
            currentTime_s, obstacleColors, options, obstacle2DHandles);
        if obstacleHandlesWereRebuilt
...
obstacleDisplayCount = max(1, numel(obstacleField.Obstacles));
globalSliceShare = max(1, floor( ...
    options.MaximumDisplayedTotalSlices / obstacleDisplayCount));
for obstacleIndex = 1:numel(obstacleField.Obstacles)
    ...
    displayedSliceCount = min([obstacle.SampleCount, ...
        options.MaximumDisplayedSlicesPerObstacle, globalSliceShare]);
    displayedSlices = unique(round(linspace( ...
        1, obstacle.SampleCount, displayedSliceCount)));
...
function [handles, handlesWereRebuilt] = updateObstacleSnapshot( ...
        axesHandle, obstacleField, currentTime_s, colors, options, handles)
% PURPOSE
%   - Update persistent obstacle handles and rebuild only after topology change.
regionGeometry_deg = cell(0, 1);
regionIsSafety = false(0, 1);
regionObstacleIndex = zeros(0, 1);
for obstacleIndex = 1:numel(obstacleField.Obstacles)
    obstacle = obstacleField.Obstacles(obstacleIndex);
    obstacleMargin_deg = ...
        options.ObstacleSafetyMargins_deg(obstacleIndex);
    ...
    if obstacleMargin_deg > 0
        for safetyRegionIndex = 1:numel(safetyRegions)
            safetyRegion_deg = safetyRegions{safetyRegionIndex};
            % The number of regions is small and topology can change at an
            % obstacle event. Bounded growth keeps that event order visible.
            regionGeometry_deg{end + 1, 1} = [ ...
                safetyRegion_deg; safetyRegion_deg(1, :)]; %#ok<AGROW>
            regionIsSafety(end + 1, 1) = true; %#ok<AGROW>
            regionObstacleIndex(end + 1, 1) = obstacleIndex; %#ok<AGROW>
        end
    end
    for rawRegionIndex = 1:numel(rawRegions)
        regionGeometry_deg{end + 1, 1} = ...
            rawRegions{rawRegionIndex}; %#ok<AGROW>
        regionIsSafety(end + 1, 1) = false; %#ok<AGROW>
        regionObstacleIndex(end + 1, 1) = obstacleIndex; %#ok<AGROW>
    end
end

regionCount = numel(regionGeometry_deg);
canReuseHandles = numel(handles) == regionCount && ...
    all(isgraphics(handles));
for regionIndex = 1:regionCount
    if ~canReuseHandles
        break;
    end
    expectedTag = "AzElOriginalBoundary";
    if regionIsSafety(regionIndex)
        expectedTag = "AzElProtectedBoundary";
    end
    canReuseHandles = string(get(handles(regionIndex), "Tag")) == ...
        expectedTag;
end

handlesWereRebuilt = ~canReuseHandles;
if handlesWereRebuilt
    if ~isempty(handles)
        delete(handles(isgraphics(handles)));
    end
    handles = gobjects(regionCount, 1);
end
for regionIndex = 1:regionCount
    geometry_deg = regionGeometry_deg{regionIndex};
    obstacleIndex = regionObstacleIndex(regionIndex);
    if regionIsSafety(regionIndex)
        if handlesWereRebuilt
            handles(regionIndex) = plot(axesHandle, ...
                geometry_deg(:, 1), geometry_deg(:, 2), "--", ...
                "Color", [0.92 0.18 0.08], ...
                "LineWidth", 2.2, "HandleVisibility", "off", ...
                "Tag", "AzElProtectedBoundary");
        else
            set(handles(regionIndex), ...
                "XData", geometry_deg(:, 1), ...
                "YData", geometry_deg(:, 2));
        end
    else
        obstacleColor = colors(obstacleIndex, :);
        if handlesWereRebuilt
            handles(regionIndex) = patch(axesHandle, ...
                geometry_deg(:, 1), geometry_deg(:, 2), obstacleColor, ...
                "FaceAlpha", max(0.18, options.ObstacleFaceAlpha), ...
                "EdgeColor", obstacleColor, "LineStyle", "-", ...
            ...
            set(handles(regionIndex), ...
                ...
                "EdgeColor", obstacleColor);
        end
    end
    set(handles(regionIndex), "UserData", obstacleIndex);
end
end
