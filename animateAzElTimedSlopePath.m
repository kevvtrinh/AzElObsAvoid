...
if hasObstacleField && ~hasOriginalObstacleField
...
legend(axes3D, [trail3D current3D slopeArrow3D goalLineHandle], ...
    "Location", "best");
legend(axes2D, [trail2D current2D velocityArrow2D], ...
    "Location", "best");
...
for frameCursor = 1:numel(frameIndices)
    ...
    if hasObstacleField && options.ShowObstacles
        ...
        uistack([trail2D current2D velocityArrow2D], "top");
    end
