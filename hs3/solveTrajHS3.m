...
if isempty(solverResult)
    ...
    if hasDirectProgress
        ...
        requestedFinalTime = [];
        if options.TimeMode == "fixed"
            requestedFinalTime = finalTime;
        end
        progressProfile = ...
            hs3Internal.solver.createRestToRestJerkProfile( ...
            progressInitialState, progressTerminalState, progressLimits, ...
            requestedFinalTime);
        profileFitsHorizon = options.TimeMode == "fixed" || ...
            progressProfile.FinalTime < maximumFinalTime - ...
            options.ArrivalTimeTolerance;
        if progressProfile.Success && profileFitsHorizon
            solverResult = liftDirectJerkSwitchingProfile( ...
                progressProfile, displacement, initialState, terminalState, ...
                limits, options, pathConstraints, toc(directTimer));
        end
    end
end
canUseSynchronizedProfile = isempty(pathConstraints.Tau) && ...
    hasSymmetricKinematicBounds(limits) && isempty(solverResult);
if canUseSynchronizedProfile
    synchronizedTimer = tic;
    requestedFinalTime = [];
    if options.TimeMode == "fixed"
        requestedFinalTime = finalTime;
    end
    synchronizedProfile = ...
        hs3Internal.solver.createSynchronizedJerkProfile( ...
        initialState, terminalState, limits, requestedFinalTime);
    profileFitsHorizon = options.TimeMode == "fixed" || ...
        synchronizedProfile.FinalTime < maximumFinalTime - ...
        options.ArrivalTimeTolerance;
    if synchronizedProfile.Success && profileFitsHorizon
        solverResult = createSynchronizedJerkSwitchingSolverResult( ...
            synchronizedProfile, terminalState, limits, options, ...
            pathConstraints, toc(synchronizedTimer));
    elseif options.TimeMode == "fixed" && ...
            all(isfinite(synchronizedProfile.MinimumAxisDuration))
        minimumRequiredTime = startTime + ...
            max(synchronizedProfile.MinimumAxisDuration);
        if finalTime < minimumRequiredTime - options.ArrivalTimeTolerance
            failureMessage = sprintf( ...
                "Requested final time %.12g is below the certified minimum %.12g.", ...
                finalTime, minimumRequiredTime);
            solverResult = createAnalyticFailureSolverResult( ...
                "fixedTimeBelowMinimum", failureMessage, ...
                toc(synchronizedTimer));
        end
    end
end
if isempty(solverResult)
...
function solverResult = liftDirectJerkSwitchingProfile( ...
        progressProfile, displacement, initialState, terminalState, ...
        limits, options, pathConstraints, elapsedTime)
% Lift a scalar switching law and certify every original coordinate.
...
positionPower = scalarPolynomial.positionPower .* displacementScale;
positionPower(:, :, 1) = positionPower(:, :, 1) + ...
    initialState.position;
polynomial = scalarPolynomial;
polynomial.positionPower = positionPower;
...
controlJerk = progressProfile.ControlJerk * displacement;
...
if maximumConstraintViolation > feasibilityTolerance
    solverResult = [];
    return;
end

integratedSquaredJerk = progressProfile.IntegratedSquaredJerk * ...
    sum(displacement .^ 2);
solverOutput = struct( ...
    "algorithm", "closedFormJerkSwitching", ...
    "iterations", 0, ...
    "funcCount", 0);
solverResult = struct( ...
    "Success", true, ...
    "Message", "A certified direct jerk-switching profile was created.", ...
    "TerminationReason", "optimizerFeasible", ...
    "Decision", controlJerk(:), ...
    "ControlJerk", controlJerk, ...
    "Polynomial", polynomial, ...
    "FinalTime", progressProfile.FinalTime, ...
    "Objective", progressProfile.FinalTime, ...
    "ExitFlag", 1, ...
    "Output", solverOutput, ...
    ...
    "StageOneObjective", progressProfile.FinalTime, ...
...
function solverResult = createSynchronizedJerkSwitchingSolverResult( ...
        profile, terminalState, limits, options, pathConstraints, elapsedTime)
% Certify a dimension-neutral switching profile in the maintained solver schema.
...
solverOutput = struct( ...
    ...
    "iterations", 0, ...
    "funcCount", 0, ...
    "minimumAxisDuration", profile.MinimumAxisDuration, ...
    "axisPathLength", profile.AxisPathLength, ...
    "axisFamily", profile.AxisFamily);
solverResult = struct( ...
    "Success", true, ...
    "Message", "A certified synchronized jerk-switching profile was created.", ...
    "TerminationReason", "optimizerFeasible", ...
    "Decision", profile.ControlJerk(:), ...
    "ControlJerk", profile.ControlJerk, ...
    "Polynomial", polynomial, ...
    "FinalTime", profile.FinalTime, ...
    "Objective", profile.FinalTime, ...
    "ExitFlag", 1, ...
    "Output", solverOutput, ...
    "MaximumInequalityViolation", maximumInequalityViolation, ...
    "MaximumEqualityViolation", maximumEqualityViolation, ...
    ...
    "ErrorMessage", "", ...
    "ElapsedTime", elapsedTime, ...
    "IntegratedSquaredJerk", profile.IntegratedSquaredJerk, ...
    "StageOneObjective", profile.FinalTime, ...
    "StageTwoObjective", NaN, ...
    "StageTwoExitFlag", NaN, ...
    "StageTwoOutput", struct(), ...
    "StageTwoErrorIdentifier", "", ...
...
function value = hasSymmetricKinematicBounds(limits)
% Exact axis equations currently require symmetric derivative bounds.
scale = max([1, limits.maximumVelocity, ...
    limits.maximumAcceleration, limits.maximumJerk], [], 2);
tolerance = 128 * eps(scale);
value = all(abs(limits.velocityLower + limits.maximumVelocity) <= tolerance) && ...
    all(abs(limits.velocityUpper - limits.maximumVelocity) <= tolerance) && ...
    all(abs(limits.accelerationLower + limits.maximumAcceleration) <= tolerance) && ...
    all(abs(limits.accelerationUpper - limits.maximumAcceleration) <= tolerance) && ...
