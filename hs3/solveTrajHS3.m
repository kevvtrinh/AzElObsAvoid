...
[hasDirectProgress, progressInitialState, progressTerminalState, ...
    progressLimits, progressPathConstraints, displacement] = ...
    createDirectProgressProblem( ...
    initialState, terminalState, limits, options, pathConstraints);
if hasDirectProgress
    progressOptions = options;
    progressOptions.ConstraintTolerance = min( ...
        options.ConstraintTolerance, 1e-9);
    if options.TimeMode == "fixed"
        progressResult = hs3Internal.solver.solveFixedTime( ...
            progressInitialState, progressTerminalState, progressLimits, ...
            progressOptions, progressPathConstraints, finalTime);
    else
        progressResult = hs3Internal.solver.solveFreeTime( ...
            progressInitialState, progressTerminalState, progressLimits, ...
            progressOptions, progressPathConstraints, minimumFinalTime, ...
            maximumFinalTime);
    end
    if progressResult.Success
        solverResult = liftDirectProgressResult( ...
            progressResult, displacement, initialState, terminalState, ...
            limits, options, pathConstraints);
    end
end
...
function [isEligible, progressInitialState, progressTerminalState, ...
        ...
        createDirectProgressProblem( ...
        initialState, terminalState, limits, options, pathConstraints)
% Reduce an eligible direct rest-to-rest motion to scalar line progress.
% Fixed time can try any feasible direct line. Free time additionally requires
% one normalized axis to own every derivative limit, preserving minimum time.
dimensionCount = numel(initialState.position);
...
progressPathConstraints = struct();
isEligible = dimensionCount > 1 && isempty(pathConstraints.Tau);
positionScale = max([1, abs(initialState.position), ...
    abs(terminalState.position)]);
positionTolerance = 256 * eps(max(positionScale));
endpointDerivatives = [ ...
...
isEligible = isEligible && any(displacement ~= 0) && ...
    all(abs(endpointDerivatives) <= derivativeTolerance);
derivativeNames = ["velocity", "acceleration", "jerk"];
maximumNames = [ ...
    "maximumVelocity", "maximumAcceleration", "maximumJerk"];
for quantityIndex = 1:numel(derivativeNames)
    name = derivativeNames(quantityIndex);
    maximum = limits.(maximumNames(quantityIndex));
    lower = limits.(name + "Lower");
    upper = limits.(name + "Upper");
    symmetryTolerance = 256 * eps(max([1, maximum]));
    isEligible = isEligible && ...
        all(abs(lower + maximum) <= symmetryTolerance) && ...
        all(abs(upper - maximum) <= symmetryTolerance);
end
positionIsBounded = initialState.position >= ...
    limits.positionLower - positionTolerance & ...
    initialState.position <= limits.positionUpper + positionTolerance & ...
    terminalState.position >= limits.positionLower - positionTolerance & ...
    terminalState.position <= limits.positionUpper + positionTolerance;
isEligible = isEligible && all(positionIsBounded);
if ~isEligible
    return;
end
...
progressLimits = struct( ...
    ...
    "positionUpper", 1, ...
    "velocityLower", 0, ...
    "velocityUpper", progressMaximum(1), ...
    "accelerationLower", -progressMaximum(2), ...
    "accelerationUpper", progressMaximum(2), ...
    "jerkLower", -progressMaximum(3), ...
    "jerkUpper", progressMaximum(3));
progressPathConstraints = struct( ...
    "Tau", zeros(0, 1), ...
    "TauEnd", zeros(0, 1), ...
    "Normal", zeros(0, 1), ...
...
function solverResult = liftDirectProgressResult( ...
        progressResult, displacement, initialState, terminalState, ...
        limits, options, pathConstraints)
% Lift scalar jerk controls and recheck every original-dimensional constraint.
controlJerk = progressResult.Decision(:) * displacement;
solverResult = progressResult;
solverResult.Decision = controlJerk(:);
if options.TimeMode == "fixed"
    objectiveScale = sum(displacement .^ 2);
    solverResult.Objective = progressResult.Objective * objectiveScale;
end
[inequality, equality] = hs3Internal.constraints.evaluateConstraints( ...
    solverResult.Decision, false, solverResult.FinalTime, ...
    solverResult.FinalTime, solverResult.FinalTime, ...
    options.SegmentCount, initialState, terminalState, limits, ...
    pathConstraints);
solverResult.MaximumInequalityViolation = max([0; inequality(:)]);
solverResult.MaximumEqualityViolation = max([0; abs(equality(:))]);
solverResult.MaximumConstraintViolation = max( ...
    solverResult.MaximumInequalityViolation, ...
    solverResult.MaximumEqualityViolation);
feasibilityTolerance = max(10 * options.ConstraintTolerance, 1e-7);
if solverResult.MaximumConstraintViolation > feasibilityTolerance
    solverResult = [];
end
end
