...
[options, displayOptions] = resolveExampleOptions( ...
    ...
    "MaximumSeedCount", 5, "FigureVisible", "on", ...
    "Title", "U-shaped obstacle opening after 7 seconds"), [2.5 2.5]);

%% Section 2: Create Obstacles

% Two permanent side regions form the open U. A separate center gate has a
% finite active history ending at the opening time. This uses the documented
% active-span contract instead of implying vertex correspondence across a
% one-ring to two-ring topology change.
...
openSides = obstacleAvoidance.obstacles.createObstacle( ...
    "permanent open U sides", 0, openBoundary_deg(:, 1), ...
    openBoundary_deg(:, 2), safetyMargin_deg);
timedGate = obstacleAvoidance.obstacles.createObstacle( ...
    "bottom gate active through opening", [0; openingTime_s], ...
    gateBoundary_deg(:, 1), gateBoundary_deg(:, 2), safetyMargin_deg);
obstacles = obstacleAvoidance.obstacles.combineObstacles(openSides, timedGate);
