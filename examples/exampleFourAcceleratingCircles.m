...
% A 2 s history step limits repeated polygon inflation. Section 5 measures
% the center interpolation error against the protection margin.
obstacleSampleTime_s = 2.0;
...
circleVertexCount = 36;
circleAngle_rad = ((0:circleVertexCount - 1).' + 0.5) * ...
    (2 * pi / circleVertexCount);
% Circumscription prevents the polygon approximation from reducing the
% intended circular occupied region.
polygonRadius_deg = circleRadius_deg / cos(pi / circleVertexCount);
unitCircle = [cos(circleAngle_rad), sin(circleAngle_rad)];
circleCount = numel(circleCenterAzimuth_deg);
obstacleByCircle = cell(circleCount, 1);
for circleIndex = 1:circleCount
...
interpolationProbeTime_s = linspace( ...
    0, obstacleMotionDuration_s, 2001).';
probeNormalizedTime = interpolationProbeTime_s / ...
    obstacleMotionDuration_s;
probeTravelFraction = 10 * probeNormalizedTime.^3 - ...
    15 * probeNormalizedTime.^4 + 6 * probeNormalizedTime.^5;
exactProbeElevation_deg = initialCenterElevation_deg + ...
    circleTravel_deg * probeTravelFraction .* circleDirection;
linearProbeElevation_deg = interp1( ...
    obstacleTime_s, circleCenterElevation_deg, ...
    interpolationProbeTime_s, "linear");
maximumCenterInterpolationError_deg = max(abs( ...
    exactProbeElevation_deg - linearProbeElevation_deg), [], "all");
interpolationProtected = ...
    maximumCenterInterpolationError_deg < safetyMargin_deg;
...
circleMotionValidation = struct( ...
    "Passed", pairsRemainTangent && allCirclesTangentAtMidpoint && ...
        ...
        hasAccelerationAndDeceleration && interpolationProtected, ...
    ...
    "MaximumCenterInterpolationError_deg", ...
        maximumCenterInterpolationError_deg, ...
    "InterpolationProtected", interpolationProtected, ...
    "TangencyTolerance_deg", tangencyTolerance_deg);

targetTravel_deg = sum(vecnorm(diff(targetPosition_deg, 1, 1), 2, 2));
targetEndpointError_deg = norm( ...
    result.Intercept.TargetPosition_deg - targetPosition_deg(end, :));
targetSpeedAtIntercept_deg_s = norm( ...
    (targetPosition_deg(end, :) - targetPosition_deg(end - 1, :)) / ...
    (targetTime_s(end) - targetTime_s(end - 1)));
movingTargetValidation = struct( ...
    ...
    "PositionOnlyCapture", ...
    ~result.Intercept.Options.MatchTargetVelocity);
...
if ~movingTargetValidation.Passed
    exampleValidation.Passed = false;
    exampleValidation.Message = ...
        "Moving-target intercept validation failed.";
end
