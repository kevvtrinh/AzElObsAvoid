...
[options, jerkConfiguration] = resolveExampleOptions(exampleOverrides, struct("GoalTimeMode", "fixedArrival", "SampleTime_s", 0.05, "AllowAzimuthWrapping", false, "FigureVisible", "on", "Title", "Moving target among four accelerating circles"), [2.5 2.5]);
...
for circleIndex = 1:numel(circleCenterAzimuth_deg)
    ...
    obstacleByCircle{circleIndex} = obstacleAvoidance.obstacles.createObstacle("Accelerating circle " + circleIndex, obstacleTime_s, circleAzimuthByTime_deg, circleElevationByTime_deg, safetyMargin_deg);
end
...
initialState.position_deg        = [-10 0];
...
profileEndpointVelocity_deg_s      = circleTravel_deg * profileEndpointRate_1_s .* circleDirection;
profileEndpointAcceleration_deg_s2 = circleTravel_deg * profileEndpointAcceleration_1_s2 .* circleDirection;
zeroEndpointVelocity               = all(abs(profileEndpointVelocity_deg_s) <= 1e-12, "all");
zeroEndpointAcceleration           = all(abs(profileEndpointAcceleration_deg_s2) <= 1e-12, "all");
hasAccelerationAndDeceleration     = any(circleCenterAcceleration_deg_s2(2:midpointIndex - 1, 1) > 0) && any(circleCenterAcceleration_deg_s2(midpointIndex + 1:end, 1) < 0);
...
targetSpeedAtIntercept_deg_s = norm((targetPosition_deg(end, :) - targetPosition_deg(end - 1, :)) / (targetTime_s(end) - targetTime_s(end - 1)));
...
shortestRouteLength_deg    = norm(targetPosition_deg(end, :) - initialState.position_deg);
sampledRouteLength_deg     = sum(vecnorm(diff(result.position_deg, 1, 1), 2, 2));
maximumCenterLineError_deg = max(abs(result.position_deg(:, 2) - initialState.position_deg(2)));
shortestRouteTolerance_deg = 1e-6;
shortestRouteValidation    = struct("Passed", result.Success && ...
        abs(sampledRouteLength_deg - shortestRouteLength_deg) <= shortestRouteTolerance_deg && maximumCenterLineError_deg <= shortestRouteTolerance_deg, "TheoreticalMinimumLength_deg", shortestRouteLength_deg, "SampledRouteLength_deg", sampledRouteLength_deg, "MaximumCenterLineError_deg", maximumCenterLineError_deg, "Tolerance_deg", shortestRouteTolerance_deg);
