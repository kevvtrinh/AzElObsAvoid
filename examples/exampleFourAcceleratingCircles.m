...
%       kinematics, tangency checks, and optional plot handles. Ordinary
%       planning failure returns Success = false; invalid input throws.
%**************************************************************************
% UNITS
...
scenarioDefaults = struct( ...
    ...
    "WrapX",          false, ...
    "FigureVisible",  "on", ...
    "Title",          "Moving target among four accelerating circles");
...
travelAcceleration_1_s2 = (60 * normalizedTime - 180 * normalizedTime.^2 + ...
    120 * normalizedTime.^3) / obstacleMotionDuration_s^2;
...
for circleIndex = 1:numel(circleCenterX_units)
    ...
    for sampleIndex = 1:numel(obstacleTime_s)
        center_units = [circleCenterX_units(circleIndex), ...
            circleCenterY_units(sampleIndex, circleIndex)];
    ...
    obstacleByCircle{circleIndex} = obstacleAvoidance.obstacles.createObstacle( ...
        ...
        circleXByTime_units, circleYByTime_units, safetyMargin_units);
end
...
exampleValidation = validateExampleResult( ...
    result, "four accelerating circles", struct());
...
pairTangencyError_units     = abs(pairCenterDistance_units - 2 * circleRadius_units);
midpointTangencyError_units = abs(midpointCenterStep_units - 2 * circleRadius_units);
pairsRemainTangent          = all(pairTangencyError_units <= tangencyTolerance_units);
allCirclesTangentAtMidpoint = all(midpointTangencyError_units <= tangencyTolerance_units);

profileEndpointNormalizedTime        = [0; 1];
profileEndpointRate_1_s              = (30 * profileEndpointNormalizedTime.^2 - ...
    60 * profileEndpointNormalizedTime.^3 + 30 * profileEndpointNormalizedTime.^4) / ...
    obstacleMotionDuration_s;
profileEndpointAcceleration_1_s2     = (60 * profileEndpointNormalizedTime - ...
    180 * profileEndpointNormalizedTime.^2 + 120 * profileEndpointNormalizedTime.^3) / ...
...
hasAccelerationAndDeceleration       = any(circleCenterAcceleration_units_s2(2:midpointIndex - 1, 1) > 0) && ...
    any(circleCenterAcceleration_units_s2(midpointIndex + 1:end, 1) < 0);
...
circleMotionValidation = struct( ...
    ...
    "TangencyTolerance_units",        tangencyTolerance_units);

targetTravel_units               = sum(vecnorm(diff(targetPosition_units, 1, 1), 2, 2));
targetEndpointError_units        = norm(result.Diagnostics.Intercept.TargetPosition_units - targetPosition_units(end, :));
targetEndpointDisplacement_units = targetPosition_units(end, :) - targetPosition_units(end - 1, :);
targetEndpointDuration_s         = targetTime_s(end) - targetTime_s(end - 1);
targetSpeedAtIntercept_units_s   = norm(targetEndpointDisplacement_units / targetEndpointDuration_s);
movingTargetValidation = struct( ...
    "Passed",                         result.Success && result.Diagnostics.Validation.Passed && ...
        targetTravel_units > 0 && targetEndpointError_units <= 1e-10 && ...
        targetSpeedAtIntercept_units_s > 0, ...
    "InterceptPassed",                result.Success && result.Diagnostics.Validation.Passed, ...
    ...
    "PositionOnlyCapture",            result.Diagnostics.Intercept.TerminalVelocityPolicy == "zero");
...
shortestRouteValidation = struct( ...
    ...
    maximumCenterLineError_units <= shortestRouteTolerance_units, ...
    "TheoreticalMinimumLength_units", shortestRouteLength_units, ...
    "SampledRouteLength_units",       sampledRouteLength_units, ...
    "MaximumCenterLineError_units",   maximumCenterLineError_units, ...
    "Tolerance_units",                shortestRouteTolerance_units);
