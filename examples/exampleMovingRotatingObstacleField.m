...
[options, displayOptions] = resolveExampleOptions(exampleOverrides, struct("GoalTimeMode", "earliestArrival", "MaximumTimeLayerCount", 25, "SampleTime_s", 0.05, "AllowAzimuthWrapping", false, "Title", "Moving rotating obstacle with three static obstacles"), [4 4]);
...
for sampleIndex = 1:numel(movingTime_s)
    ...
    boundary_deg = movingBase_deg * rotation.' + movingCenter_deg(sampleIndex, :);
    movingAzimuthByTime_deg{sampleIndex} = boundary_deg(:, 1);
    movingElevationByTime_deg{sampleIndex} = boundary_deg(:, 2);
end
obstacleItems{4} = obstacleAvoidance.obstacles.createObstacle("Moving rotating obstacle", movingTime_s, movingAzimuthByTime_deg, movingElevationByTime_deg, safetyMargin_deg);
...
limits    = struct("azimuthInterval_deg", [-12 12], ...
...
exampleValidation      = validateExampleResult(result, "mixed moving and static obstacle field", struct("RequireDirectBlocked", true), diagnosis);
