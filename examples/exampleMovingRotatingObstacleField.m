...
%     derivatives use units/s, units/s^2, and units/s^3. Angles are radians.
%**************************************************************************
...
[options, displayOptions] = resolveExampleOptions( ...
    exampleOverrides, scenarioDefaults, [4 4]);

%% Section 2: Create Obstacles
% Place three fixed rectangles on the direct route. The fourth rectangle
% translates and rotates across the available detours.
missionEndTime_s           = 32;
...
for sampleIndex = 1:numel(movingTime_s)
    ...
    boundary_units = movingBase_units * rotationMatrix.' + movingCenter_units(sampleIndex, :);
...
% Keep the endpoints on opposite sides of the obstacle row and bound both
% detour directions within the displayed workspace.
initialState = struct('time_s', 0, 'position_units', [-10 0]);
goalState    = struct('time_s', missionEndTime_s, 'position_units', [10 0]);
limits       = struct( ...
    'xInterval_units',          [-12 12], ...
    'yInterval_units',          [-6 6], ...
...
% Run the maintained planner once with no route hints or retries.
result = planner(obstacles, initialState, goalState, limits, options);

%% Section 5: Validate Result
% Independently validate the motion and confirm that the dynamic obstacle
% actually translated and rotated during the request.
exampleValidation = validateExampleResult(result, ...
    "mixed moving and static obstacle field", ...
...
% Plot only the geometry and motion returned by the planner.
