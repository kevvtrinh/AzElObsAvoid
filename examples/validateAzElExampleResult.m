function validation = validateAzElExampleResult( result, scenarioLabel, requirements)
...
if ~isstruct(result) || ~isscalar(result) || ~isstruct(requirements) || ~isscalar(requirements)
    error("validateAzElExampleResult:InvalidInput", "result and requirements must be scalar structs.");
end
scenarioLabel = string(scenarioLabel);
if ~isscalar(scenarioLabel)
    error("validateAzElExampleResult:InvalidLabel", "scenarioLabel must be scalar text.");
end
...
expectedSuccess = azElInput.normalizeLogicalScalar( ...
    expectedSuccess, "ExpectedSuccess", "validateAzElExampleResult:InvalidExpectedSuccess");
requireDirectBlocked = azElInput.normalizeLogicalScalar( ...
    requireDirectBlocked, "RequireDirectBlocked", "validateAzElExampleResult:InvalidRequireDirectBlocked");
