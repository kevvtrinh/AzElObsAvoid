...
if ~isstruct(exampleOverrides) || ~isscalar(exampleOverrides)
    error("exampleAzElInteractiveSandbox:InvalidOverrides", ...
        "exampleOverrides must be a scalar struct.");
end

[plannerOverrides, sandboxOverrides] = separateSandboxOverrides( ...
    exampleOverrides);
...
[limits, sandboxControls] = resolveSandboxControls(sandboxOverrides, ...
    displayOptions.ConfiguredFiniteMaxJerk_deg_s3);

%% Section 2: Open Sandbox Canvas and Collect Geometry

sandboxFigure = figure( ...
...
axesHandle = axes(sandboxFigure, ...
    "Units", "normalized", ...
    "Position", [0.09 0.38 0.62 0.55]);
controlHandles = createSandboxControlPanel( ...
    sandboxFigure, sandboxControls, options.Verbose);
hold(axesHandle, "on");
grid(axesHandle, "on");
box(axesHandle, "on");
...
startPosition_deg = collectPoint(axesHandle, ...
    "Click the start point.");
if size(startPosition_deg, 1) ~= 1
    close(sandboxFigure);
    error("exampleAzElInteractiveSandbox:MissingStartPoint", ...
...
goalPosition_deg = collectPoint(axesHandle, ...
    "Click the goal point.");
if size(goalPosition_deg, 1) ~= 1
    close(sandboxFigure);
    error("exampleAzElInteractiveSandbox:MissingGoalPoint", ...
        "Goal point is required.");
end
plot(axesHandle, goalPosition_deg(:, 1), goalPosition_deg(:, 2), ...
    "ro", "MarkerFaceColor", "r", "DisplayName", "Goal", ...
    "LineWidth", 1.4);

[drawnLineCollection_deg, polygonObstaclePositions_deg] = ...
    collectFreehandObstacles(axesHandle, ...
    "Draw obstacles: hold left to trace; right-click when finished.");
[limits, sandboxControls, options] = readSandboxControlPanel( ...
    limits, sandboxControls, options, controlHandles);
...
for lineIndex = 1:numel(drawnLineCollection_deg)
    pathObstacleData{lineIndex} = pathToObstacleData( ...
        ...
        sandboxControls.PathObstacleRadius_deg, ...
        sandboxControls.PathSafetyMargin_deg);
end
...
result = runSandboxPlanner( ...
    obstacles, initialState, goalState, limits, options, controlHandles);
if ~result.Success
    fprintf("[Sandbox] %s (%s)\n", result.Message, ...
...
result.ExampleInputs = struct( ...
    ...
    "sandboxFigure", sandboxFigure);
result.SandboxFigure = sandboxFigure;
result.SandboxAxes = axesHandle;

%% Section 5: Local Functions

function [plannerOverrides, sandboxOverrides] = separateSandboxOverrides( ...
        overrides)
% PURPOSE
...
sandboxControlNames = [ ...
    ...
    "WorkspaceAzimuthInterval_deg", ...
    "WorkspaceElevationInterval_deg", ...
    "MaxVelocity_deg_s", ...
...
function [limits, controls] = resolveSandboxControls( ...
        sandboxOverrides, configuredMaxJerk_deg_s3)
% PURPOSE
%   - Apply defaults and validate sandbox-specific parameters.
if isempty(sandboxOverrides)
    sandboxOverrides = struct();
end
if isfield(sandboxOverrides, "MissionTime_s") && ...
        ~isempty(sandboxOverrides.MissionTime_s)
    missionTime_s = sandboxOverrides.MissionTime_s;
else
    missionTime_s = 180;
end
if isfield(sandboxOverrides, "MaxVelocity_deg_s") && ...
        ~isempty(sandboxOverrides.MaxVelocity_deg_s)
    maxVelocity_deg_s = sandboxOverrides.MaxVelocity_deg_s;
else
    maxVelocity_deg_s = [2 2];
end
if isfield(sandboxOverrides, "MaxAcceleration_deg_s2") && ...
        ~isempty(sandboxOverrides.MaxAcceleration_deg_s2)
    maxAcceleration_deg_s2 = ...
        sandboxOverrides.MaxAcceleration_deg_s2;
else
    maxAcceleration_deg_s2 = [0.75 0.75];
end
if isfield(sandboxOverrides, "PathObstacleRadius_deg") && ...
        ~isempty(sandboxOverrides.PathObstacleRadius_deg)
    pathObstacleRadius_deg = sandboxOverrides.PathObstacleRadius_deg;
else
    pathObstacleRadius_deg = 0.5;
end
if isfield(sandboxOverrides, "PathSafetyMargin_deg") && ...
        ~isempty(sandboxOverrides.PathSafetyMargin_deg)
    pathSafetyMargin_deg = sandboxOverrides.PathSafetyMargin_deg;
else
    pathSafetyMargin_deg = 0;
end
if isfield(sandboxOverrides, "ObstacleSafetyMargin_deg") && ...
        ~isempty(sandboxOverrides.ObstacleSafetyMargin_deg)
    obstacleSafetyMargin_deg = sandboxOverrides.ObstacleSafetyMargin_deg;
else
    obstacleSafetyMargin_deg = 0.2;
end
if isfield(sandboxOverrides, "WorkspaceAzimuthInterval_deg") && ...
        ~isempty(sandboxOverrides.WorkspaceAzimuthInterval_deg)
    workspaceAzimuthInterval_deg = ...
        sandboxOverrides.WorkspaceAzimuthInterval_deg;
else
...
if isfield(sandboxOverrides, "WorkspaceElevationInterval_deg") && ...
    ...
    workspaceElevationInterval_deg = ...
        sandboxOverrides.WorkspaceElevationInterval_deg;
else
    workspaceElevationInterval_deg = [-90 90];
end
validateattributes(missionTime_s, {'numeric'}, ...
    {'real', 'finite', 'scalar', 'positive'});
validateattributes(maxVelocity_deg_s, {'numeric'}, ...
    {'real', 'finite', 'vector', 'numel', 2, 'positive'});
validateattributes(maxAcceleration_deg_s2, {'numeric'}, ...
    {'real', 'finite', 'vector', 'numel', 2, 'positive'});
validateattributes(pathObstacleRadius_deg, {'numeric'}, ...
    {'real', 'finite', 'scalar', 'positive'});
validateattributes(pathSafetyMargin_deg, {'numeric'}, ...
    {'real', 'finite', 'scalar', 'nonnegative'});
validateattributes(obstacleSafetyMargin_deg, {'numeric'}, ...
    {'real', 'finite', 'scalar', 'nonnegative'});
validateattributes(workspaceAzimuthInterval_deg, {'numeric'}, ...
    {'real', 'finite', 'vector', 'numel', 2, 'increasing'});
validateattributes(workspaceElevationInterval_deg, {'numeric'}, ...
    {'real', 'finite', 'vector', 'numel', 2, 'increasing'});
...
function controlHandles = createSandboxControlPanel( ...
        figureHandle, controls, isVerbose)
% PURPOSE
...
controlHandles = struct( ...
    ...
    "JerkHandles", addSandboxAxisPairControl(panelHandle, ...
        "Maximum jerk (deg/s^3)", 0.50, controls.MaxJerk_deg_s3));
verboseHandle = uicontrol(panelHandle, ...
    "Style", "checkbox", ...
    "String", "Verbose planner output", ...
    "Units", "normalized", ...
    "Position", [0.08 0.35 0.84 0.05], ...
    "Value", logical(isVerbose), ...
    "HorizontalAlignment", "left");
controlHandles.VerboseHandle = verboseHandle;
logPanelHandle = uipanel(figureHandle, ...
    "Title", "Planner log", ...
    "Units", "normalized", ...
    "Position", [0.09 0.05 0.88 0.26]);
controlHandles.VerboseLogHandle = uicontrol(logPanelHandle, ...
    "Style", "listbox", ...
    "String", {"Verbose output will appear here."}, ...
    "Units", "normalized", ...
    "Position", [0.02 0.06 0.96 0.90], ...
    "HorizontalAlignment", "left", ...
    "Max", 2, ...
    "Min", 0);
end

function editHandle = addSandboxScalarControl( ...
        panelHandle, labelText, topPosition, defaultText)
% PURPOSE
...
uicontrol(panelHandle, ...
    ...
    "String", labelText, ...
    "Units", "normalized", ...
    "Position", [0.08 topPosition 0.84 0.04], ...
    "HorizontalAlignment", "left");
editHandle = uicontrol(panelHandle, ...
    "Style", "edit", ...
    "String", defaultText, ...
    "Units", "normalized", ...
    "Position", [0.08 topPosition - 0.055 0.84 0.05], ...
    "HorizontalAlignment", "left");
end

function controlHandles = addSandboxAxisPairControl( ...
        panelHandle, labelText, topPosition, defaultValues)
% PURPOSE
...
uicontrol(panelHandle, ...
    ...
    "String", labelText, ...
    "Units", "normalized", ...
    "Position", [0.08 topPosition 0.84 0.035], ...
    "HorizontalAlignment", "left");
uicontrol(panelHandle, ...
    "Style", "text", ...
    "String", "Azimuth", ...
    "Units", "normalized", ...
    "Position", [0.08 topPosition - 0.04 0.39 0.025], ...
    "HorizontalAlignment", "left");
uicontrol(panelHandle, ...
    "Style", "text", ...
    "String", "Elevation", ...
    "Units", "normalized", ...
    "Position", [0.53 topPosition - 0.04 0.39 0.025], ...
    "HorizontalAlignment", "left");
controlHandles = struct( ...
    ...
    "ElevationHandle", uicontrol(panelHandle, ...
        ...
        "Units", "normalized", ...
        "Position", [0.53 topPosition - 0.09 0.39 0.045]));
end

function [limits, controls, options] = readSandboxControlPanel( ...
        limits, controls, options, controlHandles)
% PURPOSE
...
validateattributes(missionTime_s, {'numeric'}, ...
    {'real', 'finite', 'scalar', 'positive'}, ...
    "exampleAzElInteractiveSandbox", "Mission horizon (s)");
maxVelocity_deg_s = readAxisPairControl( ...
    controlHandles.VelocityHandles, "Maximum velocity (deg/s)");
maxAcceleration_deg_s2 = readAxisPairControl( ...
    controlHandles.AccelerationHandles, "Maximum acceleration (deg/s^2)");
maxJerk_deg_s3 = readAxisPairControl( ...
    controlHandles.JerkHandles, "Maximum jerk (deg/s^3)");
...
values = [ ...
    str2double(get(controlHandles.AzimuthHandle, "String")), ...
    str2double(get(controlHandles.ElevationHandle, "String"))];
validateattributes(values, {'numeric'}, ...
    {'real', 'finite', 'vector', 'numel', 2, 'positive'}, ...
    "exampleAzElInteractiveSandbox", fieldName);
values = reshape(values, 1, 2);
end

function result = runSandboxPlanner( ...
        obstacles, initialState, goalState, limits, options, controlHandles)
% PURPOSE
...
if options.Verbose
    ...
    set(controlHandles.VerboseLogHandle, "String", ...
...
function [lineCollection_deg, polygonCollection_deg] = ...
        collectFreehandObstacles(axesHandle, prompt)
% PURPOSE
...
traceHandle = plot(axesHandle, NaN, NaN, "b-", ...
    "LineWidth", 1.2, "HandleVisibility", "off");
previousCallbacks = struct( ...
    "Down", get(figureHandle, "WindowButtonDownFcn"), ...
    "Motion", get(figureHandle, "WindowButtonMotionFcn"), ...
    "Up", get(figureHandle, "WindowButtonUpFcn"));
...
restoreFigureCallbacks();
    ...
    function startTrace(~, ~)
        ...
        isCanvasClick = isequal(clickedHandle, axesHandle) || ...
            isequal(clickedAxesHandle, axesHandle);
        if ~isCanvasClick
            return;
        end
    ...
    function finishTrace(~, ~)
        ...
        elseif size(simplifiedStroke_deg, 1) >= 3
            ...
            fill(axesHandle, simplifiedStroke_deg(:, 1), ...
                ...
                "FaceAlpha", 0.18, "EdgeColor", [0.2 0.3 0.9], ...
                "HandleVisibility", "off");
        end
        stroke_deg = zeros(0, 2);
    ...
    function appendCursorPoint()
        ...
        if ~isempty(stroke_deg) && ...
                norm(cursorPosition_deg - stroke_deg(end, :)) < ...
                minimumTraceSpacing_deg
            return;
        end
        stroke_deg(end + 1, :) = cursorPosition_deg;
        set(traceHandle, "XData", stroke_deg(:, 1), ...
            "YData", stroke_deg(:, 2));
        drawnow("limitrate");
    end
...
function simplified_deg = simplifyFreehandBoundary( ...
        points_deg, tolerance_deg)
% PURPOSE
...
if lineLength_deg <= eps
    distance_deg = vecnorm( ...
        interiorPoints_deg - points_deg(1, :), 2, 2);
else
...
firstHalf_deg = simplifyFreehandBoundary( ...
    points_deg(1:splitIndex, :), tolerance_deg);
secondHalf_deg = simplifyFreehandBoundary( ...
    points_deg(splitIndex:end, :), tolerance_deg);
simplified_deg = [firstHalf_deg(1:end - 1, :); secondHalf_deg];
end

function pathObstacleData = pathToObstacleData( ...
        path_deg, time_s, radius_deg, safetyMargin_deg)
% PURPOSE
...
pathObstacleData = {makeAzElObstacleData( ...
    "drawn path obstacle", time_s, ...
    bufferVertices_deg(:, 1), bufferVertices_deg(:, 2), ...
    safetyMargin_deg)};
end
...
for segmentIndex = 1:size(path_deg, 1) - 1
    ...
    startAngles_rad = linspace( ...
        directionAngle_rad + pi / 2, directionAngle_rad + 3 * pi / 2, ...
        endCapSegmentCount + 1).';
    endAngles_rad = linspace( ...
        directionAngle_rad - pi / 2, directionAngle_rad + pi / 2, ...
        endCapSegmentCount + 1).';
    startCap_deg = startPosition_deg + constructionRadius_deg * ...
        [cos(startAngles_rad), sin(startAngles_rad)];
    endCap_deg = endPosition_deg + constructionRadius_deg * ...
        [cos(endAngles_rad), sin(endAngles_rad)];
...
for polygonIndex = 1:numel(polygonCollection_deg)
    ...
    if ~isequal(polygon_deg(1, :), polygon_deg(end, :))
        ...
        polygon_deg = vertcat( ...
            polygon_deg, polygon_deg(1, :)); %#ok<AGROW>
    end
