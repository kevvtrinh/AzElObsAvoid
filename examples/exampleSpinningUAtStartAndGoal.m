...
scenarioDefaults = struct( ...
    ...
    "TemporalResolution_s", 1.0, ...
    "SampleTime_s",         0.05, ...
    "WrapX",                false, ...
    "Title",                "Spinning U at the start and goal");
[options, displayOptions] = resolveExampleOptions(exampleOverrides, scenarioDefaults, [8 8]);
...
% convex bars represent each U, so every between-sample rotation retains exact
% vertex correspondence instead of replacing a concave sweep by a hull.
...
initialState = struct( ...
    "time_s",                  0, ...
    "position_units",          startCenter_units, ...
    "velocity_units_s",        [0 0], ...
    "acceleration_units_s2",   [0 0]);

goalState = struct( ...
    "time_s",                  missionEndTime_s, ...
    "position_units",          goalCenter_units, ...
    "velocity_units_s",        [0 0], ...
    "acceleration_units_s2",   [0 0]);

limits = struct( ...
    "xInterval_units",          [-13 13], ...
    "yInterval_units",          [-8 8], ...
    "maxVelocity_units_s",      [4 4], ...
    "maxAcceleration_units_s2", [4 4], ...
...
function [obstacleItems, histories] = createSpinningUComponents( ...
        name, bars_units, center_units, angle_rad, time_s, safetyMargin_units)
...
function validation = validateSpinningUs( ...
        startHistory, goalHistory, startCenter_units, goalCenter_units, ...
        spinAngle_rad, obstacles, missionEndTime_s)
    ...
    startRadiiPreserved = all(cellfun( ...
        @(history) historyPreservesRadius(history, startCenter_units, radiusTolerance_units), ...
        startHistory));
    radiiPreserved = startRadiiPreserved && all(cellfun( ...
        @(history) historyPreservesRadius(history, goalCenter_units, radiusTolerance_units), ...
        goalHistory));
