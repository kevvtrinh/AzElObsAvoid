...
squarePosition_units = squareCenter_units + [ ...
    -squareHalfWidth_units, -squareHalfWidth_units; ...
     squareHalfWidth_units, -squareHalfWidth_units; ...
     squareHalfWidth_units,  squareHalfWidth_units; ...
    -squareHalfWidth_units,  squareHalfWidth_units];
squareObstacle = obstacleAvoidance.obstacles.createObstacle( ...
    "Square", obstacleTime_s, ...
    {squarePosition_units(:, 1); squarePosition_units(:, 1)}, ...
    {squarePosition_units(:, 2); squarePosition_units(:, 2)}, safetyMargin_units);
...
circleObstacle = obstacleAvoidance.obstacles.createObstacle( ...
    ...
    {circlePosition_units(:, 1); circlePosition_units(:, 1)}, ...
    {circlePosition_units(:, 2); circlePosition_units(:, 2)}, safetyMargin_units);
...
starObstacle = obstacleAvoidance.obstacles.createObstacle( ...
    ...
    {starPosition_units(:, 1); starPosition_units(:, 1)}, ...
    {starPosition_units(:, 2); starPosition_units(:, 2)}, safetyMargin_units);

uCenter_units   = [9 0];
uPosition_units = uCenter_units + [ ...
    -2.0,  2.0; ...
    -1.2,  2.0; ...
    -1.2, -1.2; ...
     1.2, -1.2; ...
...
uObstacle = obstacleAvoidance.obstacles.createObstacle( ...
    ...
    {uPosition_units(:, 1); uPosition_units(:, 1)}, ...
    {uPosition_units(:, 2); uPosition_units(:, 2)}, safetyMargin_units);
...
specifiedInterceptTime_s  = missionEndTime_s * ...
    (specifiedInterceptX_units - targetPosition_units(1, 1)) / diff(targetPosition_units(:, 1));
...
exampleValidation    = validateExampleResult( ...
    result, "straight target with alternating occlusion", validationOptions);
...
[targetOccupied, blockingObstacleIndex] = ...
    obstacleAvoidance.obstacles.queryObstacleOccupancyAtTime( ...
    result.Inputs.obstacles, targetX_units, targetY_units, ...
    occupancyTime_s, obstacleQueryOptions);
...
probeTime_s     = missionEndTime_s * ...
    (probeX_units - targetPosition_units(1, 1)) / diff(targetPosition_units(:, 1));
[probeOccupied, probeBlockingObstacleIndex] = ...
    obstacleAvoidance.obstacles.queryObstacleOccupancyAtTime( ...
    result.Inputs.obstacles, probeX_units, probeY_units, ...
    probeTime_s, obstacleQueryOptions);
...
interShapeGapBounds_units = [ ...
    max(squarePosition_units(:, 1)) + safetyMargin_units, ...
    circleCenter_units(1) - circleRadius_units - safetyMargin_units; ...
    circleCenter_units(1) + circleRadius_units + safetyMargin_units, ...
    starCenter_units(1) - starOuterRadius_units - safetyMargin_units; ...
    starCenter_units(1) + starOuterRadius_units + safetyMargin_units, ...
    min(uPosition_units(:, 1)) - safetyMargin_units];
interceptInInterShapeGap = false;
interceptTargetIsClear   = false;
if result.Success
    interceptX_units          = result.Diagnostics.Intercept.TargetPosition_units(1);
    interceptInInterShapeGap = any(interceptX_units > interShapeGapBounds_units(:, 1) & ...
        interceptX_units < interShapeGapBounds_units(:, 2));
    interceptTargetIsClear = ...
        ~obstacleAvoidance.obstacles.queryObstacleOccupancyAtTime( ...
...
catchOccurredBeforeTrackEnd = result.Success && result.Diagnostics.Intercept.Time_s < missionEndTime_s;
...
scenarioPassed              = targetTrackIsStraight && alternationIsPresent && ...
    shapeProbesPassed && boresightIsFaster && interceptInInterShapeGap && ...
    interceptTargetIsClear && catchOccurredBeforeTrackEnd;
