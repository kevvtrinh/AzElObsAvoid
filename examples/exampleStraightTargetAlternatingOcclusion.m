...
[options, jerkConfiguration] = resolveAzElExampleOptions( ...
    exampleOverrides, struct( ...
        ...
        "Verbose", true, ...
        "FigureVisible", "on", ...
        "Title", "Straight target with alternating occlusion"), ...
    [2.5 2.5]);
...
squareObstacle = makeAzElObstacleData( ...
    ...
    {squarePosition_deg(:, 2); squarePosition_deg(:, 2)}, ...
    safetyMargin_deg);
...
circlePosition_deg = circleCenter_deg + circleRadius_deg * ...
    [cos(circleAngle_rad), sin(circleAngle_rad)];
circleObstacle = makeAzElObstacleData( ...
    "Circle", obstacleTime_s, ...
    {circlePosition_deg(:, 1); circlePosition_deg(:, 1)}, ...
    {circlePosition_deg(:, 2); circlePosition_deg(:, 2)}, ...
    safetyMargin_deg);
...
starAngle_rad = (0:starVertexCount - 1).' * ...
    (2 * pi / starVertexCount);
starRadius_deg = repmat( ...
    [starOuterRadius_deg; starInnerRadius_deg], starPointCount, 1);
starPosition_deg = starCenter_deg + starRadius_deg .* ...
    [cos(starAngle_rad), sin(starAngle_rad)];
starObstacle = makeAzElObstacleData( ...
    "12-point star", obstacleTime_s, ...
    {starPosition_deg(:, 1); starPosition_deg(:, 1)}, ...
    {starPosition_deg(:, 2); starPosition_deg(:, 2)}, ...
    safetyMargin_deg);

uCenter_deg = [9 0];
uPosition_deg = uCenter_deg + [ ...
    -2.0  2.0; ...
    -1.2  2.0; ...
    -1.2 -1.2; ...
     1.2 -1.2; ...
...
uObstacle = makeAzElObstacleData( ...
    ...
    {uPosition_deg(:, 1); uPosition_deg(:, 1)}, ...
    {uPosition_deg(:, 2); uPosition_deg(:, 2)}, ...
    safetyMargin_deg);

obstacles = combineAzElObstacles( ...
    squareObstacle, circleObstacle, starObstacle, uObstacle);
...
specifiedInterceptTime_s = missionEndTime_s * ...
    (specifiedInterceptAzimuth_deg - targetPosition_deg(1, 1)) / ...
    diff(targetPosition_deg(:, 1));
...
interceptOptions = struct( ...
    ...
    "SpecifiedInterceptTime_s", specifiedInterceptTime_s, ...
    "MatchTargetVelocity", false, ...
    "TargetTrackSampleCount", 241, ...
...
exampleValidation = validateAzElExampleResult( ...
    result, "straight target with alternating occlusion", ...
    struct("RequireDirectBlocked", true));

occupancySampleCount = 1201;
occupancyTime_s = linspace(0, missionEndTime_s, ...
    occupancySampleCount).';
targetAzimuth_deg = interp1(targetTime_s, ...
    targetPosition_deg(:, 1), occupancyTime_s, "linear");
targetElevation_deg = interp1(targetTime_s, ...
    targetPosition_deg(:, 2), occupancyTime_s, "linear");
[targetOccupied, blockingObstacleIndex] = queryAzElTimeObstacle( ...
    result.obstacleField, targetAzimuth_deg, targetElevation_deg, ...
    occupancyTime_s);
...
probeTime_s = missionEndTime_s * ...
    (probeAzimuth_deg - targetPosition_deg(1, 1)) / ...
    diff(targetPosition_deg(:, 1));
[probeOccupied, probeBlockingObstacleIndex] = queryAzElTimeObstacle( ...
    result.obstacleField, probeAzimuth_deg, probeElevation_deg, ...
    probeTime_s);
...
targetVelocity_deg_s = diff(targetPosition_deg, 1, 1) ./ ...
    diff(targetTime_s);
targetSpeed_deg_s = norm(targetVelocity_deg_s);
boresightIsFaster = targetSpeed_deg_s < ...
    min(limits.maxVelocity_deg_s);
...
interShapeGapBounds_deg = [ ...
    ...
    starCenter_deg(1) + starOuterRadius_deg + safetyMargin_deg, ...
        min(uPosition_deg(:, 1)) - safetyMargin_deg];
interceptInInterShapeGap = false;
interceptTargetIsClear = false;
if result.Success
    interceptAzimuth_deg = result.TargetPositionAtIntercept_deg(1);
    interceptInInterShapeGap = any( ...
        interceptAzimuth_deg > interShapeGapBounds_deg(:, 1) & ...
        interceptAzimuth_deg < interShapeGapBounds_deg(:, 2));
    interceptTargetIsClear = ~queryAzElTimeObstacle( ...
        result.obstacleField, result.TargetPositionAtIntercept_deg(1), ...
        result.TargetPositionAtIntercept_deg(2), result.InterceptTime_s);
end
catchOccurredBeforeTrackEnd = result.Success && ...
    result.InterceptTime_s < missionEndTime_s;
alternationIsPresent = targetOccupied(1) && ~targetOccupied(end) && ...
    blockedRunCount >= 5 && clearRunCount >= 5 && ...
    occupancyTransitionCount >= 9;
shapeProbesPassed = isequal(probeOccupied, expectedProbeOccupied) && ...
    isequal(probeBlockingObstacleIndex, ...
    expectedProbeBlockingObstacleIndex);
scenarioValidation = struct( ...
    ...
    "ProbeOccupied", probeOccupied, ...
    "ProbeBlockingObstacleIndex", probeBlockingObstacleIndex);

if ~scenarioValidation.Passed
    exampleValidation.Passed = false;
    exampleValidation.Message = ...
        "Straight-target blocked/free sequence validation failed.";
end
