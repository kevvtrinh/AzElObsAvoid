...
[options, displayOptions] = resolveAzElExampleOptions( ...
    ...
    "CollocationSegmentCount", 7, "MaximumPlanningTime_s", 120, ...
...
waitBehaviorPassed = false;
barrierCrossingTime_s = NaN;
maximumDetourElevation_deg = NaN;
if result.Success
    [~, crossingIndex] = min(abs(result.position_deg(:, 1)));
    barrierCrossingTime_s = result.time_s(crossingIndex);
    maximumDetourElevation_deg = max(abs(result.position_deg(:, 2)));
    staticDetourElevation_deg = max(abs(sourcePosition_deg(:, 2))) + ...
        safetyMargin_deg;
    waitBehaviorPassed = ...
        barrierCrossingTime_s > obstacleTime_s(2) && ...
        maximumDetourElevation_deg < staticDetourElevation_deg;
end
result.ExampleValidation.WaitBehaviorPassed = waitBehaviorPassed;
result.ExampleValidation.BarrierCrossingTime_s = barrierCrossingTime_s;
result.ExampleValidation.MaximumDetourElevation_deg = ...
    maximumDetourElevation_deg;
result.ExampleValidation.Passed = ...
    result.ExampleValidation.Passed && waitBehaviorPassed;
if ~waitBehaviorPassed
    result.ExampleValidation.Message = ...
        result.ExampleValidation.Message + ...
        " The motion did not use the barrier opening instead of a full detour.";
end
