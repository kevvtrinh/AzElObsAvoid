...
%**************************************************************************
% PURPOSE
%   - Exercise bounded extreme visibility candidates for the dense static
%     outlines of Hawaii, Croatia, and the Philippines.
%   - Preserve validated success or bounded failure without a no-path claim.
%**************************************************************************
% INPUTS
...
%       Final-region bounded-failure result plus ExampleValidation,
...
expectedSuccess = [false; true; false];
regionPlanningTime_s = [30; 60; 30];
...
for regionIndex = 1:regionCount
    ...
    regionOptions.MaximumPlanningTime_s = ...
        regionPlanningTime_s(regionIndex);
...
for regionIndex = 1:regionCount
    ...
    exampleValidation = validateAzElExampleResult( ...
        ...
        struct( ...
        "ExpectedSuccess", expectedSuccess(regionIndex), ...
        "RequireDirectBlocked", true));
...
result.RegionSequenceSummary = table( ...
    regionNames.', expectedSuccess, regionPassed, regionArrivalTime_s, ...
    regionRouteLength_deg, regionNativeVertexCount, ...
    regionTestVertexCount, ...
    'VariableNames', {'Region', 'ExpectedSuccess', 'Passed', ...
    'ArrivalTime_s', ...
