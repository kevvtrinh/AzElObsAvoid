...
%       MaxJerk_deg_s3 controls. RandomSeed selects the deterministic
...
targetOccupied = queryAzElTimeObstacle( ...
    result.obstacleField, targetPosition_deg(:, 1), ...
...
if result.Success && all(isfinite(result.TargetPositionAtIntercept_deg))
...
targetFrameFailureReported = ~result.Success && ...
    result.TerminationReason == "targetLeftAzElFrame" && ...
    result.TargetLeftFrameBeforeIntercept && ...
    contains(result.Message, "Cannot catch up");
...
scenarioValidation = struct( ...
    "Passed", targetStartsInside && targetWaitedInside && ...
        ...
        targetTravel_deg > 0 && ...
        (successfulInterceptValidated || targetFrameFailureReported), ...
    ...
    "TargetFrameFailureReported", targetFrameFailureReported, ...
...
if targetFrameFailureReported
    exampleValidation.Passed = scenarioValidation.Passed;
    exampleValidation.Message = ...
        "Target left the az/el frame before a feasible intercept, and the " + ...
        "planner returned the expected cannot-catch-up failure.";
elseif ~scenarioValidation.Passed
