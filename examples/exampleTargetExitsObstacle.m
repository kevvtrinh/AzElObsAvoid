...
[options, jerkConfiguration] = resolveExampleOptions(plannerOverrides, struct("GoalTimeMode", "fixedArrival", "SampleTime_s", 0.05, "AllowAzimuthWrapping", false, "FigureVisible", "on", "Title", "Target exits a containing obstacle"), [2.5 2.5]);
...
transitCircle             = obstacleAvoidance.obstacles.createObstacle("Transit circle", obstacleTime_s, {transitCirclePosition_deg(:, 1); transitCirclePosition_deg(:, 1)}, {transitCirclePosition_deg(:, 2); transitCirclePosition_deg(:, 2)}, safetyMargin_deg);
...
containingCircle             = obstacleAvoidance.obstacles.createObstacle("Target containment circle", obstacleTime_s, {containingCirclePosition_deg(:, 1); containingCirclePosition_deg(:, 1)}, {containingCirclePosition_deg(:, 2); containingCirclePosition_deg(:, 2)}, safetyMargin_deg);
...
targetOccupied        = obstacleAvoidance.obstacles.queryObstacleOccupancyAtTime(result.Inputs.obstacles, targetPosition_deg(:, 1), targetPosition_deg(:, 2), targetTime_s, obstacleQueryOptions);
...
targetWaitedInside    = all(targetOccupied(1:targetWaitSampleCount)) && all(targetPosition_deg(1:targetWaitSampleCount, :) == containingCircleCenter_deg, "all");

targetIsClearAtIntercept = false;
if result.Success && all(isfinite(result.Intercept.TargetPosition_deg))
    targetIsClearAtIntercept = ~obstacleAvoidance.obstacles.queryObstacleOccupancyAtTime(result.Inputs.obstacles, result.Intercept.TargetPosition_deg(1), result.Intercept.TargetPosition_deg(2), result.Intercept.Time_s, obstacleQueryOptions);
end

transitCircleIsBetween       = initialState.position_deg(1) < transitCircleCenter_deg(1) && transitCircleCenter_deg(1) < containingCircleCenter_deg(1);
