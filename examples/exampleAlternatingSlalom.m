...
% Three narrow rectangles cross the direct route. Their centers alternate above
...
barrierOffset_units = [-0.7 -2.5; 0.7 -2.5; 0.7 2.5; -0.7 2.5];
...
for obstacleIndex = 1:numel(centerX_units)
    ...
    rectangle_units = center_units + barrierOffset_units;
    obstacle = obstacleAvoidance.obstacles.createObstacle( ...
        "barrier " + obstacleIndex, obstacleTime_s, ...
        rectangle_units(:, 1), rectangle_units(:, 2), 0.1);
...
initialState = struct("time_s", 0, "position_units", [-8 0]);
goalState    = struct("time_s", 22, "position_units", [8 0]);
limits       = struct( ...
