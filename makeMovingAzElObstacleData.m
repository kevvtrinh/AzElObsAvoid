...
%   - Evaluate independent time slices with parfor when requested and
%     available, with a serial fallback that has identical outputs.
...
%       Source slice boundaries, geometry metrics, and parallel diagnostics.
%**************************************************************************
% UNITS
...
defaultOptions = struct("UseParallel", "auto", "Verbose", false);
...
useParallelOption = resolvedOptions.UseParallel;
...
[useParallel, parallelExecution] = resolveAzElParallelExecution( ...
    useParallelOption, verbose, "moving obstacle slice generation", ...
    sliceCount);

if useParallel && sliceCount > 1
    progressQueue = parallel.pool.DataQueue;
    if verbose
        afterEach(progressQueue, @printCompletedSlice);
    end
    parfor sampleIndex = 1:sliceCount
...
constructionOptions = struct( ...
    "Verbose", verbose, "UseParallel", useParallelOption);
