...
applicationState = initializeApplicationState(figureHandle, options, goalHandles);
...
function options = resolveSandboxOptions(overrides)
    ...
    [options, unknownNames] = obstacleAvoidance.input.resolveOptions(sandboxDefaults(), overrides);
    if ~isempty(unknownNames)
        warning("obstacleAvoidanceSandbox:UnknownOptions", "Ignoring unknown sandbox fields: %s. No behavior changed.", strjoin(unknownNames, ", "));
    end
    ...
    options.Verbose      = obstacleAvoidance.input.normalizeLogicalScalar(options.Verbose, "Verbose", "obstacleAvoidanceSandbox:InvalidLogicalOption");
    ...
    for name = countNames
        validateattributes(options.(name), {'numeric'}, {'real', 'finite', 'scalar', 'integer', 'nonnegative'}, "obstacleAvoidanceSandbox", name);
    end
...
function handles = createGoalControls(tabHandle, options)
    ...
    controls.VerboseHandle       = uicontrol(tabHandle, "Style", "checkbox", "String", "Verbose output", "Units", "normalized", "Position", [0.882 0.263 0.103 0.03], "Value", options.Verbose, "HorizontalAlignment", "left");
    controls.MotionProfileHandle = uicontrol(tabHandle, "Style", "popupmenu", "String", { "Non-zero velocity from start", "Zero velocity from start", "Trapezoidal", "Oscillating"}, "Units", "normalized", "Position", [0.71 0.263 0.165 0.03], "Value", 1, "TooltipString", "Motion profile used by Set Motion for the selected polygon.");
    addPanelHandle    = uipanel(tabHandle, "Title", "Add", "Units", "normalized", "Position", [0.01 0.268 0.24 0.095]);
    ...
    actionPanelHandle = uipanel(tabHandle, "BorderType", "none", "Units", "normalized", "Position", [0.265 0.275 0.42 0.052]);
    actionNames       = [ ...
        "SetMotion", "Run", "Reset", ...
        "Diagnostics", "Export"];
    actionLabels = [ ...
    ...
    statusPanelHandle         = uipanel(tabHandle, "Title", "Mode status and planner log", "Units", "normalized", "Position", [0.045 0.025 0.64 0.23]);
    ...
    controls.WrapXHandle = uicontrol(plannerOptionsPanelHandle, "Style", "checkbox", "String", "Wrap x", "Units", "normalized", "Position", [0.05 0.08 0.43 0.18], "Value", options.PlannerOptions.WrapX, "HorizontalAlignment", "left", "TooltipString", "Use the x workspace interval as its periodic domain.");
    controls.WrapYHandle = uicontrol(plannerOptionsPanelHandle, "Style", "checkbox", "String", "Wrap y", "Units", "normalized", "Position", [0.52 0.08 0.43 0.18], "Value", options.PlannerOptions.WrapY, "HorizontalAlignment", "left", "TooltipString", "Use the y workspace interval as its periodic domain.");
...
function addControlSectionLabel(panelHandle, labelText, rowPosition)
    ...
    uicontrol(panelHandle, "Style", "text", "String", labelText, "Units", "normalized", "Position", [0.05 rowPosition 0.91 0.045], "HorizontalAlignment", "left", "FontWeight", "bold");
end
...
function addPairColumnLabels(panelHandle, firstLabel, secondLabel, rowPosition)
    ...
    for axisIndex = 1:2
        ...
        uicontrol(panelHandle, "Style", "text", "String", columnLabels(axisIndex), "Units", "normalized", "Position", [leftPosition rowPosition 0.20 0.035], "HorizontalAlignment", "center");
    end
end
...
function editHandle = addScalarControl(panelHandle, labelText, rowPosition, value)
    ...
    uicontrol(panelHandle, "Style", "text", "String", labelText, "Units", "normalized", "Position", [0.05 rowPosition 0.68 0.045], "HorizontalAlignment", "left");
    editHandle = uicontrol(panelHandle, "Style", "edit", "String", sprintf("%.8g", value), "Units", "normalized", "Position", [0.76 rowPosition 0.20 0.045]);
end
...
function applicationState = initializeApplicationState(figureHandle, options, goalHandles)
...
function popupHandle = addPopupControl(panelHandle, labelText, rowPosition, choices, selectedIndex, tooltip)
    % Add one labeled planner-choice popup with a stable string-to-index mapping.
    uicontrol(panelHandle, "Style", "text", "String", labelText, "Units", "normalized", "Position", [0.05 rowPosition 0.44 0.18], "HorizontalAlignment", "left");
    popupHandle = uicontrol(panelHandle, "Style", "popupmenu", "String", cellstr(choices), "Units", "normalized", "Position", [0.51 rowPosition 0.44 0.20], "Value", selectedIndex, "TooltipString", tooltip);
end
...
function handleAction(sourceHandle, ~)
    ...
    try
        ...
        modeState = appendLogLines(modeState, "[Sandbox error] " + string(exceptionText));
        applicationState.GoalMode = modeState;
        guidata(figureHandle, applicationState);
        refreshApplication(figureHandle);
        if actionName == "Export" && get(figureHandle, "Visible") == "on"
            errordlg(['Bundle export failed: ' exceptionText], 'Sandbox export failed', 'modal');
        end
    end
end
...
function exceptionText = formatSandboxException(exception)
    ...
    if strlength(string(exception.identifier)) > 0
        exceptionText = sprintf('%s [%s]', exceptionText, char(exception.identifier));
    end
    if ~isempty(exception.stack)
        exceptionText = sprintf('%s at %s:%d', exceptionText, ...
...
function activateInteraction(figureHandle, requestedState)
    ...
    switch requestedState
        ...
        case "addingPolygon"
            ...
            modeState.Status = "Left-click each polygon vertex. " + "Right-click to close and add the polygon.";
        case "addingCircle"
            interactionState = "placingGoalCircleCenter";
            modeState.Status = "Click the circle center, then click a point on its edge.";
        case "addingHandDrawn"
            interactionState = "drawingGoalObstacle";
            modeState.Status = "Press and drag to draw an obstacle, then release to add it.";
        case "addingSquare"
            interactionState = "placingGoalSquareCorner";
            modeState.Status = "Click one square corner, then click toward the opposite corner.";
        case "selectingObstacleMotion"
            interactionState = "selectingGoalObstacleMotion";
            modeState.SelectedPolygonIndex = 0;
            modeState.Status               = "Click inside a polygon. Then click the arrow endpoint " + "to set its motion vector.";
    end
...
function handleFigureMouseDown(figureHandle, ~)
    ...
    if contains(applicationState.InteractionState, "adding") && endsWith(applicationState.InteractionState, "Polygon")
    ...
    if endsWith(interactionState, "CircleCenter") || endsWith(interactionState, "SquareCorner")
        ...
        if endsWith(interactionState, "CircleCenter")
            ...
            modeState.Status = "Square corner set. Click toward the opposite corner.";
        end
...
function handleObstacleMotionPreview(figureHandle, ~)
    ...
    applicationState = guidata(figureHandle);
    if ~endsWith(applicationState.InteractionState, "ObstacleMotionEnd") || isempty(applicationState.ActiveTraceHandle) || ~isgraphics(applicationState.ActiveTraceHandle)
        return;
    end
    modeState        = applicationState.GoalMode;
...
function finishObstacleMotionInteraction(figureHandle, endpoint_units)
    ...
    modeState.SelectedPolygonIndex = 0;
    modeState = clearModeSolution(modeState);
    modeState.Status = "Motion set for polygon " + polygonIndex + ": " + profile + ".";
    applicationState.GoalMode = modeState;
    guidata(figureHandle, applicationState);
    cancelInteraction(figureHandle);
    refreshApplication(figureHandle);
...
function handleFigureMouseMotion(figureHandle, ~)
    ...
    applicationState = guidata(figureHandle);
    if ~contains(applicationState.InteractionState, "drawing") || isempty(applicationState.ActiveTraceHandle) || ~isgraphics(applicationState.ActiveTraceHandle)
        return;
    end
    modeState               = applicationState.GoalMode;
    point_units               = cursorPoint(modeState.GraphicsHandles.Axes);
...
function playGoalAnimationAfterRun(figureHandle, result, validation)
    ...
    if ~result.Success || ~validation.Passed || ~options.AnimateOnRun || options.FigureVisible ~= "on" || ~isgraphics(figureHandle)
        return;
    end
    modeState   = applicationState.GoalMode;
    ...
    updateModeStatusDisplay(modeState);
    drawnow;
    plotOptions = struct("FigureVisible", "on", ...
    ...
    try
        ...
        applicationState = guidata(figureHandle);
        modeState        = applicationState.GoalMode;
        modeState.Status = finalStatus + newline + "Animation could not be played: " + string(exception.message);
        modeState = appendLogLines(modeState, "[Animation error] " + string(formatSandboxException(exception)));
    end
    applicationState.GoalMode = modeState;
    guidata(figureHandle, applicationState);
    refreshApplication(figureHandle);
...
function controls = readControls(applicationState)
    ...
    controls                         = struct("WorkspaceXInterval_units", workspaceXInterval_units, ...
        ...
        "GoalTimeMode", goalTimeModes(get(handles.GoalTimeModeHandle, "Value")), "UnsupportedTimedTopologyPolicy", unsupportedTimedTopologyPolicies(get(handles.UnsupportedTimedTopologyHandle, "Value")), "WrapX", logical(get(handles.WrapXHandle, "Value")), "WrapY", logical(get(handles.WrapYHandle, "Value")), "Verbose", logical(get(handles.VerboseHandle, "Value")));
end
...
function redrawGoalRequest(axesHandle, modeState, controls)
    ...
    if ~isempty(modeState.StartPosition_units) && ~isempty(modeState.GoalPosition_units)
        ...
        request_units(2, :) = obstacleAvoidance.input.resolveWrappedGoal(request_units(1, :), request_units(2, :), displayLimits, controls);
    ...
    if result.Success
        displayPosition_units = obstacleAvoidance.plotting.createWrappedSpatialPath(result.position_units, [result.Inputs.limits.xInterval_units; result.Inputs.limits.yInterval_units], [result.Options.WrapX result.Options.WrapY]);
    ...
    elseif ~isempty(result.BestPartialRoute_units)
        partialRoute_units = result.BestPartialRoute_units;
        partialRoute_units = obstacleAvoidance.plotting.createWrappedSpatialPath(partialRoute_units, [result.Inputs.limits.xInterval_units; result.Inputs.limits.yInterval_units], [result.Options.WrapX result.Options.WrapY]);
        plot(axesHandle, partialRoute_units(:, 1), partialRoute_units(:, 2), "-.", "Color", [0.90 0.55 0.10], "LineWidth", 1.8, "DisplayName", "Best partial route");
    end
end
...
function updateControlEnablement(applicationState)
    ...
    modeState  = applicationState.GoalMode;
        ...
        set(modeState.GraphicsHandles.Controls.MotionProfileHandle, "Enable", onOff(~isPlanning));
        if isPlanning
            return;
        end
...
function exportDiagnosis(figureHandle)
    ...
    exportInfo       = exportCurrentSandboxDiagnosis(figureHandle, fullfile(folderName, fileName), "goal");
    applicationState = guidata(figureHandle);
    modeState        = applicationState.GoalMode;
    modeState.Status = "Diagnosis bundle exported: " + exportInfo.FilePath;
    modeState = appendLogLines(modeState, "[Sandbox export] " + exportInfo.FilePath + " (" + exportInfo.Bytes + " bytes)");
    applicationState.GoalMode = modeState;
    guidata(figureHandle, applicationState);
    refreshApplication(figureHandle);
    if get(figureHandle, "Visible") == "on"
        confirmationMessage = sprintf('Saved %.0f bytes to:\n%s', ...
            exportInfo.Bytes, char(exportInfo.FilePath));
        msgbox(confirmationMessage, 'Sandbox bundle exported', 'help', 'non-modal');
    end
end

function exportInfo = exportCurrentSandboxDiagnosis(figureHandle, filePath, modeName)
    % Export the current guidata-backed sandbox state to an explicit path.
    if isempty(figureHandle) || ~isgraphics(figureHandle)
        error("obstacleAvoidanceSandbox:ClosedFigure", "The sandbox figure must remain open while exporting a bundle.");
    end
    applicationState = guidata(figureHandle);
    ...
    exportInfo       = exportSandboxDiagnosis(filePath, applicationState, modeName);
end

function applicationState = prepareSandboxStateForExport(applicationState)
...
function snapshot = publicStateSnapshot(figureHandle)
    ...
    snapshot.ExportBundle = @(filePath, modeName) exportCurrentSandboxDiagnosis(figureHandle, filePath, modeName);
end
