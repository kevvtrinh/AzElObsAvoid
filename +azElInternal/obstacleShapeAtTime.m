...
%       Protected occupied geometry. It is empty in geometry-only mode and
%       outside the active span.
...
if lowerIndex == upperIndex
    ...
    vertexSpeedBound_deg_s = adjacentVertexSpeedBound( ...
        obstacle, lowerIndex);
    topologyIsInterpolated = isfinite(vertexSpeedBound_deg_s);
    if ~topologyIsInterpolated
        % Adjacent topology-changing intervals use a static union. Treat
        % their shared exact sample as static so no infinite speed enters
        % the local optimization corridor.
    ...
    if matchingTopology
        ...
        % The conservative union is constant throughout this interval.
        vertexSpeedBound_deg_s = 0;
...
if isempty(shape.Vertices) && ~geometryOnly
