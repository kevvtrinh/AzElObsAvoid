...
corridorCheckMultiplier = 8;
for obstacleIndex = 1:numel(obstacles)
    if historyChangesTopology(obstacles(obstacleIndex))
        corridorCheckMultiplier = 4;
        break;
    end
end
% The independent adaptive validator certifies between optimization points.
corridorTau = linspace( ...
    0, 1, corridorCheckMultiplier * segmentCount + 1).';
...
solverTimer = tic;
solverAlgorithm = "sqp";
if isEarliestArrival && obstacleHistoryChanges(obstacles)
    solverAlgorithm = "interior-point";
end
stageOneTimeLimit_s = options.MaximumSolverTime_s;
if solverAlgorithm == "interior-point"
    stageOneTimeLimit_s = 0.7 * options.MaximumSolverTime_s;
end
...
if isEarliestArrival && ...
    ...
    stageTwoSolverOptions = optimoptions(solverOptions, ...
        "OutputFcn", @(decision, solverValues, state) ...
        stopForTime(decision, solverValues, state, solverTimer, ...
...
for axisIndex = 1:2
    ...
    augmentedFitMatrix = [fitMatrix; ...
        regularizationFactor * eye(controlCount)];
    augmentedFitOffset = [fitOffset; zeros(controlCount, 1)];
    jerkGuess_deg_s3(:, axisIndex) = ...
        augmentedFitMatrix \ augmentedFitOffset;
end
...
seedStepBound_deg = max(vecnorm(diff(seedPosition_deg, 1, 1), 2, 2));
obstacleChangesTopology = false(numel(obstacles), 1);
for obstacleIndex = 1:numel(obstacles)
    obstacleChangesTopology(obstacleIndex) = ...
        historyChangesTopology(obstacles(obstacleIndex));
end
for controlIndex = 1:numel(controlTau)
    for obstacleIndex = 1:numel(obstacles)
        ...
        clearanceStepFraction = 0.5 + 0.5 * ...
            obstacleChangesTopology(obstacleIndex);
        association.Clearance_deg = clearanceStepFraction * ...
            (seedStepBound_deg + ...
            maximumTimeStep_s * obstacleSpeedBound_deg_s) + 1e-4;
        if ~geometry.TopologyIsInterpolated
...
function changes = historyChangesTopology(obstacle)
% PURPOSE
%   - Detect adjacent protected slices that require a static union interval.
changes = false;
for sampleIndex = 2:numel(obstacle.time_s)
    firstAzimuth_deg = obstacle.az_deg{sampleIndex - 1};
    firstElevation_deg = obstacle.el_deg{sampleIndex - 1};
    secondAzimuth_deg = obstacle.az_deg{sampleIndex};
    secondElevation_deg = obstacle.el_deg{sampleIndex};
    matchingTopology = ...
        numel(firstAzimuth_deg) == numel(secondAzimuth_deg) && ...
        isequal(isfinite(firstAzimuth_deg), ...
        isfinite(secondAzimuth_deg)) && ...
        isequal(isfinite(firstElevation_deg), ...
        isfinite(secondElevation_deg));
    if ~matchingTopology
        changes = true;
        return;
    end
...
function changes = obstacleHistoryChanges(obstacles)
% PURPOSE
%   - Detect any input geometry change that makes final time affect shape.
changes = false;
for obstacleIndex = 1:numel(obstacles)
...
fixedAssociation = [corridor.GeometryIsFixed].';
fixedIndex = find(fixedAssociation);
if ~isempty(fixedIndex)
    fixedCorridor = corridor(fixedIndex);
    fixedControlIndex = [fixedCorridor.ControlIndex].';
    fixedNormal = vertcat(fixedCorridor.FixedNormal);
    fixedBoundaryOffset_deg = ...
        [fixedCorridor.FixedBoundaryOffset_deg].';
    fixedClearance_deg = [fixedCorridor.Clearance_deg].';
    fixedProjection_deg = sum( ...
        controlPosition_deg(fixedControlIndex, :) .* fixedNormal, 2);
    inequality(fixedIndex) = fixedBoundaryOffset_deg + ...
        fixedClearance_deg - fixedProjection_deg;
end
for associationIndex = reshape(find(~fixedAssociation), 1, [])
