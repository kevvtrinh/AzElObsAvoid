...
if ~options.BuildFullVisibilityGraphs
    ...
    if useParallel
        ...
        snapshotReuseData = cell(snapshotCount, 1);
        for snapshotIndex = 1:snapshotCount
            if toc(planningTimer) >= options.MaximumPlanningTime_s
                break;
            end
            snapshotReuseData{snapshotIndex} = graphReuseDataAtSnapshot( ...
                obstacleField, candidatePoints, candidateTypes, ...
                candidateObstacleIndex, candidateRegionIndex, ...
                candidateBoundaryGeometry, snapshotTimes_s(snapshotIndex));
            reuseIndex = 0;
            for previousIndex = 1:snapshotIndex - 1
                if graphReuseDataMatches( ...
                        snapshotReuseData{snapshotIndex}, ...
                        snapshotReuseData{previousIndex})
                    reuseIndex = previousIndex;
                    break;
                end
            end
            if reuseIndex > 0
                graphs(snapshotIndex) = reuseSnapshotVisibilityGraph( ...
                    graphs(reuseIndex), snapshotReuseData{snapshotIndex}, ...
                    snapshotTimes_s(snapshotIndex), reuseIndex);
            else
                graphs(snapshotIndex) = buildOneSnapshotVisibilityGraph( ...
                    obstacleField, candidatePoints, candidateTypes, ...
                    candidateObstacleIndex, candidateSampleIndex, ...
...
candidateIsOnSnapshot = candidateMaskAtSnapshot( ...
    obstacleField, candidatePoints, candidateObstacleIndex, snapshotTime_s);
...
function candidateIsOnSnapshot = candidateMaskAtSnapshot( ...
        obstacleField, candidatePoints, candidateObstacleIndex, ...
        snapshotTime_s)
% PURPOSE
%   - Select each obstacle's candidate set nearest to one retained time.
candidateIsOnSnapshot = false(size(candidatePoints, 1), 1);
for obstacleIndex = 1:obstacleField.ObstacleCount
    obstacleTime_s = double( ...
        obstacleField.Obstacles(obstacleIndex).TimeSeconds(:));
...
function reuseData = graphReuseDataAtSnapshot( ...
        obstacleField, candidatePoints, candidateTypes, ...
        candidateObstacleIndex, candidateRegionIndex, ...
        candidateBoundaryGeometry, snapshotTime_s)
% PURPOSE
%   - Capture all geometry that can change a snapshot visibility graph.
candidateMask = candidateMaskAtSnapshot( ...
    obstacleField, candidatePoints, candidateObstacleIndex, snapshotTime_s);
reuseData = struct( ...
    "GlobalCandidateIndex", find(candidateMask), ...
    "CandidatePosition_deg", candidatePoints(candidateMask, 1:2), ...
    "CandidateTypes", candidateTypes(candidateMask), ...
    "CandidateObstacleIndex", candidateObstacleIndex(candidateMask), ...
    "CandidateRegionIndex", candidateRegionIndex(candidateMask), ...
    "CandidateBoundaryGeometry", ...
        {candidateBoundaryGeometry(candidateMask)}, ...
    "ObstacleEdges_deg", ...
        {azElInternal.interpolateAzElObstacleEdges( ...
        obstacleField, snapshotTime_s)});
end

function matches = graphReuseDataMatches(first, second)
% PURPOSE
%   - Require exact equality of graph nodes, boundaries, and collision edges.
matches = ~isempty(first) && ~isempty(second) && ...
    isequal(first.CandidatePosition_deg, second.CandidatePosition_deg) && ...
    isequal(first.CandidateTypes, second.CandidateTypes) && ...
    isequal(first.CandidateObstacleIndex, ...
        second.CandidateObstacleIndex) && ...
    isequal(first.CandidateRegionIndex, second.CandidateRegionIndex) && ...
    isequal(first.CandidateBoundaryGeometry, ...
        second.CandidateBoundaryGeometry) && ...
    isequal(first.ObstacleEdges_deg, second.ObstacleEdges_deg);
end

function graph = reuseSnapshotVisibilityGraph( ...
        sourceGraph, reuseData, snapshotTime_s, sourceSnapshotIndex)
% PURPOSE
%   - Rebind an identical graph to the current time and candidate indices.
graph = sourceGraph;
graph.Time_s = snapshotTime_s;
graph.GraphConstructionReused = true;
graph.ReusedFromSnapshotIndex = sourceSnapshotIndex;
candidateNodeCount = max(0, numel(graph.NodeCandidateIndex) - 2);
if candidateNodeCount ~= numel(reuseData.GlobalCandidateIndex)
    error("buildAzElSnapshotVisibilityGraphs:ReuseCandidateCountMismatch", ...
        "An identical snapshot has %d graph candidates but %d indices.", ...
        candidateNodeCount, numel(reuseData.GlobalCandidateIndex));
end
if candidateNodeCount > 0
    graph.NodeCandidateIndex(3:end) = reuseData.GlobalCandidateIndex;
end
if ~isempty(graph.PathNodeIndex)
    graph.PathCandidateIndex = ...
        graph.NodeCandidateIndex(graph.PathNodeIndex);
end
graph.Message = graph.Message + " Reused collision-equivalent graph " + ...
    "geometry from retained snapshot " + string(sourceSnapshotIndex) + ".";
end
...
graph = struct( ...
    ...
    "GraphConstructionReused", false, ...
    "ReusedFromSnapshotIndex", 0, ...
