...
%     derivatives of a regular planar Bezier curve of degree three or more.
...
%**************************************************************************
% INPUTS
%   - controlPoints_deg (N-by-2 finite numeric matrix, N >= 4)
%       Bezier controls ordered by increasing curve parameter.
...
validateattributes(controlPoints_deg, {'numeric'}, ...
    {'real', 'finite', '2d', 'ncols', 2}, mfilename, ...
    'controlPoints_deg');
if size(controlPoints_deg, 1) < 4
    error("certifyQuinticArcDerivatives:InsufficientDegree", ...
        "controlPoints_deg must contain at least four rows.");
end
...
firstControl_deg = degree * diff(controlPoints_deg, 1, 1);
secondControl_deg = degree * (degree - 1) * ...
    diff(controlPoints_deg, 2, 1);
thirdControl_deg = degree * (degree - 1) * (degree - 2) * ...
    diff(controlPoints_deg, 3, 1);
...
incomingHalfTrim_deg = NaN;
outgoingHalfTrim_deg = NaN;
hasSymmetricConstruction = false;
if size(controlPoints_deg, 1) == 6
