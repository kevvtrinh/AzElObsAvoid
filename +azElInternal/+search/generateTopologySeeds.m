...
% SYNTAX
%   [seeds, diagnostics] = azElInternal.search.generateTopologySeeds( ...
...
[sweptShape, usedDenseEnvelope] = azElInternal.search.denseSweptEnvelope( ...
...
[sweptShape, diagnostics.SeedCluster] = azElInternal.search.clusterSeedShape( ...
...
for sampleTimeIndex = 1:numel(sampleTimes_s)
    ...
    for obstacleIndex = 1:numel(obstacles)
        shape = azElInternal.obstacles.shapeAtTime(obstacles(obstacleIndex), sampleTime_s);
...
function [route_deg, routeTime_s, record] = timeExpandedVisibilitySearch( ...
        nodePosition_deg, edgeCost_deg, obstacles, initialState, goalState, limits, sampleTimes_s, options)
...
for layerIndex = 1:layerCount - 1
    ...
    for currentNodeIndex = reshape(currentNodeIndices, 1, [])
        ...
        if nodeIsFree(layerIndex + 1, currentNodeIndex) && ...
                ...
                nodePosition_deg(currentNodeIndex, :), layerTimes_s(layerIndex), layerTimes_s(layerIndex + 1))
        ...
        % Test visible motion edges to the next layer; the same-node wait edge is handled separately.
        for targetNodeIndex = reshape(targetNodeIndices, 1, [])
            ...
            accelerationDuration_s = 2 * sqrt(abs(displacement_deg) ./ limits.maxAcceleration_deg_s2);
            minimumDuration_s = max([speedDuration_s, accelerationDuration_s]);
            targetLayerIndex = find(layerTimes_s > layerTimes_s(layerIndex) + minimumDuration_s - 1e-12, 1, "first");
            ...
            isClear = motionEdgeIsClear( ...
                obstacles, nodePosition_deg(currentNodeIndex, :), ...
                nodePosition_deg(targetNodeIndex, :), layerTimes_s(layerIndex), layerTimes_s(targetLayerIndex));
            if ~isClear
                rejectedTransitionCount = rejectedTransitionCount + 1;
                continue;
            end
...
record = struct("LayerTimes_s", layerTimes_s, "NodeCount", nodeCount, ...
    ...
    "ExpandedCount", expandedCount, "ExploredNodes_deg", exploredNodes_deg, "FrontierNodes_deg", frontierNodes_deg);
end
...
candidateTime_s = candidateTime_s(candidateTime_s >= startTime_s & candidateTime_s <= endTime_s);
...
% Map each uniform target time to its nearest available source-aware layer.
...
function clear = motionEdgeIsClear(obstacles, firstPosition_deg, secondPosition_deg, firstTime_s, secondTime_s)
...
occupied = queryAzElTimeObstacle(obstacles, samplePosition_deg(:, 1), samplePosition_deg(:, 2), sampleTime_s);
clear = ~any(occupied);
end
...
sampleTimes_s = boundedTimeLayers( sampleTimes_s, startTime_s, endTime_s, maximumLayerCount);
end
