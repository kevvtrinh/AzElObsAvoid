...
% SYNTAX
%   [bestMotion, bestValidation, diagnostics] = azElInternal.motion.solveCompactC3(seed, upperDuration_s, ...
...
[~, fitBasis] = azElInternal.motion.evaluatePolynomial( basisPolynomial, fitTime);
...
straight = azElInternal.motion.buildQuinticSpline( ...
...
for trialIndex = 1:8
    ...
    validation = validateAzElTrajectory( trialMotion, obstacles, initialState, goalState, limits, plannerOptions);
...
affinePolynomial = azElInternal.motion.convertBsplineToPolynomial( ...
...
[~, positionBasis, velocityBasis, accelerationBasis, jerkBasis] = azElInternal.motion.evaluatePolynomial(affinePolynomial, sampleTime_s);
...
polynomial = azElInternal.motion.convertBsplineToPolynomial( controlPoint_deg, 5, initialState.time_s, spanDuration_s);
...
[outputTime_s, position_deg, velocity_deg_s, acceleration_deg_s2, ...
    jerk_deg_s3] = azElInternal.motion.evaluatePolynomial( polynomial, outputTime_s);
...
for obstacleIndex = 1:obstacleCount
    ...
    if stationaryHistory
        ...
        for nearIndex = 1:numel(nearTimeIndex)
            ...
            shape = azElInternal.obstacles.shapeAtTime( obstacles(obstacleIndex), time_s(timeIndex));
