...
% SYNTAX
%   options = azElInternal.motion.solveCorridorQuintic()
%   motion = azElInternal.motion.solveCorridorQuintic(obstacles, ...
...
azElInternal.motion.buildQuinticSpline( ...
...
if isempty(reducedRoute_deg)
    ...
    motion = azElInternal.motion.buildQuinticSpline( route_deg, initialState, goalState, limits);
...
elseif isempty(options.ObstacleEnvelopeBoundary_deg)
    seed.CorridorBoundary_deg = azElInternal.obstacles.buildEnvelopeBoundary( obstacles, envelopePadding_deg);
else
...
baseMotion = azElInternal.motion.buildQuinticSpline( route_deg, initialState, goalState, limits, baseOptions);
corridor = azElInternal.validation.buildSeedCorridor( seed, baseMotion.Polynomial.SegmentCount);
...
baseInequality_deg = azElInternal.validation.seedCorridorInequality( baseMotion.Polynomial, corridor);
...
if canUseDirectAffineBasis
        ...
        affineBasisPolynomial = azElInternal.motion.convertBsplineToPolynomial( ...
    ...
    for decisionIndex = 1:decisionCount
        ...
        if canUseDirectAffineBasis
            ...
            projectionBernstein_deg = azElInternal.powerToBernstein( projectionPower_deg.');
            ...
            basisMotion = azElInternal.motion.buildQuinticSpline( ...
                route_deg, initialState, goalState, limits, basisOptions);
            basisInequality_deg = azElInternal.validation.seedCorridorInequality( basisMotion.Polynomial, corridor);
...
if canOptimizeExactTraversal
    [trialDecision_deg, exactTraversalDiagnostics] = azElInternal.motion.optimizeExactTraversal( ...
...
if isempty(decision_deg)
    ...
    candidateMotion = azElInternal.motion.buildQuinticSpline( ...
...
if isempty(corridorObstacles)
    ...
    [corridorCertified, certifiedClearance_deg] = azElInternal.validation.certifySeedCorridor( ...
            candidateMotion, corridorObstacles, queryOptions.ClearanceTolerance_deg);
end
validation = validateAzElTrajectory( candidateMotion, obstacles, initialState, goalState, limits, plannerOptions);
candidateInequality_deg = azElInternal.validation.seedCorridorInequality( candidateMotion.Polynomial, corridor);
envelopeShape = polyshape( seed.CorridorBoundary_deg(:, 1), seed.CorridorBoundary_deg(:, 2), "Simplify", true);
convexEnvelopeRegions = azElInternal.geometry.convexPolygonRegions(envelopeShape);
expectedRecordCount = candidateMotion.Polynomial.SegmentCount * numel(convexEnvelopeRegions);
if isempty(corridorObstacles)
    envelopeContainsObstacles = true;
else
    envelopeContainsObstacles = azElInternal.validation.seedEnvelopeContainsObstacles( ...
...
for routeIndex = 2:size(route_deg, 1) - 1
    ...
    for obstacleIndex = 1:numel(obstacles)
        shape = azElInternal.obstacles.shapeAtTime( obstacles(obstacleIndex), queryTime_s);
