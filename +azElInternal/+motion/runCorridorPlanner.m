...
if geometryIsStatic
    obstacleEnvelopeBoundary_deg = azElInternal.obstacles.buildEnvelopeBoundary( obstacles, envelopePadding_deg);
else
...
if ~geometryIsStatic && ~any(zeroLengthSpan)
        [route_deg, dynamicRouteExpansion_deg] = azElInternal.search.expandDynamicRoute( ...
    ...
    candidate = azElInternal.motion.solveCorridorQuintic( ...
...
if canTryStaticExact
        [expandedRoute_deg, staticExpansion_deg] = azElInternal.search.expandDynamicRoute( ...
            seeds(seedIndex), obstacles, initialState.time_s, goalState.time_s);
        exactTrialCount = 0;
        ...
        for densificationFactor = 1:3
            ...
            exactTrial = azElInternal.motion.solveCorridorQuintic( ...
...
if any(compactEligible) && any(isfinite(validatedDuration_s))
    ...
    compactTimer = tic;
    [compactMotion, compactValidation, compactDiagnostics] = azElInternal.motion.solveCompactC3( ...
...
for iterationIndex = 0:8
    trialMotion = azElInternal.motion.buildQuinticSpline( ...
...
validation = validateAzElTrajectory( bestMotion, obstacles, initialState, goalState, limits, plannerOptions);
...
for iterationIndex = 1:maximumIterationCount
    affineBasis = azElInternal.motion.convertBsplineToPolynomial( ...
...
if ~any(violated)
            ...
            [decision_deg, diagnostics] = azElInternal.motion.optimizeExactTraversal( ...
...
[~, position_deg] = azElInternal.motion.evaluatePolynomial( motion.Polynomial, time_s);
...
for timeIndex = 1:numel(time_s)
    ...
    for obstacleIndex = 1:numel(obstacles)
        ...
        [clearance_deg, obstaclePoint_deg] = azElInternal.geometry.pointPolygonClearance( ...
...
[~, position_deg] = azElInternal.motion.evaluatePolynomial(polynomial, time_s);
clearance_deg = Inf;

% Return the minimum signed clearance across all active obstacles at this time.
for obstacleIndex = 1:numel(obstacles)
    shape = azElInternal.obstacles.shapeAtTime(obstacles(obstacleIndex), time_s);
    clearance_deg = min(clearance_deg, azElInternal.geometry.pointPolygonClearance(shape, position_deg));
end
end
...
trial = azElInternal.motion.solveCorridorQuintic( obstacles, initialState, goalState, limits, route_deg, trialOptions);
...
for multiplierIndex = 1:numel(multiplierGrid)
    ...
    if multiplier == 1
        ...
        trial = azElInternal.motion.solveCorridorQuintic( ...
