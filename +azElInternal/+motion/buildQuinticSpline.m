...
validation = validateAzElTrajectory( ...
    motion, [], initialState, goalState, limits, options);
...
profile = buildStraightJerkProfile(route_deg, initialState, limits);
...
if ~any(activeAxis)
    ...
    scalarVelocityLimit_s = min( ...
        limits.maxVelocity_deg_s(activeAxis) ./ abs(delta_deg(activeAxis)));
    scalarAccelerationLimit_s2 = min( ...
        limits.maxAcceleration_deg_s2(activeAxis) ./ ...
        abs(delta_deg(activeAxis)));
    scalarJerkLimit_s3 = min( ...
        limits.maxJerk_deg_s3(activeAxis) ./ abs(delta_deg(activeAxis)));
end
[jerkRampTime_s, constantAccelerationTime_s, cruiseTime_s] = ...
    profileDurations( ...
...
phaseDuration_s = durationScale * [ ...
    ...
    cruiseTime_s; jerkRampTime_s; constantAccelerationTime_s; ...
    jerkRampTime_s];
phaseJerk_s3 = [1; 0; -1; 0; -1; 0; 1] * ...
    scalarJerkLimit_s3 / durationScale^3;
...
% Integrate the seven-phase scalar timing law and map each phase onto both
% axes.
for segmentIndex = 1:segmentCount
    ...
    scalarPositionPower = [ ...
        ...
        0.5 * scalarAcceleration * duration_s^2, ...
        scalarJerk * duration_s^3 / 6, 0, 0];
    scalarVelocityPower = [ ...
        scalarVelocity, scalarAcceleration * duration_s, ...
        0.5 * scalarJerk * duration_s^2, 0, 0];
    scalarAccelerationPower = [ ...
        scalarAcceleration, scalarJerk * duration_s, 0, 0];
    ...
    velocityPower_deg_s(segmentIndex, :, :) = ...
        delta_deg.' .* scalarVelocityPower;
    accelerationPower_deg_s2(segmentIndex, :, :) = ...
        delta_deg.' .* scalarAccelerationPower;
    jerkPower_deg_s3(segmentIndex, :, :) = ...
        delta_deg.' .* [scalarJerk 0 0];
...
polynomial = struct( ...
    ...
    "position_deg", route_deg(2, :), ...
    "velocity_deg_s", [0 0], "acceleration_deg_s2", [0 0]));
profile = struct( ...
    "Delta_deg", delta_deg, ...
    "PhaseDuration_s", phaseDuration_s, ...
    "PhaseJerk_s3", phaseJerk_s3, ...
    "Polynomial", polynomial);
end

function [jerkRampTime_s, constantAccelerationTime_s, cruiseTime_s] = ...
        profileDurations(velocityLimit_s, accelerationLimit_s2, jerkLimit_s3)
...
if accelerationDistance >= 1
    ...
    constantAccelerationTime_s = 0.5 * ( ...
        sqrt(jerkRampTime_s^2 + 4 / accelerationLimit_s2) - ...
        3 * jerkRampTime_s);
end
peakVelocity_s = jerkLimit_s3 * jerkRampTime_s * ...
    (jerkRampTime_s + constantAccelerationTime_s);
cruiseTime_s = 0;
...
if velocityLimit_s <= accelerationLimit_s2^2 / jerkLimit_s3
    ...
    constantAccelerationTime_s = ...
        velocityLimit_s / accelerationLimit_s2 - jerkRampTime_s;
end
minimumDistance = velocityLimit_s * ...
    (2 * jerkRampTime_s + constantAccelerationTime_s);
