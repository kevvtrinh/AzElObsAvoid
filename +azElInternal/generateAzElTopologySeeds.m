...
%**************************************************************************
% OUTPUTS
%   - seeds (column struct array): Geometry, time law, source, and duration.
...
if usedDenseEnvelope
    seedTemplate.CorridorBoundary_deg = sweptShape.Vertices;
elseif diagnostics.SeedCluster.ClusterGroupCount > 0
    seedTemplate.CorridorBoundary_deg = diagnostics.SeedCluster.ClusterBoundary_deg;
end
if diagnostics.SeedCluster.ClusterGroupCount > 0
    sampledNodes_deg = sweptShape.Vertices;
...
if ~usedDenseEnvelope && options.SeedClusterDistance_deg == 0 && ...
        obstacleHistoryChanges(obstacles) && ...
...
for sideModeIndex = 1:numel(sideModes)
    ...
    seeds(end + 1, 1) = createSpatialSeed(seedTemplate, numel(seeds) + 1, route_deg, ...
...
frontierNodes_deg = nodePosition_deg( ...
    isfinite(costToCome_deg) & ~closed, :);
record = struct( ...
    "ExpandedCount", expandedCount, ...
    "RejectedTransitionCount", rejectedTransitionCount, ...
    "ExploredNodes_deg", exploredNodes_deg, ...
...
diagnostics = struct( ...
    ...
    "DenseSeedEnvelope_deg", zeros(0, 2), ...
    "SeedCluster", struct( ...
    "Distance_deg", 0, "SourceRegionCount", 0, ...
    "ClusterGroupCount", 0, "ClusteredRegionCount", 0, ...
