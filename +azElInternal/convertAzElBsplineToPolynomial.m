function polynomial = convertAzElBsplineToPolynomial( ...
        controlPoint_deg, degree, initialTime_s, spanDuration_s)
% Convert an open B-spline to the segment-local polynomial schema.
...
knots_s = knotTime( ...
    initialTime_s, spanDuration_s, degree, size(controlPoint_deg, 1));
...
accelerationPower_deg_s2 = zeros( ...
    spanCount, coordinateCount, degree - 1);
...
for segmentIndex = 1:spanCount
    ...
    positionPower_deg(segmentIndex, :, :) = ...
        reshape(positionPower, 1, coordinateCount, degree + 1);
    velocityPower_deg_s(segmentIndex, :, :) = ...
        reshape(velocityPower, 1, coordinateCount, degree);
    accelerationPower_deg_s2(segmentIndex, :, :) = ...
        reshape(accelerationPower, 1, coordinateCount, degree - 1);
    jerkPower_deg_s3(segmentIndex, :, :) = ...
        reshape(jerkPower, 1, coordinateCount, degree - 2);
end
positionPower_deg = projectPolynomialContinuity( ...
    positionPower_deg, spanDuration_s, degree);
for segmentIndex = 1:spanCount
    positionPower = reshape( ...
        positionPower_deg(segmentIndex, :, :), coordinateCount, []);
    velocityPower = positionPower(:, 2:end) .* (1:degree) / ...
        spanDuration_s(segmentIndex);
    accelerationPower = velocityPower(:, 2:end) .* (1:degree - 1) / ...
        spanDuration_s(segmentIndex);
...
terminalPosition_deg = sum(reshape( ...
    positionPower_deg(end, :, :), coordinateCount, []), 2).';
terminalVelocity_deg_s = sum(reshape( ...
    velocityPower_deg_s(end, :, :), coordinateCount, []), 2).';
terminalAcceleration_deg_s2 = sum(reshape( ...
    accelerationPower_deg_s2(end, :, :), coordinateCount, []), 2).';
polynomial = struct( ...
...
function positionPower = projectPolynomialContinuity( ...
        positionPower, spanDuration_s, degree)
...
for segmentIndex = 1:spanCount - 1
    for derivativeOrder = 0:3
        rowIndex = rowIndex + 1;
        leftIndex = (segmentIndex - 1) * coefficientCount + ...
            (derivativeOrder + 1:coefficientCount);
        powerIndex = derivativeOrder:degree;
        leftFactor = factorial(powerIndex) ./ ...
            factorial(powerIndex - derivativeOrder) / ...
            spanDuration_s(segmentIndex) ^ derivativeOrder;
        constraintMatrix(rowIndex, leftIndex) = leftFactor;
        rightIndex = segmentIndex * coefficientCount + derivativeOrder + 1;
        constraintMatrix(rowIndex, rightIndex) = ...
...
for derivativeOrder = 0:2
    rowIndex = rowIndex + 1;
    constraintMatrix(rowIndex, derivativeOrder + 1) = ...
        factorial(derivativeOrder) / ...
        spanDuration_s(1) ^ derivativeOrder;
end
for derivativeOrder = 0:2
    rowIndex = rowIndex + 1;
    finalIndex = (spanCount - 1) * coefficientCount + ...
        (derivativeOrder + 1:coefficientCount);
    powerIndex = derivativeOrder:degree;
    constraintMatrix(rowIndex, finalIndex) = factorial(powerIndex) ./ ...
        factorial(powerIndex - derivativeOrder) / ...
        spanDuration_s(end) ^ derivativeOrder;
end
for coordinateIndex = 1:size(positionPower, 2)
    coefficient = reshape(permute( ...
        positionPower(:, coordinateIndex, :), [3 1 2]), [], 1);
    endpointValue = constraintMatrix(end - 5:end, :) * coefficient;
    constraintValue = [zeros(constraintCount - 6, 1); endpointValue];
    coefficient = coefficient + lsqminnorm( ...
        constraintMatrix, constraintValue - constraintMatrix * coefficient);
    positionPower(:, coordinateIndex, :) = permute(reshape( ...
        coefficient, coefficientCount, spanCount), [2 3 1]);
end
end

function knots_s = knotTime( ...
        initialTime_s, spanDuration_s, degree, controlCount)
%% Section 0: Header & Readme
% PURPOSE
...
if interiorBoundaryCount == 0
    ...
    interiorMultiplicity = ...
        (controlCount - degree - 1) / interiorBoundaryCount;
end
if interiorMultiplicity ~= floor(interiorMultiplicity) || ...
        interiorMultiplicity < 0 || interiorMultiplicity > degree
    error("convertAzElBsplineToPolynomial:InconsistentControlCount", ...
        "Control count does not define a valid uniform knot multiplicity.");
end
knots_s = [ ...
    repmat(spanBoundary_s(1), 1, degree + 1), ...
    repelem(spanBoundary_s(2:end - 1).', interiorMultiplicity), ...
