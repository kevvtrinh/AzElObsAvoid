...
if ~jerkConstrained
    ...
    % certified spatial grid follows the reachability discretization in
    % Pham and Pham (2018); see citation.md. A smaller maximum cell length
    % tightens local derivative envelopes without changing path geometry.
    boundaryS_deg = 0;
    runPrimitiveIndex = zeros(0, 1);

    for primitiveIndex = 1:geometricPrimitiveCount
        primitive = smoothPath.Primitives(primitiveIndex);
        cellCount = max(2, ceil(primitive.Length_deg / ...
            options.RetimerMaximumCellLength_deg));
...
if jerkConstrained
    [nodeSpeed_deg_s, feasible, failureMessage] = ...
        ...
        initialSpeed_deg_s, goalSpeed_deg_s, mandatoryStopNode, ...
        options.JerkProfileType, limits.maxTangentialSnap_deg_s4, ...
        tolerance);
end
if ~feasible
    timedPath = timedPathTemplate(limits, options, failureMessage);
...
for runIndex = 1:runCount
    if jerkConstrained
        [profile, feasible, failureMessage] = minimumTimeProfile(...
            ...
            maximumSpeed_deg_s(runIndex), ...
            maximumAcceleration_deg_s2(runIndex), ...
            maximumJerk_deg_s3(runIndex), ...
            options.JerkProfileType, ...
            limits.maxTangentialSnap_deg_s4, tolerance);
    else
        profile = profileTemplate();
        profile.JerkProfileType = "unlimited";
...
isTangentialSnapConstrained = jerkConstrained && ...
    isfinite(limits.maxTangentialSnap_deg_s4);
if jerkConstrained
    peakTangentialSnap_deg_s4 = ...
        max([profiles.PeakTangentialSnap_deg_s4]);
    tangentialSnapSatisfied = ...
        peakTangentialSnap_deg_s4 <= ...
        limits.maxTangentialSnap_deg_s4 + tolerance;
else
    % An acceleration-only profile has no defined jerk or snap history.
    % Do not reject it because its intentionally unavailable snap is NaN.
    peakTangentialSnap_deg_s4 = NaN;
    tangentialSnapSatisfied = true;
end
constraintsSatisfied = velocitySatisfied && accelerationSatisfied && ...
    jerkSatisfied && tangentialSnapSatisfied;
continuousJerkCertified = profilesHaveContinuousJerk( ...
    profiles, options.JerkProfileType, jerkConstrained, tolerance) && ...
    constraintsSatisfied;
continuousTangentialSnapCertified = ...
    profilesHaveContinuousTangentialSnap( ...
    profiles, options.JerkProfileType, jerkConstrained, tolerance) && ...
    constraintsSatisfied;
...
diagnostics.ContinuousJerkCertified = continuousJerkCertified;
diagnostics.ContinuousTangentialSnapCertified = ...
    continuousTangentialSnapCertified;
diagnostics.PeakTangentialSnap_deg_s4 = ...
    peakTangentialSnap_deg_s4;
diagnostics.TangentialSnapConstrained = ...
    isTangentialSnapConstrained;
diagnostics.TangentialSnapSatisfied = tangentialSnapSatisfied;
if jerkConstrained
    diagnostics.TangentialSnapMargin_deg_s4 = ...
        limits.maxTangentialSnap_deg_s4 - peakTangentialSnap_deg_s4;
else
    diagnostics.TangentialSnapMargin_deg_s4 = NaN;
end
diagnostics.JerkProfileType = options.JerkProfileType;
...
diagnostics.RetimerMaximumCellLength_deg = ...
    options.RetimerMaximumCellLength_deg;
...
elseif options.JerkProfileType == "continuoussine"
    timedPath.Message = ...
        "Certified continuous-sine-jerk spatial retiming succeeded.";
    timedPath.RetimerType = "certifiedAnalyticSpatialContinuousSineJerk";
elseif options.JerkProfileType == "continuoussigmoid"
    timedPath.Message = ...
        "Certified continuous-sigmoid-jerk spatial retiming succeeded.";
...
function [nodeSpeed_deg_s, feasible, message] = ...
        ...
        mandatoryStopNode, jerkProfileType, ...
        maximumTangentialSnap_deg_s4, tolerance)
%% Section 0: Header & Readme
% SYNTAX
...
%   - jerkProfileType (scalar string)
%       Constant-jerk, continuous-sine, or continuous-sigmoid family.
%   - maximumTangentialSnap_deg_s4 (positive scalar)
%       Optional scalar path-coordinate snap limit.
%   - mandatoryStopNode (logical vector), tolerance (scalar)
...
for passIndex = 1:max(8, 2 * (runCount + 1))
    ...
    for runIndex = 1:runCount
        reachable = reachableSpeed(nodeSpeed_deg_s(runIndex), ...
            ...
            maximumAcceleration_deg_s2(runIndex), ...
            maximumJerk_deg_s3(runIndex), jerkProfileType, ...
            maximumTangentialSnap_deg_s4, tolerance);
...
function speed_deg_s = reachableSpeed(boundarySpeed_deg_s, distance_deg, ...
        ...
        jerkLimit_deg_s3, jerkProfileType, ...
        maximumTangentialSnap_deg_s4, tolerance)
%% Section 0: Header & Readme
% SYNTAX
...
if transitionDistance(boundarySpeed_deg_s, speedLimit_deg_s, ...
        ...
        jerkProfileType, maximumTangentialSnap_deg_s4) <= ...
...
for iteration = 1:70
    ...
    if transitionDistance(boundarySpeed_deg_s, middle, ...
            accelerationLimit_deg_s2, jerkLimit_deg_s3, ...
            jerkProfileType, maximumTangentialSnap_deg_s4) <= distance_deg
        lower = middle;
    else
        upper = middle;
    end
...
function [profile, feasible, message] = minimumTimeProfile(length_deg, ...
        ...
        speedLimit_deg_s, accelerationLimit_deg_s2, jerkLimit_deg_s3, ...
        jerkProfileType, maximumTangentialSnap_deg_s4, tolerance)
%% Section 0: Header & Readme
% SYNTAX
...
%**************************************************************************
% PURPOSE
%   - Build the minimum-time scalar S-curve for one path run.
%**************************************************************************
% INPUTS
...
profile.JerkProfileType = jerkProfileType;
...
minimumDistance_deg = transitionDistance(startSpeed_deg_s, endSpeed_deg_s, ...
    accelerationLimit_deg_s2, jerkLimit_deg_s3, jerkProfileType, ...
    maximumTangentialSnap_deg_s4);
...
[firstDuration_s, firstJerk_deg_s3, firstPhaseLaw] = transitionPhases(...
    startSpeed_deg_s, peakSpeed_deg_s, ...
    accelerationLimit_deg_s2, jerkLimit_deg_s3, jerkProfileType, ...
    maximumTangentialSnap_deg_s4);
[lastDuration_s, lastJerk_deg_s3, lastPhaseLaw] = transitionPhases(...
...
phaseLaw = [firstPhaseLaw; "constant"; lastPhaseLaw];
...
for index = 1:(phaseCount - 1)
    duration_s = phaseDuration_s(index);
    jerk_deg_s3 = phaseJerk_deg_s3(index);
    [phaseStartPosition_deg(index + 1), ...
        phaseStartSpeed_deg_s(index + 1), ...
        phaseStartAcceleration_deg_s2(index + 1)] = ...
        integrateProfilePhase(phaseStartPosition_deg(index), ...
        phaseStartSpeed_deg_s(index), ...
        phaseStartAcceleration_deg_s2(index), duration_s, ...
        jerk_deg_s3, phaseLaw(index));
end
...
profile.PeakAcceleration_deg_s2 = max(abs([ phaseStartAcceleration_deg_s2; ...
    phaseEndAccelerations(phaseStartAcceleration_deg_s2, ...
        phaseDuration_s, phaseJerk_deg_s3, phaseLaw)]));
profile.PeakJerk_deg_s3 = max(abs(phaseJerk_deg_s3));
profile.PeakTangentialSnap_deg_s4 = phasePeakSnap( ...
    phaseDuration_s, phaseJerk_deg_s3, phaseLaw, jerkProfileType);
profile.PhaseDuration_s = phaseDuration_s;
profile.PhaseJerk_deg_s3 = phaseJerk_deg_s3;
profile.PhaseLaw = phaseLaw;
...
function [duration_s, jerk_deg_s3, phaseLaw] = transitionPhases( ...
        firstSpeed_deg_s, ...
        secondSpeed_deg_s, ...
        accelerationLimit_deg_s2, jerkLimit_deg_s3, jerkProfileType, ...
        maximumTangentialSnap_deg_s4)
%% Section 0: Header & Readme
% SYNTAX
%   [duration_s, jerk_deg_s3, phaseLaw] = transitionPhases( ...
%       firstSpeed_deg_s, secondSpeed_deg_s, ...
%       accelerationLimit_deg_s2, jerkLimit_deg_s3, jerkProfileType, ...
%       maximumTangentialSnap_deg_s4)
%**************************************************************************
% PURPOSE
%   - Return the minimum-time phases for one zero-acceleration transition.
%**************************************************************************
% INPUTS
%   - firstSpeed_deg_s, secondSpeed_deg_s (scalars)
%       Boundary speeds for one monotone transition.
...
%**************************************************************************
% OUTPUTS
%   - duration_s, jerk_deg_s3 (numeric column vectors)
%       Phase durations and signed jerk amplitudes.
%   - phaseLaw (string column vector)
%       Constant, sinusoidal, or modified-logistic phase laws.
%**************************************************************************
% UNITS
...
if changeMagnitude_deg_s == 0
    ...
    phaseLaw = repmat("constant", 3, 1);
    return;
end

if jerkProfileType == "continuoussine"
    pulseArea = 2 / pi;
    peakSlope = pi;
    pulseLaw = "sine";

    % Fang et al. (2020) use sinusoidal jerk to retain C3 motion while
    % exploiting the jerk and acceleration limits. See citation.md.
    jerkRampDuration_s = min(sqrt(changeMagnitude_deg_s / ...
        (pulseArea * jerkLimit_deg_s3)), ...
        accelerationLimit_deg_s2 / ...
        (pulseArea * jerkLimit_deg_s3));
    if isfinite(maximumTangentialSnap_deg_s4)
        snapDuration_s = min([ ...
            peakSlope * jerkLimit_deg_s3 / ...
            maximumTangentialSnap_deg_s4, ...
            sqrt(peakSlope * accelerationLimit_deg_s2 / ...
            (pulseArea * maximumTangentialSnap_deg_s4)), ...
            nthroot(peakSlope * changeMagnitude_deg_s / ...
            (pulseArea * maximumTangentialSnap_deg_s4), 3)]);
        jerkRampDuration_s = max(jerkRampDuration_s, snapDuration_s);
    end
    jerkAmplitude_deg_s3 = min([jerkLimit_deg_s3, ...
        accelerationLimit_deg_s2 / ...
        (pulseArea * jerkRampDuration_s), ...
        changeMagnitude_deg_s / ...
        (pulseArea * jerkRampDuration_s^2)]);
    peakAcceleration_deg_s2 = pulseArea * jerkAmplitude_deg_s3 * ...
        jerkRampDuration_s;
    constantAccelerationDuration_s = max(0, ...
        changeMagnitude_deg_s / peakAcceleration_deg_s2 - ...
        jerkRampDuration_s);
    phaseLaw = [pulseLaw; "constant"; pulseLaw];
elseif jerkProfileType == "continuoussigmoid"
    if ~isfinite(maximumTangentialSnap_deg_s4)
        error("retimeAzElSpatialPath:SigmoidRequiresSnapLimit", ...
            "continuousSigmoid requires a finite tangential snap limit.");
    end

    % Fang et al. (2019), Eqs. (19)-(35), first select the modified-logistic
    % ramp time from snap, jerk, acceleration, and required speed change.
    % The same hierarchy applies to one path-speed transition. It gives the
    % shortest symmetric transition in this fifteen-phase profile family.
    snapLimit_deg_s4 = maximumTangentialSnap_deg_s4;
    durationFromSpeed_s = nthroot( ...
        sqrt(3) * changeMagnitude_deg_s / (2 * snapLimit_deg_s4), 3);
    durationFromAcceleration_s = sqrt( ...
        sqrt(3) * accelerationLimit_deg_s2 / snapLimit_deg_s4);
    durationFromJerk_s = sqrt(3) * jerkLimit_deg_s3 / ...
        snapLimit_deg_s4;
    sigmoidRampDuration_s = min([durationFromSpeed_s, ...
        durationFromAcceleration_s, durationFromJerk_s]);
    jerkAmplitude_deg_s3 = snapLimit_deg_s4 * ...
        sigmoidRampDuration_s / sqrt(3);

    speedChangeWithoutPlateau_deg_s = 2 * jerkAmplitude_deg_s3 * ...
        sigmoidRampDuration_s^2;
    if changeMagnitude_deg_s <= speedChangeWithoutPlateau_deg_s * ...
            (1 + 32 * eps(changeMagnitude_deg_s))
        constantJerkDuration_s = 0;
        constantAccelerationDuration_s = 0;
    else
        requiredConstantJerkDuration_s = 0.5 * ( ...
            sqrt(sigmoidRampDuration_s^2 + ...
            4 * changeMagnitude_deg_s / jerkAmplitude_deg_s3) - ...
            3 * sigmoidRampDuration_s);
        accelerationConstantJerkDuration_s = max(0, ...
            accelerationLimit_deg_s2 / jerkAmplitude_deg_s3 - ...
            sigmoidRampDuration_s);
        constantJerkDuration_s = min(requiredConstantJerkDuration_s, ...
            accelerationConstantJerkDuration_s);
        peakAcceleration_deg_s2 = jerkAmplitude_deg_s3 * ...
            (sigmoidRampDuration_s + constantJerkDuration_s);
        constantAccelerationDuration_s = max(0, ...
            changeMagnitude_deg_s / peakAcceleration_deg_s2 - ...
            (2 * sigmoidRampDuration_s + constantJerkDuration_s));
    end

    duration_s = [sigmoidRampDuration_s; constantJerkDuration_s; ...
        sigmoidRampDuration_s; constantAccelerationDuration_s; ...
        sigmoidRampDuration_s; constantJerkDuration_s; ...
        sigmoidRampDuration_s];
    jerk_deg_s3 = direction * jerkAmplitude_deg_s3 * ...
        [1; 1; 1; 0; -1; -1; -1];
    phaseLaw = ["sigmoidRise"; "constant"; "sigmoidFall"; ...
        "constant"; "sigmoidRise"; "constant"; "sigmoidFall"];
    return;
else
    jerkRampDuration_s = min( ...
        sqrt(changeMagnitude_deg_s / jerkLimit_deg_s3), ...
        accelerationLimit_deg_s2 / jerkLimit_deg_s3);
    constantAccelerationDuration_s = max(0, ...
        changeMagnitude_deg_s / ...
        (jerkLimit_deg_s3 * jerkRampDuration_s) - ...
        jerkRampDuration_s);
    jerkAmplitude_deg_s3 = jerkLimit_deg_s3;
    phaseLaw = repmat("constant", 3, 1);
end
duration_s = [jerkRampDuration_s; constantAccelerationDuration_s; jerkRampDuration_s];
jerk_deg_s3 = direction * ...
    [jerkAmplitude_deg_s3; 0; -jerkAmplitude_deg_s3];
end

function [position_deg, speed_deg_s, acceleration_deg_s2, ...
        jerk_deg_s3] = evaluateProfilePhase(time_s, startPosition_deg, ...
        startSpeed_deg_s, startAcceleration_deg_s2, duration_s, ...
        jerkAmplitude_deg_s3, phaseLaw)
% PURPOSE
%   - Evaluate one constant, sine, or modified-logistic jerk phase.
time_s = min(max(double(time_s), 0), duration_s);
if phaseLaw == "sine" && duration_s > 0
    angularRate_rad_s = pi / duration_s;
    phase_rad = angularRate_rad_s .* time_s;
    jerk_deg_s3 = jerkAmplitude_deg_s3 .* sin(phase_rad);
    acceleration_deg_s2 = startAcceleration_deg_s2 + ...
        jerkAmplitude_deg_s3 / angularRate_rad_s .* ...
        (1 - cos(phase_rad));
    speed_deg_s = startSpeed_deg_s + ...
        startAcceleration_deg_s2 .* time_s + ...
        jerkAmplitude_deg_s3 / angularRate_rad_s .* time_s - ...
        jerkAmplitude_deg_s3 / angularRate_rad_s^2 .* sin(phase_rad);
    position_deg = startPosition_deg + startSpeed_deg_s .* time_s + ...
        0.5 * startAcceleration_deg_s2 .* time_s.^2 + ...
        0.5 * jerkAmplitude_deg_s3 / angularRate_rad_s .* time_s.^2 + ...
        jerkAmplitude_deg_s3 / angularRate_rad_s^3 .* ...
        (cos(phase_rad) - 1);
elseif (phaseLaw == "sigmoidRise" || phaseLaw == "sigmoidFall") && ...
        duration_s > 0
    normalizedTime = time_s ./ duration_s;
    [jerkShape, accelerationIntegral, velocityIntegral, ...
        positionIntegral] = sigmoidPhaseIntegrals(normalizedTime, phaseLaw);
    jerk_deg_s3 = jerkAmplitude_deg_s3 .* jerkShape;
    acceleration_deg_s2 = startAcceleration_deg_s2 + ...
        jerkAmplitude_deg_s3 .* duration_s .* accelerationIntegral;
    speed_deg_s = startSpeed_deg_s + ...
        startAcceleration_deg_s2 .* time_s + ...
        jerkAmplitude_deg_s3 .* duration_s^2 .* velocityIntegral;
    position_deg = startPosition_deg + startSpeed_deg_s .* time_s + ...
        0.5 * startAcceleration_deg_s2 .* time_s.^2 + ...
        jerkAmplitude_deg_s3 .* duration_s^3 .* positionIntegral;
else
    jerk_deg_s3 = repmat(jerkAmplitude_deg_s3, size(time_s));
    acceleration_deg_s2 = startAcceleration_deg_s2 + ...
        jerkAmplitude_deg_s3 .* time_s;
    speed_deg_s = startSpeed_deg_s + ...
        startAcceleration_deg_s2 .* time_s + ...
        0.5 * jerkAmplitude_deg_s3 .* time_s.^2;
    position_deg = startPosition_deg + startSpeed_deg_s .* time_s + ...
        0.5 * startAcceleration_deg_s2 .* time_s.^2 + ...
        jerkAmplitude_deg_s3 .* time_s.^3 / 6;
end
end

function peakSnap_deg_s4 = phasePeakSnap( ...
        phaseDuration_s, phaseJerk_deg_s3, phaseLaw, jerkProfileType)
% PURPOSE
%   - Return the certified peak scalar snap inside all profile phases.
if jerkProfileType == "piecewiseconstant"
    if any(abs(phaseJerk_deg_s3) > 0)
        peakSnap_deg_s4 = Inf;
    else
        peakSnap_deg_s4 = 0;
    end
...
peakSnap_deg_s4 = 0;
for phaseIndex = 1:numel(phaseLaw)
    duration_s = phaseDuration_s(phaseIndex);
    jerkAmplitude_deg_s3 = abs(phaseJerk_deg_s3(phaseIndex));
    if duration_s <= 0 || jerkAmplitude_deg_s3 == 0
        continue;
    end
    if phaseLaw(phaseIndex) == "sine"
        phasePeak_deg_s4 = pi * jerkAmplitude_deg_s3 / duration_s;
    elseif phaseLaw(phaseIndex) == "sigmoidRise" || ...
            phaseLaw(phaseIndex) == "sigmoidFall"
        phasePeak_deg_s4 = sqrt(3) * jerkAmplitude_deg_s3 / duration_s;
    else
        phasePeak_deg_s4 = 0;
    end
    peakSnap_deg_s4 = max(peakSnap_deg_s4, phasePeak_deg_s4);
end
end

function [jerkShape, accelerationIntegral, velocityIntegral, ...
        positionIntegral] = sigmoidPhaseIntegrals(normalizedTime, phaseLaw)
% PURPOSE
%   - Evaluate the Fang modified logistic and three normalized integrals.
originalSize = size(normalizedTime);
normalizedTime = min(max(normalizedTime(:), 0), 1);

% Composite sixteen-point Gauss-Legendre quadrature on four equal panels
% resolves this fixed, smooth law to near machine precision. Endpoint
% values are imposed from symmetry.
...
panelHalfWidth = 0.125 * normalizedTime;
quadratureTime = [ ...
    panelHalfWidth .* nodes.' + 0.125 * normalizedTime, ...
    panelHalfWidth .* nodes.' + 0.375 * normalizedTime, ...
    panelHalfWidth .* nodes.' + 0.625 * normalizedTime, ...
    panelHalfWidth .* nodes.' + 0.875 * normalizedTime];
panelWeight = panelHalfWidth .* weights.';
quadratureWeight = repmat(panelWeight, 1, 4);
riseShapeAtNodes = modifiedLogisticShape(quadratureTime);
remainingTime = normalizedTime - quadratureTime;
riseAccelerationIntegral = sum( ...
    quadratureWeight .* riseShapeAtNodes, 2);
riseVelocityIntegral = sum( ...
    quadratureWeight .* remainingTime .* riseShapeAtNodes, 2);
risePositionIntegral = 0.5 * sum( ...
    quadratureWeight .* remainingTime.^2 .* riseShapeAtNodes, 2);

jerkShape = modifiedLogisticShape(normalizedTime);
accelerationIntegral = riseAccelerationIntegral;
velocityIntegral = riseVelocityIntegral;
positionIntegral = risePositionIntegral;
if phaseLaw == "sigmoidFall"
    jerkShape = 1 - jerkShape;
    accelerationIntegral = normalizedTime - accelerationIntegral;
    velocityIntegral = 0.5 * normalizedTime.^2 - velocityIntegral;
    positionIntegral = normalizedTime.^3 / 6 - positionIntegral;
end

jerkShape = reshape(jerkShape, originalSize);
accelerationIntegral = reshape(accelerationIntegral, originalSize);
velocityIntegral = reshape(velocityIntegral, originalSize);
positionIntegral = reshape(positionIntegral, originalSize);
end

function value = modifiedLogisticShape(normalizedTime)
% PURPOSE
%   - Evaluate Fang et al. Eq. (5) without endpoint overflow.
value = zeros(size(normalizedTime));
atUpperEndpoint = normalizedTime >= 1;
interior = normalizedTime > 0 & normalizedTime < 1;
value(atUpperEndpoint) = 1;
if ~any(interior, "all")
    return;
end
interiorTime = normalizedTime(interior);
variationParameter = sqrt(3) / 2;
logit = variationParameter * (1 ./ (1 - interiorTime) - ...
    1 ./ interiorTime);
positiveLogit = logit >= 0;
interiorValue = zeros(size(logit));
interiorValue(positiveLogit) = 1 ./ ...
    (1 + exp(-logit(positiveLogit)));
negativeExponential = exp(logit(~positiveLogit));
interiorValue(~positiveLogit) = negativeExponential ./ ...
    (1 + negativeExponential);
value(interior) = interiorValue;
end

function [position_deg, speed_deg_s, acceleration_deg_s2] = ...
        integrateProfilePhase(startPosition_deg, startSpeed_deg_s, ...
        startAcceleration_deg_s2, duration_s, jerkAmplitude_deg_s3, ...
        phaseLaw)
% PURPOSE
%   - Integrate one analytic phase to its end state.
[position_deg, speed_deg_s, acceleration_deg_s2] = ...
    evaluateProfilePhase(duration_s, startPosition_deg, ...
    startSpeed_deg_s, startAcceleration_deg_s2, duration_s, ...
    jerkAmplitude_deg_s3, phaseLaw);
end

function acceleration_deg_s2 = phaseEndAccelerations( ...
        phaseStartAcceleration_deg_s2, phaseDuration_s, ...
        phaseJerk_deg_s3, phaseLaw)
% PURPOSE
%   - Evaluate every phase-end acceleration for certified scalar peaks.
phaseCount = numel(phaseDuration_s);
acceleration_deg_s2 = zeros(phaseCount, 1);
for phaseIndex = 1:phaseCount
    [~, ~, acceleration_deg_s2(phaseIndex)] = evaluateProfilePhase( ...
        phaseDuration_s(phaseIndex), 0, 0, ...
        phaseStartAcceleration_deg_s2(phaseIndex), ...
        phaseDuration_s(phaseIndex), phaseJerk_deg_s3(phaseIndex), ...
        phaseLaw(phaseIndex));
end
end

function isCertified = profilesHaveContinuousJerk( ...
        profiles, jerkProfileType, jerkConstrained, tolerance)
% PURPOSE
%   - Verify the analytic phase invariant required for continuous jerk.
continuousType = jerkProfileType == "continuoussine" || ...
    jerkProfileType == "continuoussigmoid";
isCertified = jerkConstrained && continuousType;
if ~isCertified
    return;
end
...
for profileIndex = 1:numel(profiles)
    ...
    if jerkProfileType == "continuoussine"
        validLaw = profile.PhaseLaw == "sine" | ...
            profile.PhaseLaw == "constant";
    else
        validLaw = profile.PhaseLaw == "sigmoidRise" | ...
            profile.PhaseLaw == "sigmoidFall" | ...
            profile.PhaseLaw == "constant";
    end
    [startJerk_deg_s3, endJerk_deg_s3] = phaseBoundaryJerk(profile);
    boundaryScale_deg_s3 = max(1, max(abs(profile.PhaseJerk_deg_s3)));
    boundaryTolerance_deg_s3 = tolerance * boundaryScale_deg_s3;
    hasContinuousJerk = abs(startJerk_deg_s3(1)) <= ...
        boundaryTolerance_deg_s3 && abs(endJerk_deg_s3(end)) <= ...
        boundaryTolerance_deg_s3 && all(abs(endJerk_deg_s3(1:end - 1) - ...
        startJerk_deg_s3(2:end)) <= boundaryTolerance_deg_s3);
    if ~all(validLaw) || ~hasContinuousJerk || ...
            profile.JerkProfileType ~= jerkProfileType
        isCertified = false;
        return;
    end
...
function isCertified = profilesHaveContinuousTangentialSnap( ...
        profiles, jerkProfileType, jerkConstrained, ~)
% PURPOSE
%   - Verify zero scalar snap at every modified-logistic phase boundary.
isCertified = jerkConstrained && jerkProfileType == "continuoussigmoid";
if ~isCertified
    return;
end
for profileIndex = 1:numel(profiles)
    ...
    validLaw = profile.PhaseLaw == "sigmoidRise" | ...
        ...
        profile.PhaseLaw == "constant";
    if ~all(validLaw) || ...
            profile.JerkProfileType ~= "continuoussigmoid"
        isCertified = false;
        return;
    end
end
...
function [startJerk_deg_s3, endJerk_deg_s3] = phaseBoundaryJerk(profile)
% PURPOSE
%   - Return exact one-sided jerk values for every analytic phase.
startJerk_deg_s3 = profile.PhaseJerk_deg_s3;
endJerk_deg_s3 = profile.PhaseJerk_deg_s3;
zeroEndpointLaw = profile.PhaseLaw == "sine";
startJerk_deg_s3(zeroEndpointLaw) = 0;
endJerk_deg_s3(zeroEndpointLaw) = 0;
riseLaw = profile.PhaseLaw == "sigmoidRise";
startJerk_deg_s3(riseLaw) = 0;
fallLaw = profile.PhaseLaw == "sigmoidFall";
endJerk_deg_s3(fallLaw) = 0;
end
...
phaseCount = numel(profile.PhaseDuration_s);
for index = 1:phaseCount
    if index < phaseCount
    ...
    [position_deg(belongs), speed_deg_s(belongs), ...
        acceleration_deg_s2(belongs), jerk_deg_s3(belongs)] = ...
        evaluateProfilePhase(localTime_s, ...
        profile.PhaseStartPosition_deg(index), ...
        profile.PhaseStartSpeed_deg_s(index), ...
        profile.PhaseStartAcceleration_deg_s2(index), ...
        profile.PhaseDuration_s(index), ...
        profile.PhaseJerk_deg_s3(index), profile.PhaseLaw(index));
end
...
if scaleFactor > 1 + tolerance
    ...
    for profileIndex = 1:profileCount
        ...
        profile.PeakTangentialSnap_deg_s4 = ...
            profile.PeakTangentialSnap_deg_s4 / scaleFactor^4;
...
diagnostics = struct( "PeakVelocity_deg_s", [NaN NaN], ...
    ...
    "ContinuousTangentialSnapCertified", false, ...
    "PeakTangentialSnap_deg_s4", NaN, ...
    "TangentialSnapConstrained", false, ...
    "TangentialSnapSatisfied", false, ...
    "TangentialSnapMargin_deg_s4", NaN, ...
    "JerkProfileType", options.JerkProfileType, ...
