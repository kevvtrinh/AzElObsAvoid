...
if jerkConstrained
    [nodeSpeed_deg_s, feasible, failureMessage] = ...
        ...
        options.JerkProfileType, tolerance);
end
if ~feasible
    timedPath = timedPathTemplate(limits, options, failureMessage);
    return;
...
function [nodeSpeed_deg_s, feasible, message] = ...
        ...
        mandatoryStopNode, jerkProfileType, tolerance)
%% Section 0: Header & Readme
% SYNTAX
...
%       Constant-jerk or continuous-sine transition family.
%   - mandatoryStopNode (logical vector), tolerance (scalar)
...
for passIndex = 1:max(8, 2 * (runCount + 1))
    ...
    for runIndex = 1:runCount
        reachable = reachableSpeed(nodeSpeed_deg_s(runIndex), ...
            ...
            maximumAcceleration_deg_s2(runIndex), ...
            maximumJerk_deg_s3(runIndex), jerkProfileType, tolerance);
...
function speed_deg_s = reachableSpeed(boundarySpeed_deg_s, distance_deg, ...
        ...
        jerkLimit_deg_s3, jerkProfileType, tolerance)
%% Section 0: Header & Readme
% SYNTAX
...
%   - phaseLaw (3-by-1 string vector)
%       Constant or sinusoidal analytic phase laws.
%**************************************************************************
% UNITS
...
if jerkProfileType == "continuoussine"
    ...
    % exploiting jerk and acceleration limits. See citation.md. The two
    % sine ramps have peak acceleration 2*J*T/pi and together contribute
    % that peak times T to the requested velocity change.
    jerkRampDuration_s = min(sqrt(pi * changeMagnitude_deg_s / ...
        (2 * jerkLimit_deg_s3)), ...
        pi * accelerationLimit_deg_s2 / (2 * jerkLimit_deg_s3));
    peakAcceleration_deg_s2 = 2 * jerkLimit_deg_s3 * ...
        jerkRampDuration_s / pi;
    constantAccelerationDuration_s = max(0, ...
        changeMagnitude_deg_s / peakAcceleration_deg_s2 - ...
        jerkRampDuration_s);
    phaseLaw = ["sine"; "constant"; "sine"];
else
...
% PURPOSE
%   - Evaluate one constant-jerk or continuous-sine-jerk phase.
...
isCertified = jerkConstrained && jerkProfileType == "continuoussine";
if ~isCertified
    return;
end
...
for profileIndex = 1:numel(profiles)
    ...
    hasZeroConstantJerk = all(abs( ...
        profile.PhaseJerk_deg_s3(constantPhase)) <= tolerance);
    if ~all(validLaw) || ~hasZeroConstantJerk || ...
            profile.JerkProfileType ~= "continuoussine"
        isCertified = false;
        return;
    end
end
