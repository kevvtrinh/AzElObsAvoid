...
if ~topologyMatches
    firstEdgeSet = packedSliceEdges( ...
        packedObstacle, lowerSliceIndex);
    lastEdgeSet = packedSliceEdges( ...
        packedObstacle, upperSliceIndex);
    return;
end
lowerEdges = packedSliceEdges( ...
    packedObstacle, lowerSliceIndex);
upperEdges = packedSliceEdges( ...
    packedObstacle, upperSliceIndex);
...
% PURPOSE
%   - Extract one packed slice without package dispatch in this hot path.
% The package-call form was 14.3 times slower than direct packed-array
% access in a focused timing test. Keep this small coordinate-order rule
% aligned with buildAzElTimeObstacleField.
