...
x/y frame. The planner owns obstacle interpretation, topology
...
Position uses units. Time uses seconds. Derivatives use units/s,
...
\begin{itemize}
    \item \textbf{Seed.} \texttt{position\_units} is N-by-2. \texttt{tau} strictly increases from zero to
    one.
    \item \textbf{Regions.} An R-by-1 cell array. Each cell holds one finite convex N-by-2
    exclusion polygon in units. Plane witnesses certify only these supplied
    regions.
    ...
    \texttt{ControlPoint\_units} array and \texttt{SegmentTime\_s}.
...
\draw[->](0,0)--(7,0)node[right]{x};\draw[->](0,0)--(0,5)node[above]{y};
...
regions\_units,\allowbreak{} coverage,\allowbreak{} initialState,\allowbreak{}
goalState,\allowbreak{} limits,\allowbreak{} options,\allowbreak{} warmStart)}
{\texttt{seed} & scalar struct & N-by-2 route and increasing \texttt{tau} & units \\
\texttt{regions\_units} & R-by-1 cell array & convex exclusion polygons & units \\
\texttt{coverage} & scalar struct & planner coverage proof with \texttt{Passed} & -- \\
\texttt{initialState}, \texttt{goalState} & scalar structs & rest endpoints and times & units, seconds \\
\texttt{limits}, \texttt{options} & scalar structs & bounds, time policy, work limits & mixed \\
\texttt{warmStart} & scalar struct or empty & certified control net and span time & units, seconds}
...
{\texttt{initialState}, \texttt{goalState} & scalar structs & time, position, optional rest derivatives & units, seconds \\
\texttt{limits} & scalar struct & positive velocity, acceleration, jerk bounds & derivative units \\
\texttt{options} & scalar struct & goal-time mode, sample time, tolerance & seconds, units}
...
\texttt{initialState} & scalar struct & start state & units, seconds \\
...
knotTime\_s,\allowbreak{} knotOffset\_units,\allowbreak{} axisIndex,\allowbreak{}
...
\texttt{knotTime\_s}, \texttt{knotOffset\_units} & matching vectors & increasing offset knots & seconds, units \\
...
amplitude\_units,\allowbreak{} sampleStep\_s,\allowbreak{} seedSource)}
{\texttt{baseMotion} & scalar result struct & successful two-coordinate straight progress & -- \\
\texttt{initialState}, \texttt{goalState} & scalar structs & two-coordinate rest endpoints & units, seconds \\
\texttt{progressAxisIndex}, \texttt{amplitude\_units} & integer, scalar & progress coordinate and lateral coefficient & --, units \\
...
relativeBreak\_s,\allowbreak{} segmentJerk\_units\_s3,\allowbreak{}
...
\texttt{segmentJerk\allowbreak\_units\_s3} & N-by-D numeric & constant interval jerk & units per second cubed \\
\texttt{sampleStep\_s}, \texttt{seedSource} & scalar, text & spacing and label & seconds, --}
...
{[coordinateScale\_units, geometryTolerance\_units, roundoffReserve\_units]
=\allowbreak{} bmtpEngine.createCoordinateTolerances(values\_units,\allowbreak{} ...)}
{\texttt{values\_units} & numeric arrays or cells & coordinate collections; nonfinite values ignored & units}
{\texttt{coordinateScale\_units} & maximum finite absolute coordinate, at least one coordinate unit \\
\texttt{geometryTolerance\_units} & shared geometric tolerance \\
\texttt{roundoffReserve\_units} & conservative finite-precision reserve}
...
{[time\_s, position\_units, velocity\_units\_s, acceleration\_units\_s2,
jerk\_units\_s3] =\allowbreak{} bmtpEngine.evaluatePolynomial(polynomial,\allowbreak{}
...
\texttt{position\_units} through \texttt{jerk\_units\_s3} & N-by-D histories}
...
{distance\_units =\allowbreak{}
bmtpEngine.maximumRestToRestDistance(duration\_s,\allowbreak{}
velocityLimit\_units\_s,\allowbreak{} accelerationLimit\_units\_s2,\allowbreak{}
jerkLimit\_units\_s3)}
{\texttt{duration\_s} & nonnegative scalar & available motion duration & seconds \\
\texttt{velocityLimit\allowbreak\_units\_s} & positive scalar & symmetric velocity limit & units per second \\
\texttt{accelerationLimit\allowbreak\_units\_s2} & positive scalar & symmetric acceleration limit & units per second squared \\
\texttt{jerkLimit\_units\_s3} & positive scalar & symmetric jerk limit & units per second cubed}
{\texttt{distance\_units} & exact maximum displacement}
...
{\texttt{polynomial} & scalar struct & source segments with local powers & units, seconds \\
...
{Invalid polynomial records, span counts, and units raise errors. A valid
...
regions\_units,\allowbreak{} collisionClearanceTolerance\_units)}
{\texttt{warmStart} & scalar struct & control points and positive span time & units, seconds \\
\texttt{regions\_units} & cell vector & finite convex protected polygons & units \\
\texttt{collisionClearance\allowbreak Tolerance\_units} & nonnegative scalar & required separation after reserve & units}
...
\begin{itemize}
    \item \textbf{Cache the immutable SOCP template.} Removed. The cache added
    threading and schema ownership for a modest measured speed benefit. Direct
    construction is smaller, remained result-identical, and incurred a 9.4\%
    warmed runtime cost on the declared cleanup gate.
    \item \textbf{Separating-plane reuse across alternations.} Retained as an
    automatic continuation rule. Reuse occurs only when the retained-best
    improvement is within \texttt{ArrivalTimeTolerance\_s} and the tagged
    path--region pair set is unchanged. The mechanism and its diagnostics stay
    internal to BMTP rather than creating planner options.
...
\begin{itemize}
    \item \textbf{Testing.} The complete MATLAB suite checks every change.
    ...
    A/B repetitions.
    \item \textbf{Failure visibility.} Iteration limits, exhausted alternation, unsupported
...
\begin{itemize}
    \item Can automatic plane reuse receive a stronger convergence certificate
    without exposing an alternation-policy option?
