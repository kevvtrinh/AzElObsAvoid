...
[result, summaryTemplate] = obstacleAvoidance.planner.createPlanningRecord(obstacles, initialState, goalState, limits, options, ...
...
result.SearchDiagnostics.SelectionPolicy = struct( ...
    "GoalTimeMode", options.GoalTimeMode, "JerkRole", "hardConstraintOnly");
...
seeds = obstacleAvoidance.search.createPathGuesses( ...
...
for seedIndex = 1:primarySeedCount
    ...
    if primarySummaries(seedIndex).ValidationPassed && ...
...
result.SearchDiagnostics.StageTiming = obstacleAvoidance.planner.reconcileStageTiming( ...
    timing, toc(planningTimer));
