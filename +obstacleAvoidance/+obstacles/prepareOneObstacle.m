...
function preparation = prepareSourceInterval(preparation, obstacle, intervalIndex, ...
    ...
    if isempty(product)
        [matched, alignedUpper_units, startRegions_units, endRegions_units, ...
            ...
            ringSample(lowerX_units, lowerY_units, preparation.SampleShapes{intervalIndex}), ...
            ringSample(upperX_units, upperY_units, preparation.SampleShapes{finalSampleIndex}), ...
            reusableStartRegions_units, ...
            proofRequest(preserveAlignment, translationOnly, false));
    else
        [matched, alignedUpper_units, startRegions_units, endRegions_units, ...
    ...
    if ~matched && finalSampleIndex > intervalIndex + 1
        ...
        [matched, alignedUpper_units, startRegions_units, endRegions_units, ...
            ...
            ringSample(upperX_units, upperY_units, preparation.SampleShapes{finalSampleIndex}), ...
            reusableStartRegions_units, ...
            proofRequest(protectedKeepsIndex, translationOnly, false));
    end
    ...
    for batchIndex = 1:numel(lowerX_units)
        ...
        try
            ...
            [product{:}, dependsOnPrevious] = ...
                obstacleAvoidance.obstacles.alignVerifiedSingleRing( ...
                ringSample(lowerX_units{batchIndex}, lowerY_units{batchIndex}, ...
                    lowerShapes{batchIndex}), ...
                ringSample(upperX_units{batchIndex}, upperY_units{batchIndex}, ...
                    upperShapes{batchIndex}), ...
                cell(0, 1), proofRequest(protectedKeepsIndex, translationOnly, true));
...
function sample = ringSample(x_units, y_units, shape)
    % One obstacle sample as the certifier reads it: the protected ring and
    % the same sample as a polyshape.
    sample = struct('X_units', x_units, 'Y_units', y_units, 'Shape', shape);
end

function request = proofRequest(preserveAlignment, translationOnly, deferTranslation)
    % Which proofs the certifier should try for one interval.
    request = struct('PreserveAlignment', preserveAlignment, ...
        'TranslationOnly', translationOnly, 'DeferTranslation', deferTranslation);
end
