...
%       Boundary, bounds, speed, topology, and source-slice metadata.
...
if active
    bounds_deg = [min(azimuth_deg(finiteVertex)), ...
        max(azimuth_deg(finiteVertex)), ...
        min(elevation_deg(finiteVertex)), ...
        max(elevation_deg(finiteVertex))];
end
...
geometry = struct("Active", active, "azimuth_deg", double(azimuth_deg(:)), ...
    "elevation_deg", double(elevation_deg(:)), "Bounds_deg", bounds_deg, ...
    "VertexSpeedBound_deg_s", speed_deg_s, ...
