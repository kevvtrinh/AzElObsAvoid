...
result.SearchDiagnostics.SeedEarlyExit = struct( ...
    "Applied", false, "Reason", "notApplicableExactPath", ...
    "PhysicalArrivalLowerBound_s", NaN, "ReachedBySeedIndex", 0);
...
seedSolveContext = struct( ...
    ...
    "StaticBmtpRepresentation", staticBmtpRepresentation, ...
    "SweptBmtpRepresentation", sweptBmtpRepresentation, ...
    "Projection", projection, "PreparedObstacles", preparedObstacles, ...
...
result.SearchDiagnostics.SeedEarlyExit.Reason = "lowerBoundNotReached";
...
for seedIndex = 1:seedCount
    [candidate, summary, validation, stageTiming] = solveOneSeed( ...
        seeds(seedIndex), sweptSeedIsEligible(seedIndex), ...
        seedSolveContext, stageTiming);
    ...
    if reachesPhysicalArrivalLowerBound( ...
        ...
        result.SearchDiagnostics.SeedEarlyExit.ReachedBySeedIndex = seedIndex;
        break;
    end
end
...
function [candidate, summary, validation, stageTiming] = solveOneSeed( ...
        seed, trySweptProjection, context, stageTiming)
...
elseif context.UseStaticKernel
    ...
    [candidate, solverDiagnostics] = ...
        ...
        seed, context.StaticBmtpRepresentation, initialState, ...
    ...
    if trySweptProjection
        ...
        [sweptCandidate, sweptDiagnostics] = ...
            obstacleAvoidance.planner.solveBmtpTrajectory( ...
            seed, context.SweptBmtpRepresentation, initialState, ...
            kernelGoalState, limits, options);
        ...
        sweptAttempt = createSweptProjectionRecord( ...
            sweptDiagnostics, sweptValidation, context.Projection);
...
if ~candidateWasPrevalidated
    ...
    waitRefinementAffectsObjective = any(options.GoalTimeMode == ...
        ["earliestArrival", "balancedArrival"]);
...
summary = summarizeCandidate(candidate, validation, solverDiagnostics, ...
    elapsedTime_s, context.SummaryTemplate, limits, options, ...
    initialState.time_s);
end

function reached = reachesPhysicalArrivalLowerBound( ...
        summary, validation, lowerBound_s, options)
% Stop earliest-arrival search only at the exact request-wide physical floor.
