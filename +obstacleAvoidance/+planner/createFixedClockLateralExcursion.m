...
coarseLevelCount = 8;
workspaceScale_deg = max(diff(workspaceInterval_deg, 1, 2), [], "all");
[boundaryResolution_deg, maximumBoundaryRefinementCount] = ...
    fixedClockRefinementPolicy(options, workspaceScale_deg);
diagnostics.BoundaryResolutionTarget_deg = boundaryResolution_deg;
diagnostics.MaximumBoundaryRefinementCount = ...
    maximumBoundaryRefinementCount;
...
for axisIndex = 1:dimensionCount
    ...
    for direction = [-1, 1]
        for peakIndex = 1:numel(peakTime_s)
            ...
            report.BoundaryResolutionTarget_deg = boundaryResolution_deg;
            report.MaximumBoundaryRefinementCount = ...
                maximumBoundaryRefinementCount;
            if axisGovernsClock
            ...
            while upperMagnitude_deg - lowerMagnitude_deg > ...
                    boundaryResolution_deg && refinementCount < ...
                    maximumBoundaryRefinementCount
...
[amplitudeResolution_deg, maximumBoundaryRefinementCount] = ...
    fixedClockRefinementPolicy(options, workspaceScale_deg);
report.BoundaryResolutionTarget_deg = amplitudeResolution_deg;
report.MaximumBoundaryRefinementCount = maximumBoundaryRefinementCount;
...
bestBasisIndex = 0;
for basisIndex = 1:basisCount
    basisReports(basisIndex).BoundaryResolutionTarget_deg = ...
        amplitudeResolution_deg;
    basisReports(basisIndex).MaximumBoundaryRefinementCount = ...
        maximumBoundaryRefinementCount;
    ...
    refinementCount = 0;
    while upperMagnitude_deg - lowerMagnitude_deg > ...
            amplitudeResolution_deg && refinementCount < ...
            maximumBoundaryRefinementCount
        midpointMagnitude_deg = ...
            0.5 * (lowerMagnitude_deg + upperMagnitude_deg);
...
report = struct( ...
    ...
    "BoundaryResolutionTarget_deg", NaN, ...
    "MaximumBoundaryRefinementCount", 0, ...
...
function [resolution_deg, maximumRefinementCount] = ...
        fixedClockRefinementPolicy(options, workspaceScale_deg)
% Stop amplitude bisection at a physical angular scale, not machine scale.
% One millidegree is 3.6 arcseconds and keeps the final retained candidate
% on the independently validated side of every collision boundary.
physicalAmplitudeTolerance_deg = 1e-3;
maximumRefinementCount = 12;
roundoffReserve_deg = 64 * eps(max(1, abs(workspaceScale_deg)));
resolution_deg = max([physicalAmplitudeTolerance_deg, ...
