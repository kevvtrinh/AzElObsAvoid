...
[corridorInequality, corridorNormal] = corridorConstraints( ...
    ...
    constraintLayout);
monotonicityInequality = monotonicityConstraints( ...
    polynomial, constraintLayout);
inequality = [continuousBoundConstraints(polynomial, limits, options); ...
    monotonicityInequality; ...
...
function inequality = monotonicityConstraints(polynomial, constraintLayout)
% Bound tangent velocity below by zero over every complete HS3 segment.
% Nonnegative Bernstein coefficients are a conservative whole-segment
% certificate; activation is therefore limited to certified rest-to-rest lines.
...
projectedPower_deg_s = sum(polynomial.velocityPower_deg_s .* ...
    reshape(tangent, 1, 2, 1), 2);
coefficientCount = size(projectedPower_deg_s, 3);
coefficientMatrix = reshape(permute( ...
    projectedPower_deg_s, [3 1 2]), coefficientCount, []);
bernsteinVelocity_deg_s = ...
    hs3Internal.polynomial.convertPowerToBernstein(coefficientMatrix);
inequality = -bernsteinVelocity_deg_s(:);
end
