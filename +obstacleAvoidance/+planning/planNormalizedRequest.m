...
% The initial-snapshot vertex visibility classifies every boundary vertex pair once.
...
visibilityGraph = getVisibilityGraph( ...
    scene.vertexVisibility, request.initialState.position_units, request.goalState.position_units, ...
...
[candidate, solverDiagnostics] = bmtpEngine.solve(seed, scene, ...
...
function graph = getVisibilityGraph(vertexVisibility, start_units, goal_units, kind)
    % Attach this request's endpoints to a classified snapshot vertex visibility.
...
function search = runStaticSpatialStage(search, scene, request, ...
    ...
    graph        = getVisibilityGraph(scene.vertexVisibility, request.initialState.position_units, ...
...
function result = planFixedArrivalDynamic(result, scene, request, totalTimer)
    ...
    for snapshotIndex = 1:numel(snapshotKinds)
        ...
        vertexVisibility = scene.vertexVisibility;
        if snapshotIndex == 2
