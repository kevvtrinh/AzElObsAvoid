...
useFreeGoalWindow = ~isFixedArrival && ...
    ((isfield(timedSearch,'DynamicBoundChangedGoalLayer') && ...
    timedSearch.DynamicBoundChangedGoalLayer) || ...
    (isfield(timedSearch,'SelectedGoalWindowIndex') && ...
    timedSearch.SelectedGoalWindowIndex>1));
...
if useFreeGoalWindow
    ...
    if seedRouteTime_s(end)<motionGoalState.time_s
        seedRoute_units=[seedRoute_units;seedRoute_units(end,:)];
        seedRouteTime_s=[seedRouteTime_s;motionGoalState.time_s];
    end
    cells=obstacleAvoidance.obstacles.createTimeCells( ...
        previous.PreparedObstacles,initialState.time_s,motionGoalState.time_s,true);
    ...
    [regions_units,coverage]=obstacleAvoidance.obstacles.createTimedBmtpCells( ...
        previous.PreparedObstacles,initialState.time_s,motionGoalState.time_s,16);
end
...
if useFreeGoalWindow, seed.TimingMode="variableClock"; end
...
if candidate.Success && ~useFreeGoalWindow
    [candidate,diagnostics] = certifyAgainstExactCells(candidate,diagnostics, ...
        seed,previous.PreparedObstacles,initialState,motionGoalState, ...
        previous.RequestedLimits,motionOptions);
end
