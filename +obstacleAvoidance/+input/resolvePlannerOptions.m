...
if ~isstruct(optionOverrides) || ~isscalar(optionOverrides)
    error("planner:InvalidOptions", "optionOverrides must be a scalar struct.");
end
for fieldName = ["XInterval_units", "YInterval_units"]
    if isfield(optionOverrides, fieldName)
        ...
        error("planner:WorkspaceLimitMoved", "%s has moved from options to limits.%s.", fieldName, replacementName);
    end
end
[options, unknownNames] = obstacleAvoidance.input.resolveOptions(defaults, optionOverrides);
if ~isempty(unknownNames)
    warning("planner:UnknownOptions", "Ignoring unknown option fields: %s. No behavior changed.", strjoin(unknownNames, ", "));
end
...
options.WrapX = obstacleAvoidance.input.normalizeLogicalScalar(options.WrapX, "WrapX", "planner:InvalidLogicalOption");
options.WrapY = obstacleAvoidance.input.normalizeLogicalScalar(options.WrapY, "WrapY", "planner:InvalidLogicalOption");
