...
%       MaximumSeedCount is an integer from 1 through 9. The cap bounds the
%       expensive solve-and-validate work after one required direct seed and
%       at most one timed seed; remaining seeds represent spatial classes.
%       MaximumTimeLayerCount is an integer at least 2 and requests a
%       timed-search layer ceiling, including both endpoints. The effective
%       count is also bounded by MaximumTimedSearchStateCount and
%       MaximumTimedSearchTransitionCount. Defaults are 17, 50000, and
...
defaults = struct( ...
    ...
    "MaximumTimedSearchStateCount", 50000, ...
    "MaximumTimedSearchTransitionCount", 1000000, ...
...
integerRules = {"MaximumSeedCount", 1, 9; ...
    "MaximumTimeLayerCount", 2, Inf; ...
    "MaximumTimedSearchStateCount", 1, Inf; ...
    "MaximumTimedSearchTransitionCount", 1, Inf; ...
    "MaximumWaitRefinementIterations", 0, 64};
for ruleIndex = 1:size(integerRules, 1)
    validateattributes(options.(integerRules{ruleIndex, 1}), ...
        {'numeric'}, {'real', 'finite', 'scalar', 'integer', ...
        '>=', integerRules{ruleIndex, 2}, ...
        '<=', integerRules{ruleIndex, 3}});
end
