...
% INPUTS: Public result containing the original request and any prior attempt.
...
% Generate only the prefix that can fit the explicit budget. Source knots
% are included separately before truncating the merged chronological list.
count = min(options.MaxArrivalTrials, floor((horizon_s-start_s)/resolution_s));
times_s = unique([start_s+(1:count)'*resolution_s;boundaries_s]);
times_s = times_s(times_s>initial.time_s & times_s>=start_s & times_s<=horizon_s);
...
searchedEnd_s = initial.time_s;
if tried>0, searchedEnd_s = times_s(end); end
breaks_s = unique([initial.time_s;times_s;horizon_s]);
