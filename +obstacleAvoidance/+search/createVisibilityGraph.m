...
%       vertex visibility, start_units, goal_units)
%**************************************************************************
% PURPOSE
%   - Attach one start and one goal to a vertex visibility and select the
%     shortest exact route through the classified vertex pairs.
%**************************************************************************
% INPUTS
%   - vertexVisibility (scalar struct)
%       Classified vertex visibility from createVertexVisibility.
...
%% Section 1: Place The Endpoints Among The Vertex Visibility Vertices
...
sourceFree      = pointIsFree(start_units, vertexVisibility, tolerance_units);
goalFree        = pointIsFree(goal_units, vertexVisibility, tolerance_units);
endpoints_units = [start_units; goal_units];
workspaceFree   = all(endpoints_units >= vertexVisibility.WorkspaceMinimum_units & ...
...
if sourceFree && goalFree
    ...
    for firstNode = 1:2
        ...
        clear      = obstacleAvoidance.search.classifyVisibilitySegments( ...
            vertexVisibility, nodes_units, cones, firstNode, secondNode);
    ...
    % the vertex visibility's verdict, renumbered into this node list.
    [~, nodeOfVertex] = ismember(vertexVisibility.Vertices_units, nodes_units, 'rows');
    vertexRetained    = nodeOfVertex > 2;
    acceptedParts{3}  = renumberPairs(vertexVisibility.VertexPairAccepted, nodeOfVertex, vertexRetained);
    rejectedParts{3}  = renumberPairs(vertexVisibility.VertexPairRejected, nodeOfVertex, vertexRetained);
...
function free = pointIsFree(point_units, vertexVisibility, tolerance_units)
    ...
    edge_units = vertexVisibility.EdgeVector_units;
    fraction = min(1, max(0, sum((point_units - vertexVisibility.EdgeStart_units) .* edge_units, 2) ./ sum(edge_units.^2, 2)));
    distance_units = vecnorm(point_units - vertexVisibility.EdgeStart_units - fraction .* edge_units, 2, 2);
    free = ~inside && all(distance_units > tolerance_units);
end

function pairs = renumberPairs(vertexPairs, nodeOfVertex, vertexRetained)
    % Keep the vertex visibility's pair order; it already ascends by first then
