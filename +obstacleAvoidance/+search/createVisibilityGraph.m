...
%**************************************************************************
% PURPOSE
%   - Build the exhaustive exact visibility graph and shortest route.
%**************************************************************************
% INPUTS
...
if sourceFree && goalFree
    ...
    for firstNode = 1:nodeCount - 1
        ...
        [checked, queryPoints_units, queryOwner] = segmentIntervals( ...
            nodes_units(firstNode, :), nodes_units(secondNode(checkIndex), :), ...
            edgeStart_units, edgeEnd_units, edgeVector_units, edgeBounds_units, ...
...
function cones = endpointCones(shape, nodes_units, edgeStart_units, edgeEnd_units, tolerance_units)
    ...
    hasConsistentSide = side(incoming) == side(outgoing) & side(incoming) ~= 0;
    turnScale_units2 = vecnorm(cones.Incoming, 2, 2) .* vecnorm(cones.Outgoing, 2, 2);
    turnIsResolved = abs(turn_units2) > tolerance_units * max(1, turnScale_units2);
