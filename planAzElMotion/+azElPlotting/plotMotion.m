function handles = plotMotion(result, optionOverrides)
%% Section 0: Header & Readme
% SYNTAX
%   options = azElPlotting.plotMotion()
...
for name = logicalNames
    options.(name) = azElInput.normalizeLogicalScalar( options.(name), name, "plotAzElMotion:InvalidLogicalOption");
end
...
obstacles = azElObstacles.prepareDynamic(result.Inputs.obstacles);
...
goal_deg = azElInput.goalPositionAtTime( result.Inputs.goalState, result.Inputs.goalState.time_s);
...
for obstacleIndex = 1:numel(obstacles)
    ...
    if showOriginal
        ...
        originalShape = azElObstacles.shapeAtTime( originalObstacle, time_s);
        drawShape(axesHandle, originalShape, obstacleColor, [0.12 0.12 0.12], "-", 0.22, "Original obstacle");
    end
    protectedShape = azElObstacles.shapeAtTime(obstacle, time_s);
...
for obstacleIndex = 1:numel(obstacles)
    ...
    for timeIndex = reshape(timeIndices, 1, [])
        shape = azElObstacles.shapeAtTime( obstacles(obstacleIndex), times_s(timeIndex));
        if isempty(shape.Vertices)
            previousAzimuth_deg = zeros(0, 1);
            previousElevation_deg = zeros(0, 1);
            previousTime_s = NaN;
...
if isfield(goalState, "targetPosition_deg") && ~isempty(goalState.targetPosition_deg)
    ...
    targetPosition_deg = azElInput.goalPositionAtTime( goalState, displayTime_s);
...
if isfield(goalState, "targetPosition_deg") && ~isempty(goalState.targetPosition_deg)
    plot3(axesHandle, goalState.targetPosition_deg(:, 1), ...
        ...
        "-.", "Color", [0.65 0.10 0.65], "LineWidth", 1.4, "DisplayName", "Moving target track");
targetPosition_deg = azElInput.goalPositionAtTime( goalState, goalState.time_s);
