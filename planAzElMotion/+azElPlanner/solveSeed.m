...
if maximumSolverTime_s <= 0
    ...
    candidate.SolverDiagnostics = ...
        azElPlanner.emptySolverDiagnostics( ...
...
seedCorridor = azElSearch.buildSeedCorridor(seed, segmentCount);
...
constraintFunction = @(decision) ...
    azElPlanner.evaluateTrajectoryConstraints( ...
    ...
    reconstructFunction, collinearityDirection);
...
if ~isEarliestArrival
    ...
    if size(linearInequalityMatrix, 1) ~= numel(zeroInequality) || ...
        ...
        error("solveSeed:ConstraintGradientSizeMismatch", ...
...
if isEarliestArrival
    ...
    stageOneObjective = @(decision) ...
        hs3Internal.integratedSquaredJerk(decision, false, latestFinalTime_s, ...
...
if ~isEarliestArrival
    [~, ~, problem.Hessian] = hs3Internal.integratedSquaredJerk( ...
        decision0, false, latestFinalTime_s, segmentCount, startTime_s, 2);
end
core = hs3Internal.optimize(problem, coreOptions);
...
[~, basePosition_deg, baseVelocity_deg_s, baseAcceleration_deg_s2] = ...
    azElPlanner.evaluateAzElPolynomial( ...
        basePolynomial, evaluationTime_s);
desiredPosition_deg(end, :) = azElInput.goalPositionAtTime( ...
...
if azElSearch.seedEnvelopeContainsObstacles( ...
...
for controlIndex = 1:numel(controlTau)
    ...
    for obstacleIndex = 1:numel(obstacles)
        [~, geometry] = azElObstacles.shapeAtTime( ...
        ...
        if geometry.HasOrderedSingleRegion
            ...
            shape = azElGeometry.boundaryShape( ...
...
%**************************************************************************
% INPUTS
%   - jerk_deg_s3 ((2N+1)-by-2 numeric)
%       Azimuth/elevation jerk ordinates in degrees per second cubed.
%   - initialState (scalar normalized planner state struct)
%       Unit-bearing initial time, position, velocity, and acceleration.
%   - finalTime_s (finite numeric scalar)
%       Absolute terminal time in seconds.
%   - segmentCount (positive integer scalar)
%       Number of equal-duration HS3 polynomial segments.
%**************************************************************************
% OUTPUTS
%   - polynomial (scalar struct)
%       Unit-bearing Az/El view of the dimension-neutral HS3 polynomial.
%**************************************************************************
% UNITS
...
enginePolynomial = hs3Internal.reconstructPolynomial( ...
...
[time_s, position_deg, velocity_deg_s, ...
    acceleration_deg_s2, jerk_deg_s3] = azElPlanner.evaluateAzElPolynomial( ...
    polynomial, time_s);
end
