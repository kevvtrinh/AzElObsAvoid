...
if isSameTimePath
    ...
    if ~resolvedOptions.StopAtFirstCollision
        % Validate all candidate chords for one obstacle in one call. This
        % retains obstacle-index blocking priority and avoids repeated packed
        % input validation for every dense trajectory segment.
        segmentStart_deg = position_deg(1:end - 1, :);
        segmentEnd_deg = position_deg(2:end, :);
        segmentBounds_deg = [ ...
        ...
        candidateObstacleBySegment = obstacleIsActive.' & ...
        ...
        candidateObstacleSegmentPairCount = nnz( ...
            candidateObstacleBySegment);
        broadPhaseRejectedObstacleSegmentPairCount = ...
            nnz(obstacleIsActive) * segmentCount - ...
            candidateObstacleSegmentPairCount;
        for obstacleIndex = 1:obstacleCount
            candidateSegmentIndex = find( ...
                candidateObstacleBySegment(:, obstacleIndex) & ...
                ~segmentOccupied);
            if isempty(candidateSegmentIndex)
                continue;
            end
            pairPositionIndex = reshape([candidateSegmentIndex, ...
                candidateSegmentIndex + 1].', [], 1);
            [obstacleSegmentOccupied, ~, blockingSliceIndex] = ...
                azElInternal.queryPackedMovingObstacle( ...
                packedObstacles(obstacleIndex), ...
                time_s(pairPositionIndex), ...
                position_deg(pairPositionIndex, :), ...
                resolvedOptions.BoundaryIsOccupied, ...
                resolvedOptions.TimePaddingSamples, true);
            evaluatedObstacleSegmentPairCount = ...
                evaluatedObstacleSegmentPairCount + ...
                numel(candidateSegmentIndex);
            newlyBlocked = logical(obstacleSegmentOccupied(:));
            blockedSegmentIndex = candidateSegmentIndex(newlyBlocked);
            segmentOccupied(blockedSegmentIndex) = true;
            segmentBlockingObstacleIndex(blockedSegmentIndex) = ...
                uint32(obstacleIndex);
            segmentBlockingSliceIndex(blockedSegmentIndex) = uint32( ...
                blockingSliceIndex(newlyBlocked));
        end
        evaluatedSegmentCount = segmentCount;
    else
